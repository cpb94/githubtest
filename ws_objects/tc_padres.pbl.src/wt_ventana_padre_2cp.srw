$PBExportHeader$wt_ventana_padre_2cp.srw
forward
global type wt_ventana_padre_2cp from window
end type
type pb_imprimir from picturebutton within wt_ventana_padre_2cp
end type
type p_logo from picture within wt_ventana_padre_2cp
end type
type dw_1 from datawindow within wt_ventana_padre_2cp
end type
type pb_borrar from picturebutton within wt_ventana_padre_2cp
end type
type pb_salir from picturebutton within wt_ventana_padre_2cp
end type
type pb_grabar from picturebutton within wt_ventana_padre_2cp
end type
type st_titulo from statictext within wt_ventana_padre_2cp
end type
type pb_nuevo from picturebutton within wt_ventana_padre_2cp
end type
type pb_cancelar from picturebutton within wt_ventana_padre_2cp
end type
type pb_buscar from picturebutton within wt_ventana_padre_2cp
end type
end forward
...
global wt_ventana_padre_2cp wt_ventana_padre_2cp
...
string padre_codigo1, padre_codigo2
...
if dw_1.rowcount() > 0 then
	...
	if String(dw_1.object.#2[dw_1.getrow()]) = '' or isnull (dw_1.object.#2[dw_1.getrow()]) then
		padre_codigo2 = String(f_colisiones ( trans_ts, param_colisiones))
		if padre_codigo2 = '0'  then
			messagebox("ERROR", "No se puede asignar un código.")
			rtn = -1
			ROLLBACK USING trans_ts;
		else
			dw_1.object.#2[dw_1.getrow()] = padre_codigo2
			rtn = 1
		end if
	end if
end if
...
if borrar = 1 then
	padre_codigo1 = dw_1.object.#1[dw_1.GetRow()]
	padre_codigo2 = dw_1.object.#2[dw_1.GetRow()]
	dw_1.SetTransObject(trans_ts)	
	...
	if rtn = 1 then
		...
		ROLLBACK USING trans_ts;
		dw_1.object.#1[dw_1.GetRow()] = padre_codigo1
		dw_1.object.#2[dw_1.GetRow()] = padre_codigo2
		pb_cancelar.triggerevent(Clicked!)
...
on wt_ventana_padre_2cp.create
...
on wt_ventana_padre_2cp.destroy
...
//else
	...
	IF lstr_param.i_nargumentos >= 2 THEN
		...
		if not isnull(lstr_param.s_argumentos[1]) and lstr_param.s_argumentos[1] <> "" and not isnull(lstr_param.s_argumentos[2]) and lstr_param.s_argumentos[2] <> "" then
			if dw_1.retrieve (codigo_empresa, lstr_param.s_argumentos[1], lstr_param.s_argumentos[2]) <> 1 then
				padre_codigo1 = ""
				padre_codigo2 = ""
				pb_nuevo.triggerevent(clicked!)
				mostrar_controles_insercion()
				...
				padre_codigo1 = String(lstr_param.s_argumentos[1])
				padre_codigo2 = String(lstr_param.s_argumentos[2])
...
event doubleclicked; if dwo.name = this.object.#2.name then
	pb_buscar.triggerevent(clicked!)
else
...
event clicked;if estado = 1 then
	padre_codigo1 = dw_1.object.#1[dw_1.getrow()]
	padre_codigo2 = dw_1.object.#2[dw_1.getrow()]
	dw_1.retrieve (codigo_empresa, padre_codigo1, padre_codigo2)
	mostrar_controles_edicion()
...
event clicked;long numero_valores
...
if resultado_busqueda.error = '' then
	...
	if not isnull(numero_valores) and numero_valores >= 4 then
