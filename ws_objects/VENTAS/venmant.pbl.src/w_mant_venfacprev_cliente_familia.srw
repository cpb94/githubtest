$PBExportHeader$w_mant_venfacprev_cliente_familia.srw
forward
global type w_mant_venfacprev_cliente_familia from w_int_con_empresa
end type
type pb_1 from upb_salir within w_mant_venfacprev_cliente_familia
end type
type pb_2 from upb_imprimir within w_mant_venfacprev_cliente_familia
end type
type em_fechadesde from u_em_campo within w_mant_venfacprev_cliente_familia
end type
type em_fechahasta from u_em_campo within w_mant_venfacprev_cliente_familia
end type
type gb_5 from groupbox within w_mant_venfacprev_cliente_familia
end type
type cb_familias from commandbutton within w_mant_venfacprev_cliente_familia
end type
type gb_1 from groupbox within w_mant_venfacprev_cliente_familia
end type
type cb_1 from u_boton within w_mant_venfacprev_cliente_familia
end type
type cb_paises from commandbutton within w_mant_venfacprev_cliente_familia
end type
type gb_3 from groupbox within w_mant_venfacprev_cliente_familia
end type
type uo_familias from u_marca_lista within w_mant_venfacprev_cliente_familia
end type
type dw_detalle from u_datawindow_consultas within w_mant_venfacprev_cliente_familia
end type
type uo_cliente from u_em_campo_2 within w_mant_venfacprev_cliente_familia
end type
type em_ejercicio from u_em_campo within w_mant_venfacprev_cliente_familia
end type
type cb_2 from commandbutton within w_mant_venfacprev_cliente_familia
end type
type uo_cliente_anyadir from u_em_campo_2 within w_mant_venfacprev_cliente_familia
end type
type pb_anyadir_pais from picturebutton within w_mant_venfacprev_cliente_familia
end type
type gb_2 from groupbox within w_mant_venfacprev_cliente_familia
end type
type gb_4 from groupbox within w_mant_venfacprev_cliente_familia
end type
type gb_6 from groupbox within w_mant_venfacprev_cliente_familia
end type
type uo_paises from u_marca_lista within w_mant_venfacprev_cliente_familia
end type
type dw_export_excel from datawindow within w_mant_venfacprev_cliente_familia
end type
type dw_import_excel from datawindow within w_mant_venfacprev_cliente_familia
end type
type cb_exportar from u_boton within w_mant_venfacprev_cliente_familia
end type
type cb_importar from u_boton within w_mant_venfacprev_cliente_familia
end type
type gb_excel from groupbox within w_mant_venfacprev_cliente_familia
end type
end forward
...
global w_mant_venfacprev_cliente_familia w_mant_venfacprev_cliente_familia
...
public subroutine f_control_m2 (datawindow arg_datawindow);string ls_codigo,ls_filtro,ls_filtro_paises,ls_filtro_clientes,ls_filtro_pavrev,ls_filtro_familias,ls_filtro_formatos,ls_filtro_cliente
...
ls_sel = "SELECT resultado.empresa, ~n"+&
			...
			"         isnull(paises.pais,''), ~n"+&
			"         isnull(paises.nombre,'') ~n"+&
			"UNION ALL ~n"+&
         "SELECT isnull(genter.empresa,'') as empresa, ~n"+&
			...
			"         isnull(paises.nombre,'') ~n"+&
			"UNION ALL ~n"+&
         "SELECT isnull(venfacprev_cliente_familia.empresa,'') as empresa, ~n"+&
			"       isnull(venfacprev_cliente_familia.genter_codigo,'') as genter_codigo, ~n"+&
			"       isnull(genter.razon,'') as genter_descripcion, ~n"+&
			"		  isnull(venfacprev_cliente_familia.familias_codigo,'') as familias_codigo, ~n"+&
			"		  isnull(familias.descripcion,'') as familias_descripcion, ~n"+&
			"       sum(venfacprev_cliente_familia.prevision_precio_medio) as prevision_precio_medio, ~n"+&
			"		  0.00 as m2, ~n"+&
			"		  0.00 as importe, ~n"+&
			"       sum(venfacprev_cliente_familia.prevision_importe) as prevision_importe, ~n"+&
			"       0.00 as prevision_metros, ~n"+&
			"		  '€' as moneda, ~n"+&
			"		  '###,###,##0.00' as mascara_moneda, ~n"+&
			"       '' as venagentes_agente, ~n"+&
			...
			"       isnull(paises.pais,'') as paises_codigo, ~n"+&
			"       isnull(paises.nombre,'') as paises_descripcion ~n"+&
			" FROM venfacprev_cliente_familia, ~n"+&
			"      genter, ~n"+&
			"      venclientes, ~n"+&
			"		 paises, ~n"+&
			"		 familias ~n"+&
			"WHERE ( venfacprev_cliente_familia.empresa        = genter.empresa ) ~n"+&
			"and   ( 'C'                                       = genter.tipoter ) ~n"+&
			"and   ( venfacprev_cliente_familia.genter_codigo  = genter.codigo ) ~n"+&
			"and   ( genter.activo                             = 'S' ) ~n"+&
			"and   ( venfacprev_cliente_familia.empresa        = venclientes.empresa ) ~n"+&
			"and   ( venfacprev_cliente_familia.genter_codigo  = venclientes.codigo ) ~n"+&
			"and   ( genter.pais                                = paises.pais ) ~n"+&
			"and   ( venfacprev_cliente_familia.empresa         = familias.empresa ) ~n"+&
			"and   ( venfacprev_cliente_familia.familias_codigo = familias.codigo ) ~n"+&
			ls_filtro_cliente+" ~n"+&
			ls_filtro_paises+" ~n"+&
			ls_filtro_clientes+" ~n"+&
			ls_filtro_familias+" ~n"+&
			"and   ( ( venfacprev_cliente_familia.empresa = '"+codigo_empresa+"' ) ~n"+&
			"and     ( venfacprev_cliente_familia.ejercicio = "+string(ll_ejercicio_presupuesto,"###0")+" ) ~n"+&			
			" ) ~n"+&
			"GROUP BY venfacprev_cliente_familia.empresa, ~n"+&
			"         venfacprev_cliente_familia.genter_codigo, ~n"+&
			"         genter.razon, ~n"+&
			"		    venfacprev_cliente_familia.familias_codigo, ~n"+&
			"		    familias.descripcion, ~n"+&
			"         isnull(paises.pais,''), ~n"+&
			"         isnull(paises.nombre,'') ) as resultado ~n"+&
...
public subroutine f_anyadir_cliente (datawindow arg_datawindow, string arg_pais);string ls_codigo,ls_filtro,ls_filtro_cliente,ls_filtro_agentes,ls_filtro_pavrev,ls_filtro_familias,ls_filtro_formatos,ls_filtro_agente
string ls_sel,ls_campos_select,ls_campos_group_by,ls_filtro_articulos,ls_sort_actual,ls_texto_seleccion,ls_texto_filtro
integer li_indice,li_opciones,li_opciones_marcadas
long    ll_indice,ll_total,ll_ejercicio_presupuesto
datetime ldt_fecha_desde,ldt_fecha_hasta

str_venfacprev_cliente_familia istr_venfacprev_cliente_familia
...
ls_sel = "SELECT resultado.empresa, ~n"+&
			...
			"      articulos, ~n"+&
			"		 familias ~n"+&
			"WHERE ( genter.empresa      = articulos.empresa ) ~n"+&
			"and   ( genter.pais         = paises.pais ) ~n"+&
			"and   ( articulos.empresa   = familias.empresa ) ~n"+&
			"and   ( articulos.familia   = familias.codigo ) ~n"+&
			ls_filtro_cliente+" ~n"+&
...
if ll_total > 0 then
	if ds_datos.RowsCopy(1,ll_total, Primary!, arg_datawindow, 1, Primary!) = 1 then
		for ll_indice = 1 to ll_total
			istr_venfacprev_cliente_familia.empresa                = ds_datos.object.empresa[ll_indice]
			istr_venfacprev_cliente_familia.ejercicio              = dec(em_ejercicio.text)
			istr_venfacprev_cliente_familia.genter_codigo          = ds_datos.object.genter_codigo[ll_indice]
			istr_venfacprev_cliente_familia.familias_codigo        = ds_datos.object.familias_codigo[ll_indice]
			istr_venfacprev_cliente_familia.prevision_precio_medio = 0
			istr_venfacprev_cliente_familia.prevision_importe      = 0
			istr_venfacprev_cliente_familia.prevision_metros       = 0
			//istr_venfacprev_cliente_familia.venagentes_agente      = ""
			
			if f_insert_venfacprev_cliente_familia(istr_venfacprev_cliente_familia) then
				commit;
			else
				rollback;
			end if		
...
istr_parametros.s_titulo_ventana = "Presupuestos Cliente/Familia"
...
on w_mant_venfacprev_cliente_familia.create
int iCurrent
call super::create
...
this.Control[iCurrent+14]=this.em_ejercicio
this.Control[iCurrent+15]=this.cb_2
this.Control[iCurrent+16]=this.uo_cliente_anyadir
this.Control[iCurrent+17]=this.pb_anyadir_pais
this.Control[iCurrent+18]=this.gb_2
this.Control[iCurrent+19]=this.gb_4
this.Control[iCurrent+20]=this.gb_6
this.Control[iCurrent+21]=this.uo_paises
this.Control[iCurrent+22]=this.dw_export_excel
this.Control[iCurrent+23]=this.dw_import_excel
this.Control[iCurrent+24]=this.cb_exportar
this.Control[iCurrent+25]=this.cb_importar
this.Control[iCurrent+26]=this.gb_excel
end on

on w_mant_venfacprev_cliente_familia.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_mant_venfacprev_cliente_familia
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_mant_venfacprev_cliente_familia
integer x = 110
end type

type st_empresa from w_int_con_empresa`st_empresa within w_mant_venfacprev_cliente_familia
integer x = 9
integer y = 8
integer width = 2336
integer height = 108
...
string dataobject = "dw_mant_venfacprev_cliente_familia"
...
event itemchanged;call super::itemchanged;str_venfacprev_cliente_familia istr_venfacprev_cliente_familia
...
//else
	choose case dwo.name
		case "prevision_precio_medio"
			istr_venfacprev_cliente_familia.empresa                = this.object.empresa[row]
			istr_venfacprev_cliente_familia.ejercicio              = dec(em_ejercicio.text)
			istr_venfacprev_cliente_familia.genter_codigo          = this.object.genter_codigo[row]
			istr_venfacprev_cliente_familia.familias_codigo        = this.object.familias_codigo[row]
			istr_venfacprev_cliente_familia.prevision_precio_medio = dec(data)
			istr_venfacprev_cliente_familia.prevision_importe      = round(this.object.prevision_metros[row] * dec(data),2)
			istr_venfacprev_cliente_familia.prevision_metros       = this.object.prevision_metros[row]
			
			if f_insert_venfacprev_cliente_familia(istr_venfacprev_cliente_familia) then
				commit;
				this.object.prevision_importe[row] = istr_venfacprev_cliente_familia.prevision_importe
			else
				rollback;
				return 2
			end if
		case "prevision_importe"
			...
			istr_venfacprev_cliente_familia.genter_codigo          = this.object.genter_codigo[row]
			istr_venfacprev_cliente_familia.familias_codigo        = this.object.familias_codigo[row]
			istr_venfacprev_cliente_familia.prevision_precio_medio = this.object.prevision_precio_medio[row]
			istr_venfacprev_cliente_familia.prevision_importe      = dec(data)
			if this.object.prevision_precio_medio[row] <> 0 then
				istr_venfacprev_cliente_familia.prevision_metros       = round((dec(data) / this.object.prevision_precio_medio[row]),2)
			else
				if this.object.prevision_metros[row] <> 0 then
					this.object.prevision_precio_medio[row] = round((dec(data) / this.object.prevision_metros[row]),2)
				else
					//istr_venfacprev_cliente_familia.prevision_metros       = 0
				end if
			end if
			...
			if f_insert_venfacprev_cliente_familia(istr_venfacprev_cliente_familia) then
				commit;
				this.object.prevision_metros[row] = istr_venfacprev_cliente_familia.prevision_metros
			else
				rollback;
				return 2
			end if		
		case "prevision_metros"
			...
			istr_venfacprev_cliente_familia.familias_codigo        = this.object.familias_codigo[row]
			istr_venfacprev_cliente_familia.prevision_precio_medio = this.object.prevision_precio_medio[row]
			istr_venfacprev_cliente_familia.prevision_importe      = round((dec(data) * this.object.prevision_precio_medio[row]),2)
			istr_venfacprev_cliente_familia.prevision_metros       = dec(data)
...
string dataobject = "dw_mant_venfacprev_cliente_familia_export"
...
string dataobject = "dw_mant_venfacprev_cliente_familia_import"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = styleraised!
end type
...
event clicked;call super::clicked;long ll_columnas = 11 , ll_fila_desde = 3,ll_total_detalle
	...
	If getfilesavename ('Fichero EXCEL',lsPath,lsFichero,'XLS','XLS Files (*.XLS), *.XLS') = 1 then
		...
		if not uo_excel.f_importar_libreoffice ( dw_import_excel, lsPath, ll_columnas , ll_fila_desde ) then
			...
			for ll_indice = 1 to ll_total
				...
				ls_familia                = dw_import_excel.object.familias_codigo[ll_indice]
				...
				ll_donde = dw_detalle.find("genter_codigo = '"+ls_cliente+"' and familias_codigo = '"+ls_familia+"'",1,ll_total_detalle)
				...
				if ll_donde > 0 then				
					...
					dw_detalle.event itemchanged(ll_donde,dw_detalle.object.prevision_importe,string(dw_detalle.object.prevision_importe[ll_donde]))
				end if
			next
