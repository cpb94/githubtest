$PBExportHeader$w_mant_venfacprev_cliente_submarca.srw
forward
global type w_mant_venfacprev_cliente_submarca from w_int_con_empresa
end type
type pb_1 from upb_salir within w_mant_venfacprev_cliente_submarca
end type
type pb_2 from upb_imprimir within w_mant_venfacprev_cliente_submarca
end type
type em_fechadesde from u_em_campo within w_mant_venfacprev_cliente_submarca
end type
type em_fechahasta from u_em_campo within w_mant_venfacprev_cliente_submarca
end type
type gb_5 from groupbox within w_mant_venfacprev_cliente_submarca
end type
type cb_familias from commandbutton within w_mant_venfacprev_cliente_submarca
end type
type gb_1 from groupbox within w_mant_venfacprev_cliente_submarca
end type
type cb_1 from u_boton within w_mant_venfacprev_cliente_submarca
end type
type cb_paises from commandbutton within w_mant_venfacprev_cliente_submarca
end type
type cb_marcas from commandbutton within w_mant_venfacprev_cliente_submarca
end type
type cb_submarcas from commandbutton within w_mant_venfacprev_cliente_submarca
end type
type gb_3 from groupbox within w_mant_venfacprev_cliente_submarca
end type
type uo_submarcas from u_marca_lista within w_mant_venfacprev_cliente_submarca
end type
type uo_marcas from u_marca_lista within w_mant_venfacprev_cliente_submarca
end type
type uo_familias from u_marca_lista within w_mant_venfacprev_cliente_submarca
end type
type dw_detalle from u_datawindow_consultas within w_mant_venfacprev_cliente_submarca
end type
type uo_cliente from u_em_campo_2 within w_mant_venfacprev_cliente_submarca
end type
type em_ejercicio from u_em_campo within w_mant_venfacprev_cliente_submarca
end type
type cb_2 from commandbutton within w_mant_venfacprev_cliente_submarca
end type
type uo_cliente_anyadir from u_em_campo_2 within w_mant_venfacprev_cliente_submarca
end type
type pb_anyadir_pais from picturebutton within w_mant_venfacprev_cliente_submarca
end type
type gb_2 from groupbox within w_mant_venfacprev_cliente_submarca
end type
type gb_4 from groupbox within w_mant_venfacprev_cliente_submarca
end type
type gb_6 from groupbox within w_mant_venfacprev_cliente_submarca
end type
type uo_paises from u_marca_lista within w_mant_venfacprev_cliente_submarca
end type
end forward
...
global w_mant_venfacprev_cliente_submarca w_mant_venfacprev_cliente_submarca
...
public subroutine f_control_m2 (datawindow arg_datawindow);string ls_codigo,ls_filtro,ls_filtro_paises,ls_filtro_clientes,ls_filtro_pavrev,ls_filtro_familias,ls_filtro_formatos,ls_filtro_marcas,ls_filtro_submarcas,ls_filtro_cliente
...
ls_sel = "SELECT resultado.empresa, ~n"+&
         ...
         "SELECT isnull(genter.empresa,'') as empresa, ~n"+&
			...
			"		    submarcas.descripcion ~n"+&
			"UNION ALL ~n"+&
         "SELECT isnull(venfacprev_cliente_submarca.empresa,'') as empresa, ~n"+&
			"       isnull(venfacprev_cliente_submarca.genter_codigo,'') as genter_codigo, ~n"+&
			"       isnull(genter.razon,'') as genter_descripcion, ~n"+&
			"		  isnull(venfacprev_cliente_submarca.familias_codigo,'') as familias_codigo, ~n"+&
			"		  isnull(familias.descripcion,'') as familias_descripcion, ~n"+&
			"		  isnull(venfacprev_cliente_submarca.marcas_codigo,'') as marcas_codigo, ~n"+&
			"		  isnull(marcas.descripcion,'') as marcas_descripcion, ~n"+&
			"		  isnull(venfacprev_cliente_submarca.submarcas_codigo,'') as submarcas_codigo, ~n"+&
			"		  isnull(submarcas.descripcion,'') as submarcas_descripcion, ~n"+&
			"       sum(venfacprev_cliente_submarca.prevision_precio_medio) as prevision_precio_medio, ~n"+&
			"		  0.00 as m2, ~n"+&
			"		  0.00 as importe, ~n"+&
			"       sum(venfacprev_cliente_submarca.prevision_importe) as prevision_importe, ~n"+&
			"       0.00 as prevision_metros, ~n"+&
			"		  '€' as moneda, ~n"+&
			"		  '###,###,##0.00' as mascara_moneda, ~n"+&
			"       '' as venagentes_agente, ~n"+&
			"       '' as venagentes_nombre ~n"+&
			" FROM venfacprev_cliente_submarca, ~n"+&
			"      genter, ~n"+&
			"      venclientes, ~n"+&
			"		 paises, ~n"+&
			"		 familias, ~n"+&
			"		 marcas, ~n"+&
			"		 submarcas ~n"+&
			"WHERE ( venfacprev_cliente_submarca.empresa        = genter.empresa ) ~n"+&
			"and   ( 'C'                                        = genter.tipoter ) ~n"+&
			"and   ( venfacprev_cliente_submarca.genter_codigo  = genter.codigo ) ~n"+&
			"and   ( genter.activo                              = 'S' ) ~n"+&
			"and   ( venfacprev_cliente_submarca.empresa        = venclientes.empresa ) ~n"+&
			"and   ( venfacprev_cliente_submarca.genter_codigo  = venclientes.codigo ) ~n"+&
			"and   ( genter.pais                                = paises.pais ) ~n"+&
			"and   ( venfacprev_cliente_submarca.empresa   = familias.empresa ) ~n"+&
			"and   ( venfacprev_cliente_submarca.familias_codigo = familias.codigo ) ~n"+&
			"and   ( venfacprev_cliente_submarca.empresa   = marcas.empresa ) ~n"+&
			"and   ( venfacprev_cliente_submarca.familias_codigo = marcas.familias_codigo ) ~n"+&
			"and   ( venfacprev_cliente_submarca.marcas_codigo = marcas.codigo ) ~n"+&
			"and   ( venfacprev_cliente_submarca.empresa = submarcas.empresa ) ~n"+&
			"and   ( venfacprev_cliente_submarca.familias_codigo = submarcas.familias_codigo ) ~n"+&
			"and   ( venfacprev_cliente_submarca.marcas_codigo = submarcas.marcas_codigo ) ~n"+&
			"and   ( venfacprev_cliente_submarca.submarcas_codigo = submarcas.codigo ) ~n"+&
			ls_filtro_cliente+" ~n"+&
			ls_filtro_paises+" ~n"+&
			ls_filtro_clientes+" ~n"+&
			ls_filtro_familias+" ~n"+&
			ls_filtro_marcas+" ~n"+&
			ls_filtro_submarcas+" ~n"+&
			"and   ( ( venfacprev_cliente_submarca.empresa = '"+codigo_empresa+"' ) ~n"+&
			"and     ( venfacprev_cliente_submarca.ejercicio = "+string(ll_ejercicio_presupuesto,"###0")+" ) ~n"+&			
			" ) ~n"+&
			"GROUP BY venfacprev_cliente_submarca.empresa, ~n"+&
			"         venfacprev_cliente_submarca.genter_codigo, ~n"+&
			"         genter.razon, ~n"+&
			"		    venfacprev_cliente_submarca.familias_codigo, ~n"+&
			"		    familias.descripcion, ~n"+&
			"		    venfacprev_cliente_submarca.marcas_codigo, ~n"+&
			"		    marcas.descripcion, ~n"+&
			"		    venfacprev_cliente_submarca.submarcas_codigo, ~n"+&
...
str_venfacprev_cliente_submarca istr_venfacprev_cliente_submarca
...
if ll_total > 0 then
	if ds_datos.RowsCopy(1,ll_total, Primary!, arg_datawindow, 1, Primary!) = 1 then
		for ll_indice = 1 to ll_total
			istr_venfacprev_cliente_submarca.empresa                = ds_datos.object.empresa[ll_indice]
			istr_venfacprev_cliente_submarca.ejercicio              = dec(em_ejercicio.text)
			istr_venfacprev_cliente_submarca.genter_codigo          = ds_datos.object.genter_codigo[ll_indice]
			istr_venfacprev_cliente_submarca.familias_codigo        = ds_datos.object.familias_codigo[ll_indice]
			istr_venfacprev_cliente_submarca.marcas_codigo          = ds_datos.object.marcas_codigo[ll_indice]
			istr_venfacprev_cliente_submarca.submarcas_codigo       = ds_datos.object.submarcas_codigo[ll_indice]
			istr_venfacprev_cliente_submarca.prevision_precio_medio = 0
			istr_venfacprev_cliente_submarca.prevision_importe      = 0
			istr_venfacprev_cliente_submarca.prevision_metros       = 0
			//istr_venfacprev_cliente_submarca.venagentes_agente      = ""
			
			if f_insert_venfacprev_cliente_submarca(istr_venfacprev_cliente_submarca) then
				commit;
			else
				rollback;
			end if		
...
on w_mant_venfacprev_cliente_submarca.create
int iCurrent
call super::create
...
this.Control[iCurrent+20]=this.uo_cliente_anyadir
...
on w_mant_venfacprev_cliente_submarca.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_mant_venfacprev_cliente_submarca
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_mant_venfacprev_cliente_submarca
integer x = 110
end type

type st_empresa from w_int_con_empresa`st_empresa within w_mant_venfacprev_cliente_submarca
integer x = 9
integer y = 8
integer width = 2336
integer height = 108
...
string dataobject = "dw_mant_venfacprev_cliente_submarca"
...
event itemchanged;call super::itemchanged;str_venfacprev_cliente_submarca istr_venfacprev_cliente_submarca
...
//else
	choose case dwo.name
		case "prevision_precio_medio"
			istr_venfacprev_cliente_submarca.empresa                = this.object.empresa[row]
			istr_venfacprev_cliente_submarca.ejercicio              = dec(em_ejercicio.text)
			istr_venfacprev_cliente_submarca.genter_codigo          = this.object.genter_codigo[row]
			istr_venfacprev_cliente_submarca.familias_codigo        = this.object.familias_codigo[row]
			istr_venfacprev_cliente_submarca.marcas_codigo          = this.object.marcas_codigo[row]
			istr_venfacprev_cliente_submarca.submarcas_codigo       = this.object.submarcas_codigo[row]
			istr_venfacprev_cliente_submarca.prevision_precio_medio = dec(data)
			istr_venfacprev_cliente_submarca.prevision_importe      = round(this.object.prevision_metros[row] * dec(data),2)
			istr_venfacprev_cliente_submarca.prevision_metros       = this.object.prevision_metros[row]
			
			if f_insert_venfacprev_cliente_submarca(istr_venfacprev_cliente_submarca) then
				commit;
				this.object.prevision_importe[row] = istr_venfacprev_cliente_submarca.prevision_importe
			else
				rollback;
				return 2
			end if
		case "prevision_importe"
			...
			istr_venfacprev_cliente_submarca.marcas_codigo          = this.object.marcas_codigo[row]
			istr_venfacprev_cliente_submarca.submarcas_codigo       = this.object.submarcas_codigo[row]
			istr_venfacprev_cliente_submarca.prevision_precio_medio = this.object.prevision_precio_medio[row]
			istr_venfacprev_cliente_submarca.prevision_importe      = dec(data)
			if this.object.prevision_precio_medio[row] <> 0 then
				istr_venfacprev_cliente_submarca.prevision_metros       = round((dec(data) / this.object.prevision_precio_medio[row]),2)
			else
				istr_venfacprev_cliente_submarca.prevision_metros       = 0
			end if
			...
			if f_insert_venfacprev_cliente_submarca(istr_venfacprev_cliente_submarca) then
				commit;
				this.object.prevision_metros[row] = istr_venfacprev_cliente_submarca.prevision_metros
			else
				rollback;
				return 2
			end if		
		case "prevision_metros"
			...
			istr_venfacprev_cliente_submarca.submarcas_codigo       = this.object.submarcas_codigo[row]
			istr_venfacprev_cliente_submarca.prevision_precio_medio = this.object.prevision_precio_medio[row]
			istr_venfacprev_cliente_submarca.prevision_importe      = round((dec(data) * this.object.prevision_precio_medio[row]),2)
			istr_venfacprev_cliente_submarca.prevision_metros       = dec(data)
