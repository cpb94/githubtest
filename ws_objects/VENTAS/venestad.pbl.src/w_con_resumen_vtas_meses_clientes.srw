$PBExportHeader$w_con_resumen_vtas_meses_clientes.srw
forward
global type w_con_resumen_vtas_meses_clientes from w_int_con_empresa
end type
type gb_6 from groupbox within w_con_resumen_vtas_meses_clientes
end type
type gb_5 from groupbox within w_con_resumen_vtas_meses_clientes
end type
type gb_3 from groupbox within w_con_resumen_vtas_meses_clientes
end type
type gb_2 from groupbox within w_con_resumen_vtas_meses_clientes
end type
type gb_1 from groupbox within w_con_resumen_vtas_meses_clientes
end type
type em_anyo from u_em_campo within w_con_resumen_vtas_meses_clientes
end type
type uo_mes1 from u_em_campo_2 within w_con_resumen_vtas_meses_clientes
end type
type uo_mes2 from u_em_campo_2 within w_con_resumen_vtas_meses_clientes
end type
type lbx_comparacion from dropdownlistbox within w_con_resumen_vtas_meses_clientes
end type
type em_cantidad from u_em_campo within w_con_resumen_vtas_meses_clientes
end type
type uo_serie from u_em_campo_2 within w_con_resumen_vtas_meses_clientes
end type
type gb_4 from groupbox within w_con_resumen_vtas_meses_clientes
end type
type uo_cliente from u_em_campo_2 within w_con_resumen_vtas_meses_clientes
end type
type rb_um from radiobutton within w_con_resumen_vtas_meses_clientes
end type
type rb_piezas from radiobutton within w_con_resumen_vtas_meses_clientes
end type
type rb_m2 from radiobutton within w_con_resumen_vtas_meses_clientes
end type
type rb_ml from radiobutton within w_con_resumen_vtas_meses_clientes
end type
type pb_1 from upb_salir within w_con_resumen_vtas_meses_clientes
end type
type pb_2 from upb_imprimir within w_con_resumen_vtas_meses_clientes
end type
type cb_1 from u_boton within w_con_resumen_vtas_meses_clientes
end type
type dw_detalle_cliente from u_datawindow_consultas within w_con_resumen_vtas_meses_clientes
end type
type dw_listado_clientes from u_datawindow within w_con_resumen_vtas_meses_clientes
end type
type cbx_otra from checkbox within w_con_resumen_vtas_meses_clientes
end type
type cb_2 from commandbutton within w_con_resumen_vtas_meses_clientes
end type
type cbx_conjunta from checkbox within w_con_resumen_vtas_meses_clientes
end type
type dw_cursor_venlifac1 from datawindow within w_con_resumen_vtas_meses_clientes
end type
type dw_cursor_venlifac_mes from datawindow within w_con_resumen_vtas_meses_clientes
end type
end forward
...
dw_cursor_venlifac1 dw_cursor_venlifac1
dw_cursor_venlifac_mes dw_cursor_venlifac_mes
end type
global w_con_resumen_vtas_meses_clientes w_con_resumen_vtas_meses_clientes
...
public subroutine f_cargar_um (datawindow data, datawindow data1);String Sel,serie1,serie2,cliente,nombre_cliente, serie,cli1,cli2, &
	restriccion,unidad,filtro,resul,empresa
Dec  cantidad,mov,vtotal,vtotalpts,por,tot,t,cant, mes1,mes2,mes,cont
Integer r,s,c,d,reg, meses, i,j,conta,anyo
	...
	string otra_empresa
		...
		if not (cbx_conjunta.checked and &
			...
			for cont = mes1 to mes2
				...
				select 	sum((total_neto - impdtopp)*cambio) 
				into  	:cant_ptas[cont]
				from 		venfac
				where		venfac.empresa like :empresa
...
public subroutine f_cargar_cantidad (datawindow data, string unidad, datawindow data1);String Sel,serie1,serie2,cliente,nombre_cliente, serie,cli1,cli2, &
	restriccion,filtro,resul,resul2,empresa,uni
Dec  cantidad,mov,vtotal,vtotalpts,por,tot,t,cant, mes1,mes2,mes,cont
...
if not cbx_otra.checked and not cbx_conjunta.checked then
	// no verán los datos de la otra empresa 
	restriccion = "cliente <>'"+trim(f_cliente_mercaderia(codigo_empresa))+"'"
else // los verán
	restriccion = ""
end if
...
registros = dw_cursor_venlifac1.retrieve(empresa,serie1,serie2,cli1,cli2,anyo,mes1,mes2,unidad)
...
For reg = 1 To registros
	empresa = dw_cursor_venlifac1.GetItemString(reg,"empresa")
	cliente = dw_cursor_venlifac1.GetItemString(reg,"cliente")
	serie   = dw_cursor_venlifac1.GetItemString(reg,"serie")
	nombre_cliente = f_razon_genter(codigo_empresa,"C",cliente)
	...
	if busca = 0 then // si el cliente no está todavía, lo doy de alta
		...
		resul = dw_cursor_venlifac1.Describe("evaluate ('if ("+filtro+",1,0)',"+String(reg,"###")+")")
		
		//// excluimos la otra empresa en caso de no haberla seleccionado \\
		if not cbx_otra.checked and not cbx_conjunta.checked then
			if cliente = trim(f_cliente_mercaderia(codigo_empresa)) then
				resul2 = "0"
			else
				resul2 = "1"
			end if
			//resul2 = dw_cursor_venlifac1.Describe("evaluate ('if ("+restriccion+",1,0)',"+String(reg,"###")+")")
			//messagebox ("Describe","evaluate ('if ("+restriccion+",1,0)',"+String(reg,"###")+")")
		else
			resul2 = "1"
		end if
		
		if resul = "1" and resul2 <> "0" then //inserto el cliente puesto que cumple la condición
			string otra_empresa
			otra_empresa = f_mercaderia_empresa(codigo_empresa)
			// cómo mola este if eh? 
			...
			if not (cbx_conjunta.checked and &
				...
				for cont = mes1 to mes2
					...
					dw_cursor_venlifac_mes.reset()
					tot_mes = dw_cursor_venlifac_mes.retrieve(empresa,cliente,cont,unidad,ANYO)
					if tot_mes = 0 then 
						cant_uni[cont] = 0
					else
						cant_uni[cont]=dw_cursor_venlifac_mes.GetItemNumber(1,"cant_cliente")
					end if
			...
			if resul2 = "1" then
				...
				for cont = mes1 to mes2
					...
					if tot_mes = 0 then 
						...
						cant = dw_cursor_venlifac_mes.GetItemNumber(1,"cant_cliente")
					end if
	...
	f_mensaje_proceso("Leyendo facturas",reg,dw_cursor_venlifac1.RowCount())
Next
...
on w_con_resumen_vtas_meses_clientes.create
int iCurrent
call super::create
...
this.dw_cursor_venlifac1=create dw_cursor_venlifac1
...
this.Control[iCurrent+25]=this.cbx_conjunta
this.Control[iCurrent+26]=this.dw_cursor_venlifac1
this.Control[iCurrent+27]=this.dw_cursor_venlifac_mes
end on

on w_con_resumen_vtas_meses_clientes.destroy
call super::destroy
...
destroy(this.dw_cursor_venlifac1)
...
//dw_detalle_cliente.Sharedata(dw_listado_clientes)
dw_cursor_venlifac1.SetTransObject(sqlca)
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_resumen_vtas_meses_clientes
integer taborder = 10
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_resumen_vtas_meses_clientes
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_resumen_vtas_meses_clientes
integer width = 2665
end type
...
//	end if	
//end if
IF dw_listado_clientes.rowcount() > 0 then
...
integer taborder = 0
boolean bringtotop = true
string dataobject = "dw_con_resumen_vtas_meses_clientes"
...
integer taborder = 50
boolean bringtotop = true
string dataobject = "report_con_resumen_vtas_meses_clientes"
end type
...
integer taborder = 60
boolean bringtotop = true
string dataobject = "dw_cursor_cantidad"
boolean vscrollbar = true
...
boolean bringtotop = true
string dataobject = "dw_cursor_cantidad2"
boolean vscrollbar = true
boolean livescroll = true
end type
