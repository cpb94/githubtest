$PBExportHeader$w_int_ventarifas.srw
forward
global type w_int_ventarifas from w_int_con_empresa
end type
type pb_1 from upb_salir within w_int_ventarifas
end type
type st_2 from statictext within w_int_ventarifas
end type
type em_tarifa from u_em_campo within w_int_ventarifas
end type
type st_nombre_tarifa from statictext within w_int_ventarifas
end type
type dw_fechas from datawindow within w_int_ventarifas
end type
type dw_proceso from datawindow within w_int_ventarifas
end type
type cb_continuar from u_boton within w_int_ventarifas
end type
type dw_proceso2 from datawindow within w_int_ventarifas
end type
type cb_calidades from u_boton within w_int_ventarifas
end type
type uo_tipo from u_marca_lista within w_int_ventarifas
end type
type dw_proceso_cal from datawindow within w_int_ventarifas
end type
type gb_2 from groupbox within w_int_ventarifas
end type
type sle_moneda from statictext within w_int_ventarifas
end type
type st_3 from statictext within w_int_ventarifas
end type
type em_cambio from u_em_campo within w_int_ventarifas
end type
type ddlb_pantilla from dropdownlistbox within w_int_ventarifas
end type
type st_4 from statictext within w_int_ventarifas
end type
type dw_proceso3 from datawindow within w_int_ventarifas
end type
type dw_detalle from u_datawindow_consultas within w_int_ventarifas
end type
type cb_salir from u_boton within w_int_ventarifas
end type
type cb_procesar from u_boton within w_int_ventarifas
end type
type gb_1 from groupbox within w_int_ventarifas
end type
type st_1 from statictext within w_int_ventarifas
end type
end forward
...
global w_int_ventarifas w_int_ventarifas
...
public function boolean f_grabar (string cod_familia, string cod_subfamilia, string cod_formato, string cod_calidad, decimal precio)
end prototypes
...
public function boolean f_grabar (string cod_familia, string cod_subfamilia, string cod_formato, string cod_calidad, decimal precio);String   tar_empresa,tar_tarifa,tar_formato,tar_familia,&
         tar_articulo,tar_calidad,tar_montajeartcal,&
         tar_modelo,tar_activo,tar_listar,tar_usuario,&
         tar_descripcion,tar_excepcion,tar_tipo_linea,tar_subfamilia
...
var_fecha = dw_fechas.GetItemDatetime(dw_fechas.GetRow(),"fecha")
  INSERT INTO ventarfamfor  
         ( empresa,tarifa,fecha,familia,formato,calidad,precio,subfamilia )  
  VALUES ( :codigo_empresa,:em_tarifa.text,:var_fecha,:cod_familia,   
           :cod_formato,:cod_calidad,:precio,:cod_subfamilia )  ;

registros = dw_proceso2.Retrieve(codigo_empresa,cod_familia,cod_subfamilia,cod_formato,DateTime(Today()))
If registros <>0 Then
  ...
  INSERT INTO venlintarifas  
         ( empresa,tarifa,fecha,articulo,calidad,precio,falta,   
           ...
           usuario,descripcion,excepcion,tipo_linea,sub_familia )  
  VALUES ( :tar_empresa,:tar_tarifa,:tar_fecha,:tar_articulo,   
           ...
           :tar_usuario,:tar_descripcion,:tar_excepcion,:tar_tipo_linea,:tar_subfamilia);
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_int_ventarifas
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_int_ventarifas
end type

type st_empresa from w_int_con_empresa`st_empresa within w_int_ventarifas
integer x = 27
integer y = 8
integer width = 2427
end type
...
integer height = 112
boolean bringtotop = true
string dataobject = "dw_proceso_calformatos"
boolean livescroll = true
end type
...
IF dw_fechas.GetRow() = 0 Then
   MessageBox("Marcar un fecha","No se ha seleccionado un fecha de tarifa",Exclamation!, OK!,1)
  dw_fechas.Setfocus()
  return
END IF
...
IF registros2 = 0 Then  return
 ...
 IF NOT f_bloquear(tabla,seleccion,titulo) THEN
  ...
  bloqueado="S"
  String   var_formato,var_calidad,var_familia,var_subfamilia
  ...
  FOR x2 = 1 To registros2
     ...
     var_subfamilia  =     dw_proceso3.GetItemString(x2,"subfamilia")
  ...
  FOR x1 = 1 To registros
     ...
     dw_detalle.SetItem(contando,"sub_familia",f_nombre_subfamilia(codigo_empresa,var_subfamilia))
     ...
     dw_detalle.SetItem(contando,"cod_sub_familia",var_subfamilia)
...
integer height = 112
boolean bringtotop = true
string dataobject = "dw_proceso_articulos_familia_formato"
boolean livescroll = true
end type
...
integer taborder = 30
boolean bringtotop = true
string dataobject = "dw_familias_formatos_articulos"
boolean livescroll = true
end type
...
integer taborder = 20
boolean bringtotop = true
string dataobject = "dw_int_ventarifas"
borderstyle borderstyle = styleraised!
end type
...
event clicked;Integer registros,opcion,x1
...
String var_familia,var_formato,var_calidad,var_subfamilia
Decimal var_precio
For x1 = 1 To registros
      var_formato   =  dw_detalle.GetItemString(x1,"cod_formato") 
      var_familia   =  dw_detalle.GetItemString(x1,"cod_familia") 
      var_subfamilia   =  dw_detalle.GetItemString(x1,"cod_sub_familia") 
      ...
      If var_precio <> 0 Then
         f_grabar(var_familia,var_subfamilia,var_formato,var_calidad,var_precio)
      End If
