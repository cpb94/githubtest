$PBExportHeader$wtc_mant_modelo_archivo.srw
forward
global type wtc_mant_modelo_archivo from wt_ventana_padre_grid
end type
type dw_imagen from u_dw_imagen within wtc_mant_modelo_archivo
end type
type st_guardar from statictext within wtc_mant_modelo_archivo
end type
type pb_descarga from picturebutton within wtc_mant_modelo_archivo
end type
type st_info from statictext within wtc_mant_modelo_archivo
end type
type p_seleccion from picture within wtc_mant_modelo_archivo
end type
type pb_cargar_archivo from picturebutton within wtc_mant_modelo_archivo
end type
type pb_1 from picturebutton within wtc_mant_modelo_archivo
end type
end forward
...
integer width = 5810
integer height = 2700
string title = "Exposición - Mantenimiento de Archivos del Modelo"
dw_imagen dw_imagen
...
global wtc_mant_modelo_archivo wtc_mant_modelo_archivo

type variables
string modelo, tamanyo_visualizacion
end variables

on wtc_mant_modelo_archivo.create
int iCurrent
call super::create
...
on wtc_mant_modelo_archivo.destroy
call super::destroy
...
//else
	...
	IF lstr_param.i_nargumentos >= 1 THEN
		if not isnull(lstr_param.s_argumentos[1]) and lstr_param.s_argumentos[1] <> "" then
			...
			modelo = lstr_param.s_argumentos[2]
			st_codigo.text = modelo
			dw_1.retrieve (codigo_empresa, padre_codigo, modelo) 
		end if
	END IF
	...
	dw_1.title = 'Modelo ' + modelo
	
	tamanyo_visualizacion = ""
	SELECT tamanyo 
	INTO :tamanyo_visualizacion 
	FROM archivo_resolucion 
	WHERE empresa = :codigo_empresa AND codigo = '2' USING SQLCA;
//end if
end event
...
type st_codigo from wt_ventana_padre_grid`st_codigo within wtc_mant_modelo_archivo
...
type pb_duplicar1 from wt_ventana_padre_grid`pb_duplicar1 within wtc_mant_modelo_archivo
boolean visible = false
integer x = 4649
integer y = 88
end type

type pb_borrar1 from wt_ventana_padre_grid`pb_borrar1 within wtc_mant_modelo_archivo
integer x = 187
integer y = 2380
end type
...
Long fila_actual
String ruta, ruta_archivos_exposicion, coleccion, modelo_borrar, codigo
...
modelo_borrar = dw_1.object.modelo[fila_actual]
codigo = dw_1.object.codigo[fila_actual]

parametros.titulo = "Atención Borrar Archivo"
parametros.subtitulo = "Borrar Archivo Modelo"
...
if rtn = 1 then
	...
	if FileDelete(ruta) then
		DELETE FROM modelo_archivo_expo 
		WHERE empresa = :codigo_empresa AND coleccion = :coleccion AND modelo = :modelo_borrar AND codigo = :codigo 
		USING trans_ts;
...
if rtn = 1 then
	...
	registro.tabla = "modelo_archivo_expo"
	registro.clave = codigo_empresa + "#" + coleccion + "#" + modelo_borrar + "#" + codigo
	registro.valor = ruta
...
if dw_1.retrieve (codigo_empresa, padre_codigo, modelo) > 0 then
	dw_1.EVENT rowfocuschanged(dw_1.getrow())
end if
end event

type pb_anyadir1 from wt_ventana_padre_grid`pb_anyadir1 within wtc_mant_modelo_archivo
integer x = 37
integer y = 2380
integer weight = 700
end type
...
ELSE
	...
	rtn = GetFileOpenName ("Añadir Imagen al modelo "+modelo, pathname, filename, "jpg", "Imagen JPG,*.jpg", ruta_mostrada, 2)
...
if rtn = 1 then
	...
	param_colisiones.tabla = "modelo_archivo_expo"
	param_colisiones.campo = "codigo"
	param_colisiones.filtro = "modelo_archivo_expo.coleccion = '"+padre_codigo+"' AND modelo_archivo_expo.modelo = '"+modelo+"'"
	...
	nombre = padre_codigo+"-"+modelo+"-"+max_codigo+extension
	...
	if rtn = 1 then
		...
		WHERE empresa = :codigo_empresa AND coleccion = :padre_codigo AND modelo = :modelo 
		USING trans_ts;
		...
		INSERT INTO modelo_archivo_expo (empresa, coleccion, modelo, codigo, ruta, fichero, activo, fecha_modificacion, produccion, tamanyo) 
		VALUES (:codigo_empresa, :padre_codigo, :modelo, :max_codigo, :ruta_carpeta_bd, :nombre, 0, :fecha_modificacion, :produccion, :tamanyo) 
		USING trans_ts;
		...
		IF trans_ts.SQLCode <> 0 THEN         
			...
			registro.trans = trans_ts
			registro.tabla = "modelo_archivo_expo"
			registro.clave = codigo_empresa + "#" + padre_codigo + "#" + modelo + "#" + max_codigo
	...
	dw_1.settransobject(sqlca)	
	filas = dw_1.retrieve (codigo_empresa, padre_codigo, modelo) 
	if filas > 0 then
		dw_1.setrow(filas)
	end if
...
type pb_imprimir from wt_ventana_padre_grid`pb_imprimir within wtc_mant_modelo_archivo
boolean visible = false
integer x = 4283
integer y = 88
end type

type p_logo from wt_ventana_padre_grid`p_logo within wtc_mant_modelo_archivo
integer x = 5275
integer y = 28
end type

type dw_1 from wt_ventana_padre_grid`dw_1 within wtc_mant_modelo_archivo
integer y = 276
integer width = 1042
integer height = 2248
boolean titlebar = true
string title = "Modelos"
string dataobject = "dwtc_expo_archivos_modelo"
...
event dw_1::itemchanged;call super::itemchanged;Long i, cambio
if row > 0 then
	if dwo.name = "produccion" then
		...
		Next
		//dwo.Primary[row] = Long(data)
		return 2
	end if	
end if
end event

type pb_salir from wt_ventana_padre_grid`pb_salir within wtc_mant_modelo_archivo
integer x = 5637
integer y = 136
end type

type pb_grabar from wt_ventana_padre_grid`pb_grabar within wtc_mant_modelo_archivo
end type

type st_titulo from wt_ventana_padre_grid`st_titulo within wtc_mant_modelo_archivo
end type

type pb_cancelar from wt_ventana_padre_grid`pb_cancelar within wtc_mant_modelo_archivo
boolean visible = false
integer x = 4466
integer y = 84
end type
...
str_registro_sistema registro
Datetime fecha_modificacion
String max_codigo, nombre, extension, extension_antigua, ruta_modelo, ruta_temporal, ruta_antigua_exposicion, ruta_archivos_exposicion, ruta_fichero, ruta_carpeta, ruta_carpeta_bd
...
if rtn = 1 then
	...
	if rtn = 1 then
		...
		if rtn = 1 then
			...
			WHERE empresa = :codigo_empresa AND coleccion = :padre_codigo AND modelo = :modelo AND codigo = :codigo USING trans_ts;
			...
			IF trans_ts.SQLCode <> 0 THEN         
				...
				registro.usuario_windows = usuario_windows
				registro.trans = trans_ts
				registro.tabla = "modelo_archivo_disenyo"
				registro.clave = codigo_empresa + "#" + padre_codigo + "#" + modelo + "#" + codigo
