$PBExportHeader$wi_con_ordenes_laboratorio.srw
forward
global type wi_con_ordenes_laboratorio from w_int_con_empresa
end type
type pb_1 from upb_salir within wi_con_ordenes_laboratorio
end type
type dw_1 from datawindow within wi_con_ordenes_laboratorio
end type
type dw_listado from datawindow within wi_con_ordenes_laboratorio
end type
type pb_imprimir from upb_imprimir within wi_con_ordenes_laboratorio
end type
type cb_1 from commandbutton within wi_con_ordenes_laboratorio
end type
type gb_2 from groupbox within wi_con_ordenes_laboratorio
end type
type gb_1 from groupbox within wi_con_ordenes_laboratorio
end type
type em_fechadesde from u_em_campo within wi_con_ordenes_laboratorio
end type
type em_fechahasta from u_em_campo within wi_con_ordenes_laboratorio
end type
type cb_detalle from commandbutton within wi_con_ordenes_laboratorio
end type
type rb_1 from radiobutton within wi_con_ordenes_laboratorio
end type
type rb_todas from radiobutton within wi_con_ordenes_laboratorio
end type
end forward
...
global wi_con_ordenes_laboratorio wi_con_ordenes_laboratorio
...
real cantidad,cant_formula
datetime fecha,fmin,var_fecha_fin_prod	
long donde
...
//¿Como coño compruebo la continuidad de la producción de los articulos?
string ultimo_articulo_del_dia
long   ultima_orden_del_dia,dias_despues,coincide
dec    continuidad_produccion,cantidad_dia
date   fecha_hasta,nuevo_dia,ultimo_dia_produccion
fecha_hasta = Date(em_fechahasta.text)
select max(orden)
into   :ultima_orden_del_dia
from   proddiaria
where  empresa = :codigo_empresa
and    fecha   = :fecha_hasta;

select articulo
into   :ultimo_articulo_del_dia
from   proddiaria
where  empresa = :codigo_empresa
and    fecha   = :fecha_hasta
and    orden   = :ultima_orden_del_dia;
setnull(ultimo_dia_produccion)
continuidad_produccion = 0
dias_despues = 0
do
	dias_despues ++
	nuevo_dia = relativedate(fecha_hasta,dias_despues)
	select count(*)
	into   :coincide
	from   proddiaria
	where  empresa  = :codigo_empresa
	and    fecha    = :nuevo_dia
	and    articulo = :ultimo_articulo_del_dia
	and    orden    = 1;
	if isnull(coincide) then coincide = 0 
	if coincide > 0 then
		select sum(cantidad)
		into   :cantidad_dia
		from   proddiaria
		...
		if isnull(cantidad_dia) then cantidad_dia = 0
		continuidad_produccion = continuidad_produccion + cantidad_dia
		...
		if isnull(coincide) then coincide = 0 
		if coincide = 0 then
			ultimo_dia_produccion = relativedate(Date(em_fechahasta.text),dias_despues)
			cantidad_dia = 0
		end if
	else
		cantidad_dia = 0
	end if	
loop while cantidad_dia <> 0
if isnull(ultimo_dia_produccion) then
	ultimo_dia_produccion = relativedate(Date(em_fechahasta.text),dias_despues -1)
end if
//
seleccion1 ="select articulo,familia,formato,sum(cantidad) cantidad,"+&
				"min(fecha) fecha,max(fecha) max_fecha "+&
				"from proddiaria "+&
				"where empresa = '"+codigo_empresa+"' and "+&
				"fecha between '"+STring(Date(em_fechadesde.text),"mm/dd/yy")+"' and '"+STring(Date(em_fechahasta.text),"mm/dd/yy")+"'"
...
if total1 > 0 then
	...
	for indice1 = 1 to total1
		...
		cantidad = articulos.getitemdecimal(indice1,"cantidad")
		fecha       = articulos.getitemdatetime(indice1,"fecha")
		if articulo = ultimo_articulo_del_dia then
			if continuidad_produccion > 0 then
				cantidad = cantidad + continuidad_produccion
				var_fecha_fin_prod = datetime(ultimo_dia_produccion)
			else
				var_fecha_fin_prod = articulos.getitemdatetime(indice1,"max_fecha")
			end if
		else
			var_fecha_fin_prod = articulos.getitemdatetime(indice1,"max_fecha")
		end if
...
//				data.setitem(donde,"cant_dia",(cant_formula * cantidad)/1000)
				...
				data.Object.fecha_fin_prod[donde] = var_fecha_fin_prod
...
on wi_con_ordenes_laboratorio.create
int iCurrent
call super::create
...
on wi_con_ordenes_laboratorio.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within wi_con_ordenes_laboratorio
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within wi_con_ordenes_laboratorio
integer x = 1586
integer y = 24
integer width = 352
integer height = 312
...
type st_empresa from w_int_con_empresa`st_empresa within wi_con_ordenes_laboratorio
integer width = 837
end type
