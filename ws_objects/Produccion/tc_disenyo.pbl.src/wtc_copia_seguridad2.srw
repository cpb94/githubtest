$PBExportHeader$wtc_copia_seguridad2.srw
forward
global type wtc_copia_seguridad2 from window
end type
type em_fecha from editmask within wtc_copia_seguridad2
end type
type cb_2 from commandbutton within wtc_copia_seguridad2
end type
type sle_ruta from singlelineedit within wtc_copia_seguridad2
end type
type st_2 from statictext within wtc_copia_seguridad2
end type
type st_1 from statictext within wtc_copia_seguridad2
end type
type st_info from statictext within wtc_copia_seguridad2
end type
type pb_1 from picturebutton within wtc_copia_seguridad2
end type
type barra_1 from hprogressbar within wtc_copia_seguridad2
end type
type cb_1 from commandbutton within wtc_copia_seguridad2
end type
type p_logo from picture within wtc_copia_seguridad2
end type
type st_titulo from statictext within wtc_copia_seguridad2
end type
end forward
...
global wtc_copia_seguridad2 wtc_copia_seguridad2
...
on wtc_copia_seguridad2.create
...
on wtc_copia_seguridad2.destroy
...
String sel, fecha_inicio, ruta_papelera_disenyo, ruta_archivos_disenyo, ruta_archivos_exposicion, ruta, fichero, ruta_archivo, ruta_copia
Int i, rtn
Long numero
Datastore archivos
Boolean actualizar
...
fecha_inicio = em_fecha.text //yyyy-mm-dd?
if fecha_inicio = "00-00-0000" or isnull(fecha_inicio) then
	MessageBox("Error", "Debe indicar una fecha")
	return
end if

ruta_copia = sle_ruta.text
if ruta_copia = "" or isnull(ruta_copia) then
	MessageBox("Error", "Debe indicar una ruta de copia")
	return
end if

SELECT ruta_papelera_disenyo, ruta_archivos_disenyo, ruta_archivos_exposicion INTO :ruta_papelera_disenyo, :ruta_archivos_disenyo, :ruta_archivos_exposicion FROM empresa WHERE empresa = :codigo_empresa;

st_info.text = "Iniciando copia de seguridad de Archivos de Máquina"

sel = "SELECT TOP 1 ruta, fichero FROM archivo_disenyo_sistema_archivos WHERE fecha_modificacion > '"+fecha_inicio+"' "
