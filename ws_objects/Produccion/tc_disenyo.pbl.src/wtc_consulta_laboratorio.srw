$PBExportHeader$wtc_consulta_laboratorio.srw
forward
global type wtc_consulta_laboratorio from wt_consultas_padre
end type
type dw_imagen_grande from u_dw_imagen within wtc_consulta_laboratorio
end type
type dw_datos2 from datawindow within wtc_consulta_laboratorio
end type
type dw_historico from u_dw within wtc_consulta_laboratorio
end type
type dw_imagen from u_dw_imagen within wtc_consulta_laboratorio
end type
end forward
...
global wtc_consulta_laboratorio wtc_consulta_laboratorio
...
for j = 1 to dw_montaje_filtro.rowcount()
	if pos(dw_montaje_filtro.object.linea[j], "Fecha Rechazado") = 0 and pos(dw_montaje_filtro.object.linea[j], "Estado") = 0 then
		if montaje = '' then
...
on wtc_consulta_laboratorio.create
int iCurrent
call super::create
...
this.Control[iCurrent+3]=this.dw_historico
this.Control[iCurrent+4]=this.dw_imagen
end on

on wtc_consulta_laboratorio.destroy
call super::destroy
...
sql_inicial = "SELECT modelo_pieza.empresa,   "+&
		...
		"deshistorico.f_resp "+&		
		 ...
		 "( modelo_trabajo.codigo = (SELECT TOP 1 tr.codigo FROM modelo_trabajo tr WHERE tr.empresa = modelo_trabajo.empresa AND tr.modelo = modelo_trabajo.modelo AND tr.cliente = modelo_trabajo.cliente AND tr.propuesta = modelo_trabajo.propuesta AND tr.modelo_version IS NOT NULL ORDER BY tr.codigo DESC)) and "+&
		 ...
		 "( modelo_pieza.empresa *= deshistorico.empresa ) and  "+&
         "( modelo_pieza.cliente *= deshistorico.cliente ) and  "+&
         "( modelo.coleccion *= deshistorico.coleccion )  and  "+&
...
sql_auxiliar = "SELECT modelo_pieza.empresa,   "+&
         ...
         "(REPLACE((SELECT ruta_archivos_exposicion FROM empresas WHERE empresa = '"+codigo_empresa+"') + modelo_pieza_archivo.ruta + (SELECT tamanyo FROM archivo_resolucion WHERE codigo = '5') + '\' + modelo_pieza_archivo.fichero, ' ','')) AS ruta_fichero,  "+&
		"deshistorico.estado "+&
         "FROM modelo_pieza, "+&
         "modelo_pieza_archivo,   "+&   
		"modelo,   "+&
         "modelo_cliente,  "+& 
         "modelo_propuesta,   "+&
         "modelo_trabajo,   "+&
         ...
         "art_tipomoldura,  "+&
		"deshistorico "+&
         "WHERE (modelo_pieza.empresa *= modelo_pieza_archivo.empresa) and "+&
		 "( modelo_pieza.codigo_lab *= modelo_pieza_archivo.pieza ) and  "+&
		 "( modelo_pieza.empresa = modelo.empresa ) and  "+&
         "( modelo_pieza.modelo = modelo.modelo ) and  "+&
         "( modelo_pieza.empresa = modelo_cliente.empresa ) and  "+&
         "( modelo_pieza.modelo = modelo_cliente.modelo ) and  "+&
         "( modelo_pieza.cliente = modelo_cliente.cliente ) and  "+&
...
type st_registros_padre from wt_consultas_padre`st_registros_padre within wtc_consulta_laboratorio
end type

type st_total_padre from wt_consultas_padre`st_total_padre within wtc_consulta_laboratorio
end type

type pb_imprimir from wt_consultas_padre`pb_imprimir within wtc_consulta_laboratorio
integer x = 4229
integer y = 156
integer taborder = 0
end type
...
type pb_borrar from wt_consultas_padre`pb_borrar within wtc_consulta_laboratorio
integer x = 4434
integer y = 152
integer taborder = 0
end type

type dw_montaje_filtro from wt_consultas_padre`dw_montaje_filtro within wtc_consulta_laboratorio
integer width = 3867
integer height = 224
end type

type pb_1 from wt_consultas_padre`pb_1 within wtc_consulta_laboratorio
integer x = 3177
integer y = 124
integer taborder = 0
end type
...
filtro_dw = "1 = 1"
For j = 1 To dw_montaje_filtro.rowcount()
	if pos(dw_montaje_filtro.object.linea[j] , "Fecha Rechazado") <> 0 or pos(dw_montaje_filtro.object.linea[j] , "Estado") <> 0 then
...
type pb_reset from wt_consultas_padre`pb_reset within wtc_consulta_laboratorio
integer x = 4590
integer y = 152
integer taborder = 0
end type

type pb_anyadir1 from wt_consultas_padre`pb_anyadir1 within wtc_consulta_laboratorio
integer x = 3026
integer y = 124
integer taborder = 0
end type
...
if ddlb_campo.text <> "" or dw_montaje_filtro.rowcount() = 0 then
	...
	choose case ddlb_campo.text
		...
		case "Fecha Rechazado"
			if cadena_or = '' then
				cadena_or = " estado = 'Rechazado' and fecha_rechazado "+ddlb_operador.text+" Datetime('"+f_reemplazar(em_fecha1.text, "-", "/")+" 00:00:00') "
		...
		case "Estado"
			for k = 1 to lb_lista.totalitems()
				if lb_lista.state(k) = 1 then
					if cadena_or = '' then
						cadena_or = " estado = '"+lb_lista.Text (k)+"' "
						cadena_or_visible = " Estado = "+lb_lista.Text (k)
					else
						cadena_or = cadena_or+" OR estado = '"+lb_lista.Text (k)+"' "
		...
		case "Base (Descripción)"
			for i = 1 to dw_filtro.rowcount()
				if dw_filtro.IsSelected (i)  then
					if cadena_or = '' then
						cadena_or = " EXISTS (SELECT * FROM modelo_pieza_version WHERE modelo_pieza_version.empresa = modelo_pieza.empresa AND modelo_pieza_version.modelo = modelo_pieza.modelo AND modelo_pieza_version.cliente = modelo_pieza.cliente AND modelo_pieza_version.propuesta = modelo_pieza.propuesta AND modelo_pieza_version.pieza = modelo_pieza.codigo AND modelo_pieza_version .base= '"+dw_filtro.object.#1[i]+"' ) "
						cadena_or_visible = " Base = "+dw_filtro.object.#2[i]
					else
						cadena_or = cadena_or+" OR EXISTS (SELECT * FROM modelo_pieza_version WHERE modelo_pieza_version.empresa = modelo_pieza.empresa AND modelo_pieza_version.modelo = modelo_pieza.modelo AND modelo_pieza_version.cliente = modelo_pieza.cliente AND modelo_pieza_version.propuesta = modelo_pieza.propuesta AND modelo_pieza_version.pieza = modelo_pieza.codigo AND modelo_pieza_version .base= '"+dw_filtro.object.#1[i]+"' ) "
						cadena_or_visible = cadena_or_visible+", "+dw_filtro.object.#2[i]
					end if
				end if
			next 
		case "Base (Código)"
			if cadena_or = '' then
				cadena_or = " EXISTS (SELECT * FROM modelo_pieza_version WHERE modelo_pieza_version.empresa = modelo_pieza.empresa AND modelo_pieza_version.modelo = modelo_pieza.modelo AND modelo_pieza_version.cliente = modelo_pieza.cliente AND modelo_pieza_version.propuesta = modelo_pieza.propuesta AND modelo_pieza_version.pieza = modelo_pieza.codigo AND modelo_pieza_version .base= '"+sle_valor1.text+"' ) "
				cadena_or_visible = " Base = "+sle_valor1.text
			end if
	end choose
...
type ddlb_operador from wt_consultas_padre`ddlb_operador within wtc_consulta_laboratorio
integer x = 951
integer taborder = 20
integer weight = 700
...
type ddlb_campo from wt_consultas_padre`ddlb_campo within wtc_consulta_laboratorio
integer width = 878
integer height = 1308
integer taborder = 10
integer weight = 700
long textcolor = 24076881
string item[] = {"Diseñador Trabajo","Comercial Trabajo","Técnico Colección","Fecha Colección","Colección","Cliente","Estado","Fecha Solicitud Trabajo","Fecha Laboratorio","Fecha Rechazado","Activa","Ancho Pieza","Largo Pieza","Base (Código)","Base (Descripción)"}
end type
...
CHOOSE CASE ddlb_campo.text
	...
	CASE "Base (Descripción)"
		...
		ddlb_operador.AddItem ("=")
		ddlb_operador.text = "="
		consulta = "SELECT articulos.codigo, CONVERT(char(60),articulos.descripcion), formatos.abreviado, almusos.descripcion, genter.razon "+&
							"FROM articulos "+&
...
type dw_datos from wt_consultas_padre`dw_datos within wtc_consulta_laboratorio
integer y = 512
integer width = 3863
integer height = 2192
integer taborder = 60
...
type pb_salir from wt_consultas_padre`pb_salir within wtc_consulta_laboratorio
integer x = 4745
integer y = 152
integer taborder = 0
end type

type st_titulo from wt_consultas_padre`st_titulo within wtc_consulta_laboratorio
end type

type p_logo from wt_consultas_padre`p_logo within wtc_consulta_laboratorio
integer x = 4389
integer y = 36
end type

type dw_filtro from wt_consultas_padre`dw_filtro within wtc_consulta_laboratorio
integer x = 1317
integer y = 240
integer width = 1673
integer height = 260
...
type pb_buscar from wt_consultas_padre`pb_buscar within wtc_consulta_laboratorio
integer x = 2871
integer y = 132
integer taborder = 90
end type

type sle_valor1 from wt_consultas_padre`sle_valor1 within wtc_consulta_laboratorio
integer x = 1312
integer width = 398
integer height = 96
integer taborder = 30
...
type em_fecha1 from wt_consultas_padre`em_fecha1 within wtc_consulta_laboratorio
integer x = 1307
integer height = 96
...
type sle_texto from wt_consultas_padre`sle_texto within wtc_consulta_laboratorio
integer x = 1312
integer y = 132
integer height = 96
integer taborder = 40
...
type lb_lista from wt_consultas_padre`lb_lista within wtc_consulta_laboratorio
integer x = 1312
integer y = 132
integer width = 462
integer height = 356
