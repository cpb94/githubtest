$PBExportHeader$wtc_consumo_disenyo_colecciones.srw
forward
global type wtc_consumo_disenyo_colecciones from wt_consultas_padre
end type
type dw_grafico1 from datawindow within wtc_consumo_disenyo_colecciones
end type
end forward
...
integer width = 8731
integer height = 3480
string title = "Consumo Disco - Archivos Colección"
dw_grafico1 dw_grafico1
end type
global wtc_consumo_disenyo_colecciones wtc_consumo_disenyo_colecciones
...
on wtc_consumo_disenyo_colecciones.create
int iCurrent
call super::create
...
on wtc_consumo_disenyo_colecciones.destroy
call super::destroy
...
"			coleccion_sistema_archivos.coleccion, "+&
"			coleccion_sistema_archivos.fichero, "+&
"			ISNULL(coleccion_sistema_archivos.tamanyo, 0) as tamanyo, "+&
"			coleccion_sistema_archivos.fecha_modificacion, "+&
"			REPLACE(coleccion_sistema_archivos.coleccion, ' ','') as codigo_unico, "+&
"			descoleccionsol.disenyador, "+&
"			tecnico_dis.descripcion "+&
"FROM coleccion_sistema_archivos "+&
"INNER JOIN descoleccionsol ON coleccion_sistema_archivos.empresa = descoleccionsol.empresa AND coleccion_sistema_archivos.coleccion = descoleccionsol.codigo "+&
"INNER JOIN tecnico_dis ON descoleccionsol.empresa = tecnico_dis.empresa AND descoleccionsol.disenyador = tecnico_dis.codigo "+&
"WHERE coleccion_sistema_archivos.empresa = '"+codigo_empresa+"'"

sql_graf_inicial = sql_inicial
 
end event

type pb_imprimir from wt_consultas_padre`pb_imprimir within wtc_consumo_disenyo_colecciones
integer x = 8055
integer y = 128
end type

type pb_borrar from wt_consultas_padre`pb_borrar within wtc_consumo_disenyo_colecciones
integer x = 8215
integer y = 124
end type

type dw_montaje_filtro from wt_consultas_padre`dw_montaje_filtro within wtc_consumo_disenyo_colecciones
integer width = 8622
integer height = 224
end type

type pb_1 from wt_consultas_padre`pb_1 within wtc_consumo_disenyo_colecciones
integer x = 3195
integer y = 120
integer taborder = 0
end type
...
type pb_reset from wt_consultas_padre`pb_reset within wtc_consumo_disenyo_colecciones
integer x = 8370
integer y = 124
integer taborder = 0
end type

type pb_anyadir1 from wt_consultas_padre`pb_anyadir1 within wtc_consumo_disenyo_colecciones
integer x = 3045
integer y = 120
integer taborder = 0
integer weight = 700
...
if ddlb_campo.text <> "" or dw_montaje_filtro.rowcount() = 0 then
	...
	choose case ddlb_campo.text
		...
		case "Fichero"
			if cadena_or = '' then
				cadena_or = " REPLACE(coleccion_sistema_archivos.coleccion,' ','') LIKE '%"+sle_valor1.text+"%' "
		...
		case "Número de Versiones"
			if cadena_or = '' then
				cadena_or = " EXISTS (SELECT * FROM coleccion_sistema_archivos M WHERE M.coleccion =  coleccion_sistema_archivos.coleccion GROUP BY M.coleccion HAVING  COUNT(M.codigo) "+ddlb_operador.text+" "+sle_valor1.text +")"
		...
		case "Colección en Producción"
			for k = 1 to lb_lista.totalitems()
				if lb_lista.state(k) = 1 then
					if cadena_or = '' then
						cadena_or = "coleccion_sistema_archivos.coleccion IN(SELECT modelo.coleccion FROM modelo WHERE modelo IN (SELECT articulos.modelo_origen FROM articulos WHERE articulos.empresa = modelo.empresa AND articulos.modelo_origen = modelo.modelo))"
...
type ddlb_operador from wt_consultas_padre`ddlb_operador within wtc_consumo_disenyo_colecciones
integer x = 951
integer taborder = 20
integer weight = 700
...
type ddlb_campo from wt_consultas_padre`ddlb_campo within wtc_consumo_disenyo_colecciones
integer width = 878
integer height = 904
integer taborder = 10
integer weight = 700
...
type dw_datos from wt_consultas_padre`dw_datos within wtc_consumo_disenyo_colecciones
integer y = 512
integer width = 3049
integer height = 2764
integer taborder = 60
string dataobject = "dwtc_consumo_disenyo_colecciones"
end type
...
type pb_salir from wt_consultas_padre`pb_salir within wtc_consumo_disenyo_colecciones
integer x = 8526
integer y = 124
integer taborder = 0
end type

type st_titulo from wt_consultas_padre`st_titulo within wtc_consumo_disenyo_colecciones
integer width = 2939
end type

type p_logo from wt_consultas_padre`p_logo within wtc_consumo_disenyo_colecciones
integer x = 8155
integer y = 8
end type

type dw_filtro from wt_consultas_padre`dw_filtro within wtc_consumo_disenyo_colecciones
integer x = 1298
integer y = 232
integer width = 1673
integer height = 260
...
type pb_buscar from wt_consultas_padre`pb_buscar within wtc_consumo_disenyo_colecciones
integer x = 2871
integer y = 132
integer taborder = 80
end type

type sle_valor1 from wt_consultas_padre`sle_valor1 within wtc_consumo_disenyo_colecciones
integer x = 1312
integer width = 398
integer height = 96
integer taborder = 30
...
type em_fecha1 from wt_consultas_padre`em_fecha1 within wtc_consumo_disenyo_colecciones
integer x = 1307
integer height = 96
integer taborder = 90
string text = ""
...
type sle_texto from wt_consultas_padre`sle_texto within wtc_consumo_disenyo_colecciones
integer x = 1312
integer y = 132
integer height = 96
integer taborder = 40
...
type lb_lista from wt_consultas_padre`lb_lista within wtc_consumo_disenyo_colecciones
integer x = 1312
integer y = 132
integer width = 1042
integer height = 496
...
string title = "none"
string dataobject = "graf_consumo_disenyo_colecciones"
