$PBExportHeader$wtc_mant_tipomaquina_disenyo.srw
forward
global type wtc_mant_tipomaquina_disenyo from wt_ventana_padre
end type
type dw_con_piezas_coleccion from u_dw_padre within wtc_mant_tipomaquina_disenyo
end type
type pb_anyade_trabajo from picturebutton within wtc_mant_tipomaquina_disenyo
end type
end forward

global type wtc_mant_tipomaquina_disenyo from wt_ventana_padre
integer height = 904
string title = "Mantenimiento de Tipos de Máquinas de Diseño"
dw_con_piezas_coleccion dw_con_piezas_coleccion
pb_anyade_trabajo pb_anyade_trabajo
end type
global wtc_mant_tipomaquina_disenyo wtc_mant_tipomaquina_disenyo
...
param_colisiones.tabla = "tipomaquina_disenyo"
...
if dw_1.rowcount() > 0 then
	...
	if IsNull(dw_1.object.gestion_color[dw_1.getrow()]) or Trim(String(dw_1.object.gestion_color[dw_1.getrow()])) = "" then
		  campo = "gestion_color"
		  motivo = "Campo Obligatorio: Gestión Color. "
		  falta_campo = True
	end if
	if falta_campo then
...
on wtc_mant_tipomaquina_disenyo.create
int iCurrent
call super::create
...
on wtc_mant_tipomaquina_disenyo.destroy
call super::destroy
...
event activate;call super::activate;wtc_mant_tipomaquina_disenyo = ventanas_activas[id_ventana_activa].ventana
end event

type pb_imprimir from wt_ventana_padre`pb_imprimir within wtc_mant_tipomaquina_disenyo
boolean visible = false
end type

type p_logo from wt_ventana_padre`p_logo within wtc_mant_tipomaquina_disenyo
end type

type dw_1 from wt_ventana_padre`dw_1 within wtc_mant_tipomaquina_disenyo
integer y = 276
integer height = 436
string dataobject = "dwtc_tipomaquina_disenyo"
boolean border = false
end type

type pb_borrar from wt_ventana_padre`pb_borrar within wtc_mant_tipomaquina_disenyo
end type

type pb_salir from wt_ventana_padre`pb_salir within wtc_mant_tipomaquina_disenyo
end type

type pb_grabar from wt_ventana_padre`pb_grabar within wtc_mant_tipomaquina_disenyo
end type

type st_titulo from wt_ventana_padre`st_titulo within wtc_mant_tipomaquina_disenyo
end type

type pb_nuevo from wt_ventana_padre`pb_nuevo within wtc_mant_tipomaquina_disenyo
end type
...
type pb_cancelar from wt_ventana_padre`pb_cancelar within wtc_mant_tipomaquina_disenyo
end type

type pb_buscar from wt_ventana_padre`pb_buscar within wtc_mant_tipomaquina_disenyo
end type
...
choose case campo
	case "codigo"
		...
		padre_busqueda.consulta =  " SELECT tipomaquina_disenyo.codigo, tipomaquina_disenyo.codigo, tipomaquina_disenyo.descripcion "+&	
									" FROM tipomaquina_disenyo "+&
									" WHERE tipomaquina_disenyo.empresa = '"+codigo_empresa+"' "+&
									" ORDER BY CONVERT(INTEGER, tipomaquina_disenyo.codigo) desc"
		padre_busqueda.titulos[1] = "Código"
		padre_busqueda.titulos[2] = "Código"
		padre_busqueda.titulos[3] = "Descripción"	

	case "descripcion"
		padre_busqueda.consulta =  " SELECT tipomaquina_disenyo.codigo, tipomaquina_disenyo.descripcion "+&	
									" FROM tipomaquina_disenyo "+&
									" WHERE tipomaquina_disenyo.empresa = '"+codigo_empresa+"' "+&
									" ORDER BY tipomaquina_disenyo.descripcion desc"
		padre_busqueda.titulos[1] = "Código"
