$PBExportHeader$wtc_mant_modelo_sistema_archivos.srw
forward
global type wtc_mant_modelo_sistema_archivos from wt_ventana_padre_grid
end type
type pb_descargar_archivo from picturebutton within wtc_mant_modelo_sistema_archivos
end type
type p_seleccion from picture within wtc_mant_modelo_sistema_archivos
end type
type pb_cargar_archivo from picturebutton within wtc_mant_modelo_sistema_archivos
end type
type st_guardar from statictext within wtc_mant_modelo_sistema_archivos
end type
type st_produccion from statictext within wtc_mant_modelo_sistema_archivos
end type
end forward
...
global wtc_mant_modelo_sistema_archivos wtc_mant_modelo_sistema_archivos
...
if dw_1.rowcount() > 0 then
	if IsNull(dw_1.object.modelo_sistema_archivos_cliente[dw_1.getrow()]) or Trim(String(dw_1.object.modelo_sistema_archivos_cliente[dw_1.getrow()])) = "" then
		  campo = "modelo_sistema_archivos_cliente"
...
param_colisiones.tabla = "modelo_sistema_archivos"
param_colisiones.campo = "codigo"
param_colisiones.filtro = "modelo_sistema_archivos.modelo = '"+padre_codigo+"'"
...
if rtn <> -1 then
	...
	For i = 1 To dw_1.deletedCount()
		...
		if registrar then
			...
			registro.clave = trim(dw_1.GetItemString(i,"empresa",Delete!,false)) + "#" + trim(dw_1.GetItemString(i,"modelo",Delete!,false)) + "#" + trim(dw_1.GetItemString(i,"codigo",Delete!,false))
	...
	Do while (i <= total_filas and rtn = 1)
		...
		if dw_1.object.ruta_local[i] = 1 then
			...
			nombre = padre_codigo+"-"+dw_1.object.codigo[i]+extension
		...
		if registrar then
			...
			registro.trans = trans_ts
			registro.tabla = "modelo_sistema_archivos"
			registro.clave = trim(dw_1.GetItemString(i,"empresa")) + "#" + trim(dw_1.GetItemString(i,"modelo")) + "#" + trim(dw_1.GetItemString(i,"codigo"))
...
on wtc_mant_modelo_sistema_archivos.create
int iCurrent
call super::create
...
on wtc_mant_modelo_sistema_archivos.destroy
call super::destroy
...
event open;call super::open;Long modelos_pr

dw_1.title = 'Modelo ' + padre_codigo
...
type st_codigo from wt_ventana_padre_grid`st_codigo within wtc_mant_modelo_sistema_archivos
...
type pb_duplicar1 from wt_ventana_padre_grid`pb_duplicar1 within wtc_mant_modelo_sistema_archivos
boolean visible = false
integer x = 4251
integer y = 28
end type

type pb_borrar1 from wt_ventana_padre_grid`pb_borrar1 within wtc_mant_modelo_sistema_archivos
integer x = 265
integer y = 1620
end type
...
type pb_anyadir1 from wt_ventana_padre_grid`pb_anyadir1 within wtc_mant_modelo_sistema_archivos
integer x = 110
integer y = 1620
integer weight = 700
end type
...
if res = 1 then
	dw_1.object.archivo_local[fila] = filename[1]
...
type pb_imprimir from wt_ventana_padre_grid`pb_imprimir within wtc_mant_modelo_sistema_archivos
boolean visible = false
integer x = 4023
integer y = 28
end type

type p_logo from wt_ventana_padre_grid`p_logo within wtc_mant_modelo_sistema_archivos
integer x = 5047
integer y = 20
end type

type dw_1 from wt_ventana_padre_grid`dw_1 within wtc_mant_modelo_sistema_archivos
integer x = 37
integer y = 268
integer width = 5536
integer height = 1512
...
string dataobject = "dwtc_modelo_sistema_archivos"
...
type pb_salir from wt_ventana_padre_grid`pb_salir within wtc_mant_modelo_sistema_archivos
integer x = 5413
integer y = 124
end type

type pb_grabar from wt_ventana_padre_grid`pb_grabar within wtc_mant_modelo_sistema_archivos
end type

type st_titulo from wt_ventana_padre_grid`st_titulo within wtc_mant_modelo_sistema_archivos
integer width = 2482
end type

type pb_cancelar from wt_ventana_padre_grid`pb_cancelar within wtc_mant_modelo_sistema_archivos
boolean visible = false
integer x = 4613
integer y = 24
end type
...
if fila > 0 then  
	...
	if not isnull(ruta) and ruta <> "" and local = 0 then
		...
		if res = 1 then
			pathname = f_reemplazar(pathname, filename[1], String(dw_1.object.fichero[fila]))
