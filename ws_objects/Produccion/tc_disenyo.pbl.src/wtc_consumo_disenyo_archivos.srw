$PBExportHeader$wtc_consumo_disenyo_archivos.srw
forward
global type wtc_consumo_disenyo_archivos from wt_consultas_padre
end type
type dw_grafico1 from datawindow within wtc_consumo_disenyo_archivos
end type
end forward
...
integer width = 8731
integer height = 3480
string title = "Consumo Disco - Archivos Máquina"
dw_grafico1 dw_grafico1
end type
global wtc_consumo_disenyo_archivos wtc_consumo_disenyo_archivos
...
on wtc_consumo_disenyo_archivos.create
int iCurrent
call super::create
...
on wtc_consumo_disenyo_archivos.destroy
call super::destroy
...
sql_inicial = "SELECT 	modelo_archivo.modelo, "+&
"		   	modelo_archivo.cliente, "+&
"			modelo_archivo.propuesta, "+&
"			modelo_archivo.pieza, "+&
"			modelo_archivo.modelo_version, "+&
"			archivo_disenyo_sistema_archivos.tipomaquina_disenyo, "+&
"			archivo_disenyo_sistema_archivos.archivo, "+&
"			archivo_disenyo_sistema_archivos.fichero, "+&
"			ISNULL(archivo_disenyo_sistema_archivos.tamanyo, 0) as tamanyo, "+&
"			archivo_disenyo_sistema_archivos.fecha_modificacion, "+&
"			REPLACE(archivo_disenyo_sistema_archivos.ruta,'\','') as codigo_unico, "+&
"			modelo.tecnico_dis, "+&
"			tecnico_dis.descripcion "+&
"FROM archivo_disenyo_sistema_archivos "+&
"INNER JOIN modelo_archivo ON archivo_disenyo_sistema_archivos.empresa = modelo_archivo.empresa AND archivo_disenyo_sistema_archivos.tipomaquina_disenyo = modelo_archivo.tipo_maquina_disenyo AND archivo_disenyo_sistema_archivos.archivo = modelo_archivo.archivo_disenyo "+&
"INNER JOIN modelo ON modelo_archivo.empresa = modelo.empresa AND modelo_archivo.modelo = modelo.modelo "+&
...
type pb_imprimir from wt_consultas_padre`pb_imprimir within wtc_consumo_disenyo_archivos
integer x = 8055
integer y = 128
end type

type pb_borrar from wt_consultas_padre`pb_borrar within wtc_consumo_disenyo_archivos
integer x = 8215
integer y = 124
end type

type dw_montaje_filtro from wt_consultas_padre`dw_montaje_filtro within wtc_consumo_disenyo_archivos
integer width = 8622
integer height = 224
end type

type pb_1 from wt_consultas_padre`pb_1 within wtc_consumo_disenyo_archivos
integer x = 3195
integer y = 120
integer taborder = 0
end type
...
type pb_reset from wt_consultas_padre`pb_reset within wtc_consumo_disenyo_archivos
integer x = 8370
integer y = 124
integer taborder = 0
end type

type pb_anyadir1 from wt_consultas_padre`pb_anyadir1 within wtc_consumo_disenyo_archivos
integer x = 3045
integer y = 120
integer taborder = 0
integer weight = 700
...
if ddlb_campo.text <> "" or dw_montaje_filtro.rowcount() = 0 then
	...
	choose case ddlb_campo.text
		...
		case "Fichero"
			if cadena_or = '' then
				cadena_or = " REPLACE(archivo_disenyo_sistema_archivos.ruta,'\ ','') LIKE '%"+sle_valor1.text+"%' "
		...
		case "Número de Versiones"
			if cadena_or = '' then
				cadena_or = " EXISTS (SELECT * FROM archivo_disenyo_sistema_archivos M WHERE M.tipomaquina_disenyo =  archivo_disenyo_sistema_archivos.tipomaquina_disenyo AND M.archivo = archivo_disenyo_sistema_archivos.archivo GROUP BY M.tipomaquina_disenyo, M.archivo HAVING  COUNT(M.codigo) "+ddlb_operador.text+" "+sle_valor1.text +")"
...
type ddlb_operador from wt_consultas_padre`ddlb_operador within wtc_consumo_disenyo_archivos
integer x = 951
integer taborder = 20
integer weight = 700
...
type ddlb_campo from wt_consultas_padre`ddlb_campo within wtc_consumo_disenyo_archivos
integer width = 878
integer height = 904
integer taborder = 10
integer weight = 700
...
type dw_datos from wt_consultas_padre`dw_datos within wtc_consumo_disenyo_archivos
integer y = 512
integer width = 3049
integer height = 2764
integer taborder = 60
string dataobject = "dwtc_consumo_disenyo_archivos"
end type
...
if fila > 0 then
	...
	lstr_param.s_argumentos[1] = dw_datos.GetItemString(fila,"modelo_archivo_modelo")
	lstr_param.i_nargumentos = 1
	lstr_param.nombre_ventana = "wtc_consumo_disenyo_archivos"
...
type pb_salir from wt_consultas_padre`pb_salir within wtc_consumo_disenyo_archivos
integer x = 8526
integer y = 124
integer taborder = 0
end type

type st_titulo from wt_consultas_padre`st_titulo within wtc_consumo_disenyo_archivos
integer width = 2939
end type

type p_logo from wt_consultas_padre`p_logo within wtc_consumo_disenyo_archivos
integer x = 8155
integer y = 8
end type

type dw_filtro from wt_consultas_padre`dw_filtro within wtc_consumo_disenyo_archivos
integer x = 1298
integer y = 232
integer width = 1673
integer height = 260
...
type pb_buscar from wt_consultas_padre`pb_buscar within wtc_consumo_disenyo_archivos
integer x = 2871
integer y = 132
integer taborder = 80
end type

type sle_valor1 from wt_consultas_padre`sle_valor1 within wtc_consumo_disenyo_archivos
integer x = 1312
integer width = 398
integer height = 96
integer taborder = 30
...
type em_fecha1 from wt_consultas_padre`em_fecha1 within wtc_consumo_disenyo_archivos
integer x = 1307
integer height = 96
integer taborder = 90
string text = ""
...
type sle_texto from wt_consultas_padre`sle_texto within wtc_consumo_disenyo_archivos
integer x = 1312
integer y = 132
integer height = 96
integer taborder = 40
...
type lb_lista from wt_consultas_padre`lb_lista within wtc_consumo_disenyo_archivos
integer x = 1312
integer y = 132
integer width = 1042
integer height = 496
...
string title = "none"
string dataobject = "graf_consumo_disenyo_archivos"
