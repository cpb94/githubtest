$PBExportHeader$wtc_consulta_pantallas2.srw
forward
global type wtc_consulta_pantallas2 from wt_consultas_padre
end type
type cbx_piezas from checkbox within wtc_consulta_pantallas2
end type
type cb_1 from commandbutton within wtc_consulta_pantallas2
end type
end forward
...
cbx_piezas cbx_piezas
cb_1 cb_1
end type
global wtc_consulta_pantallas2 wtc_consulta_pantallas2
...
on wtc_consulta_pantallas2.create
int iCurrent
call super::create
this.cbx_piezas=create cbx_piezas
this.cb_1=create cb_1
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.cbx_piezas
this.Control[iCurrent+2]=this.cb_1
end on

on wtc_consulta_pantallas2.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.cbx_piezas)
destroy(this.cb_1)
end on
...
"         REPLACE(modelo_pieza_archivo.ruta + (SELECT tamanyo FROM archivo_resolucion WHERE empresa = archivo_disenyo.empresa AND codigo = '1') + '\' + modelo_pieza_archivo.fichero, ' ', '' ) AS fichero,   "+&
"         REPLACE(modelo_archivo_expo.ruta + (SELECT tamanyo FROM archivo_resolucion WHERE empresa = archivo_disenyo.empresa AND codigo = '1') + '\' + modelo_archivo_expo.fichero, ' ', '' ) AS fichero_modelo  "+&
"    FROM archivo_disenyo LEFT OUTER JOIN almacen_laboratorio_tipo ON archivo_disenyo.empresa = almacen_laboratorio_tipo.empresa AND archivo_disenyo.tipo_pantallas = almacen_laboratorio_tipo.codigo LEFT OUTER JOIN modelo_archivo ON archivo_disenyo.empresa = modelo_archivo.empresa AND archivo_disenyo.tipomaquina_disenyo = modelo_archivo.tipo_maquina_disenyo AND archivo_disenyo.codigo = modelo_archivo.archivo_disenyo LEFT OUTER JOIN modelo_pieza ON modelo_archivo.empresa = modelo_pieza.empresa AND modelo_archivo.modelo = modelo_pieza.modelo AND modelo_archivo.cliente = modelo_pieza.cliente AND modelo_archivo.propuesta = modelo_pieza.propuesta AND modelo_archivo.pieza = modelo_pieza.codigo LEFT OUTER JOIN modelo_pieza_archivo ON modelo_pieza.empresa = modelo_pieza_archivo.empresa AND modelo_pieza.codigo_lab = modelo_pieza_archivo.pieza LEFT OUTER JOIN modelo_prueba ON modelo_pieza.empresa = modelo_prueba.empresa AND modelo_pieza.modelo = modelo_prueba.modelo AND modelo_pieza.cliente = modelo_prueba.cliente AND modelo_pieza.propuesta = modelo_prueba.propuesta AND modelo_pieza.codigo = modelo_prueba.pieza LEFT OUTER JOIN tecnico_lab ON modelo_prueba.empresa = tecnico_lab.empresa AND modelo_prueba.tecnico_lab = tecnico_lab.codigo LEFT OUTER JOIN modelo_archivo_expo ON modelo_archivo.empresa = modelo_archivo_expo.empresa AND modelo_archivo.modelo = modelo_archivo_expo.modelo  "+&
...
if cbx_piezas.checked = true then
	dw_datos.Modify("DataWindow.Detail.Height = 384")//dw_datos.Object.DataWindow.Header.#2.Height = 380
else
	dw_datos.Modify("DataWindow.Detail.Height = 0")// dw_datos.Object.DataWindow.Header.#2.Height = 0 
end if
end event
...
type st_registros_padre from wt_consultas_padre`st_registros_padre within wtc_consulta_pantallas2
end type

type st_total_padre from wt_consultas_padre`st_total_padre within wtc_consulta_pantallas2
end type

type pb_imprimir from wt_consultas_padre`pb_imprimir within wtc_consulta_pantallas2
integer x = 3374
integer y = 128
integer taborder = 0
end type

type pb_borrar from wt_consultas_padre`pb_borrar within wtc_consulta_pantallas2
integer x = 3579
integer y = 124
integer taborder = 0
end type

type dw_montaje_filtro from wt_consultas_padre`dw_montaje_filtro within wtc_consulta_pantallas2
integer width = 2926
integer height = 224
end type

type pb_1 from wt_consultas_padre`pb_1 within wtc_consulta_pantallas2
integer x = 3168
integer y = 124
integer taborder = 0
end type
...
type pb_reset from wt_consultas_padre`pb_reset within wtc_consulta_pantallas2
integer x = 3735
integer y = 124
integer taborder = 0
end type

type pb_anyadir1 from wt_consultas_padre`pb_anyadir1 within wtc_consulta_pantallas2
integer x = 3017
integer y = 124
integer taborder = 0
end type
...
type ddlb_operador from wt_consultas_padre`ddlb_operador within wtc_consulta_pantallas2
integer x = 951
integer taborder = 20
integer weight = 700
...
type ddlb_campo from wt_consultas_padre`ddlb_campo within wtc_consulta_pantallas2
integer width = 878
integer height = 1308
integer taborder = 10
integer weight = 700
long textcolor = 24076881
string item[] = {"Revisado","Ubicación","Tipo Ubicación","Juego Pantallas","Número Pantallas","Técnico Laboratorio"}
end type
...
type dw_datos from wt_consultas_padre`dw_datos within wtc_consulta_pantallas2
integer y = 512
integer width = 3986
integer height = 2744
integer taborder = 60
string dataobject = "dwtc_consulta_pantallas2"
end type
...
elseif dwo.name = "nombre" then
	...
	lstr_param.nombre_ventana = "wtc_consulta_pantallas2"
...
type pb_salir from wt_consultas_padre`pb_salir within wtc_consulta_pantallas2
integer x = 3890
integer y = 124
integer taborder = 0
end type

type st_titulo from wt_consultas_padre`st_titulo within wtc_consulta_pantallas2
end type

type p_logo from wt_consultas_padre`p_logo within wtc_consulta_pantallas2
integer x = 3515
integer y = 20
end type

type dw_filtro from wt_consultas_padre`dw_filtro within wtc_consulta_pantallas2
integer x = 1298
integer y = 232
integer width = 1673
integer height = 260
...
type pb_buscar from wt_consultas_padre`pb_buscar within wtc_consulta_pantallas2
integer x = 2871
integer y = 132
integer taborder = 90
end type

type sle_valor1 from wt_consultas_padre`sle_valor1 within wtc_consulta_pantallas2
integer x = 1312
integer width = 398
integer height = 96
integer taborder = 30
...
type em_fecha1 from wt_consultas_padre`em_fecha1 within wtc_consulta_pantallas2
integer x = 1307
integer height = 96
...
type sle_texto from wt_consultas_padre`sle_texto within wtc_consulta_pantallas2
integer x = 1312
integer y = 132
integer height = 96
integer taborder = 40
...
type lb_lista from wt_consultas_padre`lb_lista within wtc_consulta_pantallas2
integer x = 1312
integer y = 132
integer width = 462
integer height = 252
...
string text = "Mostrar Piezas"
end type

event clicked;if cbx_piezas.checked = true then
	dw_datos.Modify("DataWindow.Detail.Height = 384")//dw_datos.Object.DataWindow.Header.#2.Height = 380
else
	dw_datos.Modify("DataWindow.Detail.Height = 0")// dw_datos.Object.DataWindow.Header.#2.Height = 0 
end if
...
if MessageBox("Atención", "Va a desubicar todas las pantallas mostradas en el listado, que estén revisadas. Compruebe que haya filtrado por el técnico correcto, ya que la ubicación de las pantallas no se podrá recuperar. ¿Desea continuar?", Question!, YesNo!, 2) = 1 then
	For i = 1 To dw_datos.rowcount() 
		if dw_datos.object.revisado[i] = 'S' then
			...
			UPDATE archivo_disenyo SET ubicacion = NULL, tipo_pantallas = NULL, n_pantallas = NULL, revisado = 'N' WHERE empresa = :codigo_empresa AND tipomaquina_disenyo = :tipomaquina_disenyo AND codigo = :codigo USING SQLCA;
			if sqlca.sqlcode = 0 then
				commit using sqlca;
			else
				rollback using sqlca;
