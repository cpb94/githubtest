$PBExportHeader$wtc_mant_grupo_combi.srw
forward
global type wtc_mant_grupo_combi from wt_ventana_padre
end type
end forward

global type wtc_mant_grupo_combi from wt_ventana_padre
integer height = 816
string title = "Mantenimiento de Grupos combinados"
end type
global wtc_mant_grupo_combi wtc_mant_grupo_combi
...
param_colisiones.tabla = "grupo_combi"
...
on wtc_mant_grupo_combi.create
call super::create
end on

on wtc_mant_grupo_combi.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
end on

type pb_imprimir from wt_ventana_padre`pb_imprimir within wtc_mant_grupo_combi
end type

type p_logo from wt_ventana_padre`p_logo within wtc_mant_grupo_combi
end type

type dw_1 from wt_ventana_padre`dw_1 within wtc_mant_grupo_combi
integer y = 288
integer height = 328
string title = "Mantenimeinto de Colecciones"
string dataobject = "dwtc_grupo_combi"
...
CHOOSE CASE dwo.name
	...
	CASE "codigo"
		SELECT grupo_combi.descripcion
		INTO :resultado  
		FROM grupo_combi  
		WHERE ( grupo_combi.empresa = :codigo_empresa ) AND  
					( grupo_combi.codigo  = :data );
...
type pb_borrar from wt_ventana_padre`pb_borrar within wtc_mant_grupo_combi
end type

type pb_salir from wt_ventana_padre`pb_salir within wtc_mant_grupo_combi
end type

type pb_grabar from wt_ventana_padre`pb_grabar within wtc_mant_grupo_combi
end type

type st_titulo from wt_ventana_padre`st_titulo within wtc_mant_grupo_combi
end type

type pb_nuevo from wt_ventana_padre`pb_nuevo within wtc_mant_grupo_combi
end type

type pb_cancelar from wt_ventana_padre`pb_cancelar within wtc_mant_grupo_combi
end type

type pb_buscar from wt_ventana_padre`pb_buscar within wtc_mant_grupo_combi
end type
