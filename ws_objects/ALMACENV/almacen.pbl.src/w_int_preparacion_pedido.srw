$PBExportHeader$w_int_preparacion_pedido.srw
$PBExportComments$Obsoleto
forward
global type w_int_preparacion_pedido from w_int_con_empresa
end type
type pb_1 from upb_salir within w_int_preparacion_pedido
end type
type st_pedido from statictext within w_int_preparacion_pedido
end type
type em_pedido from u_em_campo within w_int_preparacion_pedido
end type
type st_anyo from statictext within w_int_preparacion_pedido
end type
type em_anyo from u_em_campo within w_int_preparacion_pedido
end type
type st_etiqueta_operario from statictext within w_int_preparacion_pedido
end type
type sle_operario from singlelineedit within w_int_preparacion_pedido
end type
type sle_operario_nombre from singlelineedit within w_int_preparacion_pedido
end type
type cb_salir from u_boton within w_int_preparacion_pedido
end type
type gb_2 from groupbox within w_int_preparacion_pedido
end type
type uo_almacen from u_em_campo_2 within w_int_preparacion_pedido
end type
type cb_preparar from u_boton within w_int_preparacion_pedido
end type
type dw_proceso from datawindow within w_int_preparacion_pedido
end type
type dw_movhis from datawindow within w_int_preparacion_pedido
end type
type dw_proceso_ubi from datawindow within w_int_preparacion_pedido
end type
type dw_almlinubicahis from datawindow within w_int_preparacion_pedido
end type
type cb_continuar from u_boton within w_int_preparacion_pedido
end type
type cb_stock from u_boton within w_int_preparacion_pedido
end type
type cb_borrar_pedido from u_boton within w_int_preparacion_pedido
end type
type dw_detalle from datawindow within w_int_preparacion_pedido
end type
type gb_1 from groupbox within w_int_preparacion_pedido
end type
type cb_modificar from u_boton within w_int_preparacion_pedido
end type
type st_fecha from statictext within w_int_preparacion_pedido
end type
type st_cliente from statictext within w_int_preparacion_pedido
end type
type st_1 from statictext within w_int_preparacion_pedido
end type
type uo_cliente from u_em_campo_2 within w_int_preparacion_pedido
end type
type pb_imprimir from upb_imprimir within w_int_preparacion_pedido
end type
type dw_conubi from datawindow within w_int_preparacion_pedido
end type
type cb_procesar from u_boton within w_int_preparacion_pedido
end type
type cb_3 from u_boton within w_int_preparacion_pedido
end type
type cb_borrar from u_boton within w_int_preparacion_pedido
end type
type cb_insertat from u_boton within w_int_preparacion_pedido
end type
type dw_preparacion from datawindow within w_int_preparacion_pedido
end type
type dw_pedidos from datawindow within w_int_preparacion_pedido
end type
type dw_desubi from u_datawindow within w_int_preparacion_pedido
end type
type gb_3 from groupbox within w_int_preparacion_pedido
end type
type dw_consulta from u_datawindow within w_int_preparacion_pedido
end type
type st_6 from statictext within w_int_preparacion_pedido
end type
end forward
...
global w_int_preparacion_pedido w_int_preparacion_pedido
...
Dec{0}  var_calibre,var_pedido,var_periodo,var_pallets,var_cajas,var_total_cajas,var_orden_preparacion,var_linea_pedido,var_numpalet
...
public function boolean f_desdoblar_linea (string arg_empresa, decimal arg_anyo, decimal arg_pedido, decimal arg_linea, decimal arg_cantidad)
...
public subroutine f_calculo_linea (integer elemento);Dec{2} metros,unidades,cajas,total_cajas,pallets,cantidad,pal,cj
...
IF ante_pallets= pallets and ante_cajas = cajas and cantidad = ante_cantidad Then Return
...
IF control_pico = "S" Then
	...
	ELSE
		...
		total_cajas    = Dec(Mid(cadena,13,6))
...
String situacion
	...
	IF dw_preparacion.GetRow() = 0 Then Return 
	 ...
	 IF Not f_bloquear_almacen(w_int_preparacion_pedido.title) Then
		...
		f_desbloquear(tabla,seleccion,titulo)
		COMMIT;
		f_desbloquear_almacen(w_int_preparacion_pedido.title)
	 	END IF
...
var_referencia = f_componer_ref(v_articulo,&
                                      v_calidad,&
                                      v_tono,v_calibre)
...
String  cur_empresa,cur_tipo_pallet,cur_ubicacion
         ...
         SetNull(linubica.observaciones)
...
//---------------------------------------------------------------------
// Despues de todos los controleses empezamos el proceso de inserción
//---------------------------------------------------------------------
...
IF registros<>0 THEN
 FOR x1 = 1 To registros
  ...
  IF (IsNull(cur_pedido) or trim(string(cur_pedido))="") and (cur_tipo_pallet=ubihis.tipo_pallet) THEN
     ...
     var_observaciones = cur_observaciones
       UPDATE almlinubica  
...
SELECT max(almlinubica.linea)  INTO :cur_linea   FROM  almlinubica  
   ...
   END IF
    cur_linea=cur_linea+1
    ubi.linea=cur_linea
    ubi.observaciones=mov.observaciones
...
public function boolean f_desdoblar_linea (string arg_empresa, decimal arg_anyo, decimal arg_pedido, decimal arg_linea, decimal arg_cantidad);Dec disponible
str_venliped     ln  
str_almlincargas  la
Dec numero,vbien = 0
...
SELECT venliped.empresa, venliped.anyo, venliped.pedido,  venliped.linea,   
         ...
         venliped.almacen_deposito, venliped.envio  
   ...
   IF ln.cantidad <> 0 Then
			...
			ln.pallets      = Dec(Mid(cadena,1,6))	
			ln.cajas        = Dec(Mid(cadena,7,6))
			ln.total_cajas  = Dec(Mid(cadena,13,6))
			ln.cantidad     = Dec(Mid(cadena,19,9))
			IF control_ordenes = "S" Then
				la.linea        = la.linea +1
...
public function boolean f_insertar ();String preparado
...
event open;call super::open;istr_parametros.s_titulo_ventana="Preparación pedido"
...
on w_int_preparacion_pedido.create
int iCurrent
call super::create
...
this.Control[iCurrent+28]=this.dw_conubi
this.Control[iCurrent+29]=this.cb_procesar
this.Control[iCurrent+30]=this.cb_3
this.Control[iCurrent+31]=this.cb_borrar
this.Control[iCurrent+32]=this.cb_insertat
this.Control[iCurrent+33]=this.dw_preparacion
this.Control[iCurrent+34]=this.dw_pedidos
this.Control[iCurrent+35]=this.dw_desubi
this.Control[iCurrent+36]=this.gb_3
this.Control[iCurrent+37]=this.dw_consulta
this.Control[iCurrent+38]=this.st_6
end on

on w_int_preparacion_pedido.destroy
call super::destroy
...
event activate;call super::activate;w_int_preparacion_pedido = ventanas_activas[id_ventana_activa].ventana
end event

type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_int_preparacion_pedido
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_int_preparacion_pedido
end type

event sle_opcion_orden::getfocus;call super::getfocus;f_activar_campo(em_pedido)
end event

type st_empresa from w_int_con_empresa`st_empresa within w_int_preparacion_pedido
end type
...
event clicked;call super::clicked;dw_consulta.reset()
dw_consulta.enabled = FALSE
...
String  preparado,calidad,nombre_operario,cod_operario,articulo,familia,control_pico,tipo_pallet,tipo_unidad,referencia,ubicacion,tono
Dec{2}  metros,unidades,pedido,cantidad,disponible
dw_consulta.AcceptText()
...
IF preparado = "S" Then  
		...
		disponible = f_disponible_referencia_tipo_pallet(codigo_empresa,&
						 referencia,dw_consulta.GetItemString(registro,"tipo_pallet_entrada"))
...
Else	
   ...
   IF MessageBox("Preparación pico","El pico se prepara en Pallet Normal? ",Question!,YesNo!,1)	=1 Then
   var_tipo_pallet_entrada = "1"
END IF
END IF	

  // para prepararar un material
...
//  f_calculo_linea(registro)
  ...
  IF total_reg =0 then
	    f_mensaje("No Existe Disponible","No Hay Stock Disponible a esta referencia")
    	 dw_consulta.SetColumn("tono")
	    dw_consulta.SetFocus()
		 Return
	Else
...
String  preparado,calidad,nombre_operario,cod_operario,articulo,familia,control_pico,tipo_pallet,tipo_unidad,referencia,ubicacion,texto,situacion,tono,cadena,caja
Dec  metros,unidades,cajas,total_cajas,var_anyo,cantidad,pedido,disponible,cantidda,cantidad_anterior
int opcion
...
Select venliped.cantidad,venliped.situacion 
Into  :cantidad_anterior,:ante_situ
From   venliped
WHERE  venliped.empresa  =  :codigo_empresa 
AND    venliped.anyo     =  :anyo 
AND    venliped.pedido   =  :pedido 
...
IF MessageBox("Proceso modificación linea de pedido","¿Desea modificar la linea de pedido?",Question!,YesNo!) = 1 Then
		...
		preparado    =  dw_consulta.GetItemString(registro,"preparado")
		IF preparado = "S" Then
			f_mensaje("Esta linea no se puede modificar","La linea esta preparada")
			Return
		END IF
		...
		disponible = f_disponible_referencia_tipo_pallet(codigo_empresa,&
						 referencia,dw_consulta.GetItemString(registro,"tipo_pallet"))
		iF IsNUll(disponible) Then disponible = 0				 
		if dw_consulta.GetItemString(registro,"situacion") <> "F" then
   		disponible = disponible + cantidad_anterior
		end if
		
	   situacion = "C"

	   If disponible  < cantidad Then
				opcion = MessageBox("No existe bastante disponible"," Desea grabar la lina en situación (FABRICAR) ?",Question!, YesNo!,2) 
				If opcion = 2 THEN 
						Return
					ELSE
						situacion = "F"
      ...
      End if
		...
		tono             =  dw_consulta.GetItemString(registro,"tono")
		calibre          =  dw_consulta.GetItemNumber(registro,"calibre")
		cajas            =  dw_consulta.GetItemNumber(registro,"cajas")
		total_cajas      =  dw_consulta.GetItemNumber(registro,"total_cajas")
		referencia= f_componer_ref(articulo,calidad,tono,calibre)
		cantidad_desglose = 0
		IF cantidad_anterior > cantidad Then
				IF MessageBox("Control Resto","Dejar el resto pendiente",Question!,YesNo!) = 2 Then
					 cantidad_desglose = 0
				  ELSE
					 cantidad_desglose = cantidad_anterior - cantidad
				END IF
		END IF
		...
		END IF
			...
			cadena          =f_calculo_unidades(codigo_empresa,articulo,var_tipo_pallet,caja,0,0,cantidad)
			pallets      = Dec(Mid(cadena,1,6))	
			cajas        = Dec(Mid(cadena,7,6))
			total_cajas  = Dec(Mid(cadena,13,6))
			cantidad     = Dec(Mid(cadena,19,9))
		...
		SET    tonochar     = :tono,   
		 ...
		 WHERE venliped.empresa = :codigo_empresa 
			...
			//Desdoblar linea de pedido,y control_modificacion orden
			f_desdoblar_linea(codigo_empresa,anyo,pedido,linea,cantidad_desglose)
...
event clicked;call super::clicked;dw_desubi.AcceptTExt()
IF Not f_bloquear_almacen(w_int_preparacion_pedido.title) Then
	f_procesar_desubi()
	dw_consulta.Setfocus()
	dw_consulta.SetRow(1)
...
event clicked;call super::clicked;cb_borrar.visible    = FALSE
...
IF key = KeyF5! Then 
	...
	IF This.GetRow() <> 0 Then
     ...
     This.SetItem(This.GetRow(),"cantidad_marcada",Dec(Mid(cadena,19,9)))
	END IF
	This.Accepttext()
	f_procesar()
	END IF
IF key = KeyEscape! Then f_salir()
...
IF dwo.name= "salir" Then f_salir()
end event
...
event rowfocuschanged;call super::rowfocuschanged; IF procesando = "S" Then Return
 If currentrow = 0 Then  Return
 if rowcount()=0 then return
 String  var_operario,preparado
