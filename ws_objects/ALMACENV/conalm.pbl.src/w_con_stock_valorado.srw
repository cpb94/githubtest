$PBExportHeader$w_con_stock_valorado.srw
forward
global type w_con_stock_valorado from w_int_con_empresa
end type
type pb_2 from upb_salir within w_con_stock_valorado
end type
type dw_1 from u_datawindow_consultas within w_con_stock_valorado
end type
type em_fechadesde from u_em_campo within w_con_stock_valorado
end type
type st_7 from statictext within w_con_stock_valorado
end type
type cb_2 from commandbutton within w_con_stock_valorado
end type
type st_2 from statictext within w_con_stock_valorado
end type
type em_tarifa from u_em_campo within w_con_stock_valorado
end type
type sle_fecha from u_em_campo within w_con_stock_valorado
end type
type st_nombre_tarifa from statictext within w_con_stock_valorado
end type
type dw_fechas from datawindow within w_con_stock_valorado
end type
type pb_imprimir_preli from upb_imprimir within w_con_stock_valorado
end type
end forward
...
global w_con_stock_valorado w_con_stock_valorado
...
on w_con_stock_valorado.create
int iCurrent
call super::create
...
on w_con_stock_valorado.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_stock_valorado
integer taborder = 60
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_stock_valorado
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_stock_valorado
integer y = 20
integer width = 2587
integer height = 92
end type
...
integer height = 1048
integer taborder = 70
string dataobject = "dw_con_stock_valorado"
...
event clicked;string articulo,calidad,tarifa
dec    precio
...
f_mascara_columna(dw_1,"totalglobal",f_mascara_moneda(f_moneda_ventarifas( codigo_empresa , tarifa )) )	//f_mascara_moneda(f_moneda_ventarifas( empresa , tarifa ))
f_mascara_columna(dw_1,"cimporte",f_mascara_moneda(f_moneda_ventarifas( codigo_empresa , tarifa )) )	//f_mascara_moneda(f_moneda_ventarifas( empresa , tarifa ))

dw_1.Retrieve(codigo_empresa,tarifa,fecha_tarifa,fecha)
...
//		pagina = Int(cont/56)
		...
		precio   = f_precio_articulo_venlintarifas(codigo_empresa,tarifa,fecha_tarifa,articulo,calidad)
   	//li_stock = f_stock_articulo_calidad_fecha(codigo_empresa,articulo,calidad,fecha)
		li_stock = 0
		dw_1.SetItem(cont,"cstock",li_stock)
		dw_1.SetItem(cont,"articulos_precio",precio)
//		dw_1.SetItem(cont,"cpagina",pagina)
		li_subtotal = li_stock * precio
		dw_1.SetItem(cont,"cimporte",li_subtotal)
		f_mensaje_proceso("Procesando",cont,dw_1.RowCount())
	next
	dw_1.setfilter("cstock <> 0")
	dw_1.filter()
