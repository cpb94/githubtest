$PBExportHeader$w_expedir_pedidos.srw
forward
global type w_expedir_pedidos from w_int_con_empresa
end type
type pb_1 from upb_salir within w_expedir_pedidos
end type
type em_cliente from u_em_campo within w_expedir_pedidos
end type
type st_nomcliente from statictext within w_expedir_pedidos
end type
type cb_marcar_todo from u_boton within w_expedir_pedidos
end type
type st_operario from statictext within w_expedir_pedidos
end type
type uo_operario from u_em_campo_2 within w_expedir_pedidos
end type
type st_fecha from statictext within w_expedir_pedidos
end type
type em_fecha from u_em_campo within w_expedir_pedidos
end type
type st_albaran from statictext within w_expedir_pedidos
end type
type em_albaran from u_em_campo within w_expedir_pedidos
end type
type listar from radiobutton within w_expedir_pedidos
end type
type em_copias from u_em_campo within w_expedir_pedidos
end type
type st_copias from statictext within w_expedir_pedidos
end type
type cb_1 from u_boton within w_expedir_pedidos
end type
type dw_etiqueta_agencia from datawindow within w_expedir_pedidos
end type
type cb_informacion from commandbutton within w_expedir_pedidos
end type
type dw_consulta from u_datawindow_consultas within w_expedir_pedidos
end type
type dw_temporal from datawindow within w_expedir_pedidos
end type
type dw_albaranes from datawindow within w_expedir_pedidos
end type
type sle_articulo from singlelineedit within w_expedir_pedidos
end type
type dw_ubicaciones from datawindow within w_expedir_pedidos
end type
type dw_infor from datawindow within w_expedir_pedidos
end type
type dw_1 from datawindow within w_expedir_pedidos
end type
type orden_anyo from editmask within w_expedir_pedidos
end type
type orden_codigo from editmask within w_expedir_pedidos
end type
type gb_1 from groupbox within w_expedir_pedidos
end type
type gb_2 from groupbox within w_expedir_pedidos
end type
type gb_3 from groupbox within w_expedir_pedidos
end type
type st_1 from statictext within w_expedir_pedidos
end type
type pb_3 from picturebutton within w_expedir_pedidos
end type
type st_cmr from statictext within w_expedir_pedidos
end type
type em_cmr from u_em_campo within w_expedir_pedidos
end type
type dw_listado_cmr from datawindow within w_expedir_pedidos
end type
type dw_listado_vgm from datawindow within w_expedir_pedidos
end type
type dw_pedido from u_datawindow_consultas within w_expedir_pedidos
end type
type dw_detalle from u_datawindow_consultas within w_expedir_pedidos
end type
end forward
...
global w_expedir_pedidos w_expedir_pedidos
...
// David
IF linped.cantidad > 0 or linped.cantidad_pzs > 0 then
//IF linped.cantidad <> 0 then
// Fin David
	UPDATE venliped
	Set venliped.pallets           = :linped.pallets,
...
ELSE
	   ...
	   WHERE (venliped.empresa = :codigo_empresa )
		...
		//BORRADO LINEAS ORDENES DE CARGA **************************************
		DELETE FROM orden_carga_linea_origen 
		WHERE EXISTS (SELECT * FROM orden_carga_linea 
							WHERE orden_carga_linea.empresa       = orden_carga_linea_origen.empresa 
							AND orden_carga_linea.anyo            = orden_carga_linea_origen.anyo 
							AND orden_carga_linea.codigo          = orden_carga_linea_origen.codigo 
							AND orden_carga_linea.linea           = orden_carga_linea_origen.linea 
							AND orden_carga_linea.empresa         = :codigo_empresa 
							AND orden_carga_linea.venliped_anyo   = :var_anyo 
							AND orden_carga_linea.venliped_pedido = :var_pedido
							AND orden_carga_linea.venliped_linea  = :var_linea);
							
		DELETE FROM orden_carga_linea_reservas 
		WHERE EXISTS (SELECT * FROM orden_carga_linea 
		...
		//FIN BORRADO LINEAS ORDENES DE CARGA **************************************
		
		
		//Control lineas de ordenes de carga (ANTIGUO - NO USADO)
		select count(*)
		into   :lineas_ordenes_carga
		from   almlincargas
		where  empresa   = :codigo_empresa
		and    anyo_ped  = :var_anyo
		and    pedido    = :var_pedido
		and    linea_ped = :var_linea;
		
		if isnull(lineas_ordenes_carga) then lineas_ordenes_carga = 0
		if lineas_ordenes_carga > 0 then
			...
			IF SQLCA.SQLCODE <> 0  Then 	
				var_bien = 1
				f_mensaje("Error al borrar las lineas de ordenes de carga",sqlca.sqlErrtext)
			END IF				
		end if
		//Fin control lineas ordenes de carga
END IF
...
if var_bien = 0 then
	//BORRADO ORDENES DE CARGA VACIAS **************************************
	
	DELETE FROM orden_carga 
	WHERE finalizada = 'S' AND 0>= ISNULL((SELECT COUNT(*) FROM orden_carga_linea o
						WHERE o.empresa = orden_carga.empresa 
						AND o.anyo = orden_carga.anyo 
						AND o.codigo = orden_carga.codigo), 0);			
			
	IF SQLCA.SQLCODE <> 0  Then 	
		var_bien = 1
		f_mensaje("Error al borrar orden_carga",sqlca.sqlErrtext)
	END IF			
			
	//Borrado de lineas de ordenes de carga sueltas  (No deberían haber, pero por si acaso)
	DELETE FROM orden_carga_linea 
	WHERE NOT EXISTS (SELECT * FROM orden_carga o
						WHERE o.empresa = orden_carga_linea.empresa 
						AND o.anyo = orden_carga_linea.anyo 
						AND o.codigo = orden_carga_linea.codigo);		

	IF SQLCA.SQLCODE <> 0  Then 	
		var_bien = 1
		f_mensaje("Error al borrar orden_carga_linea",sqlca.sqlErrtext)
	END IF									
						
	DELETE FROM orden_carga_linea_origen 
	WHERE NOT EXISTS (SELECT * FROM orden_carga o
						WHERE o.empresa = orden_carga_linea_origen.empresa 
						AND o.anyo = orden_carga_linea_origen.anyo 
						AND o.codigo = orden_carga_linea_origen.codigo);		

	IF SQLCA.SQLCODE <> 0  Then 	
		var_bien = 1
		f_mensaje("Error al borrar orden_carga_linea_origen",sqlca.sqlErrtext)
	END IF									
	
	//FIN BORRADO ORDENES DE CARGA VACIAS **************************************
end if
...
Dec{0} var_anyo,var_linea,contador,var_calibre, var_total_cajas
string cadena,var_referencia,var_tipo_pallet,var_caja, var_orden_carga, &
	...
	v_almacen,var_cliente,var_almacen,var_zona,var_tipolinea,var_tono, var_familia, var_formato, var_tipounidad, var_subformato, var_modelo, var_subfamilia
Dec {6} var_cantidad_carga,total_cantidad,cur_existencias,cantidad_pendiente,cantidad_borrada 
Dec {6} var_metroscaja
Integer var_bien
Date fecha
Datetime ahora
Dec {6} ln
...
if f_control_almacen_ventipolin(codigo_empresa,var_tipolinea)="S" then
	...
	var_orden_carga = " "
	if not isnull(dw_pedido.GetItemNumber(elemento,"orden_carga_anyo")) and dw_pedido.GetItemNumber(elemento,"orden_carga_anyo") > 0 then
		var_orden_carga = " OP:"+String(dw_pedido.GetItemNumber(elemento,"orden_carga_anyo")) + "/" + dw_pedido.GetItemString(elemento,"orden_carga_codigo") + " "
	end if
	
	var_ubicacion      = f_componer_ubicacion(dw_pedido.GetItemString(elemento,"almacen"), dw_pedido.GetItemString(elemento,"zona"), dw_pedido.GetItemNumber(elemento,"fila"), dw_pedido.GetItemNumber(elemento,"altura"))
	if isnull(var_ubicacion) then var_ubicacion = ""
	if trim(var_ubicacion) = "" then
		var_ubicacion = "00001A    000100"
	end if
	...
	ls_gestionar_en_piezas = dw_pedido.GetItemString(elemento,"ventipolin_gestionar_en_piezas")
	var_cantidad_carga_pzs = dw_pedido.GetItemNumber(elemento,"venliped_cantidad_pzs")
	...
	if ls_gestionar_en_piezas = "S" then
		total_cantidad = var_cantidad_carga_pzs
	else
		if var_tipounidad = "1" then //metros
			total_cantidad =  truncate(var_cantidad_carga / var_metroscaja,0) * var_piezascaja
		else
	...
	lstr_movimiento_almacen.cantidad           = (total_cantidad * -1)
	lstr_movimiento_almacen.orden_carga_anyo   = dw_pedido.object.orden_carga_anyo[elemento]
	lstr_movimiento_almacen.orden_carga_codigo = dw_pedido.object.orden_carga_codigo[elemento]
	lstr_movimiento_almacen.orden_carga_linea  = dw_pedido.object.orden_carga_linea_linea[elemento]
	...
	lstr_movimiento_almacen.almacen            = var_almacen
	lstr_movimiento_almacen.zona               = var_zona
	lstr_movimiento_almacen.fila               = var_fila
	lstr_movimiento_almacen.altura             = var_altura
	lstr_movimiento_almacen.tercero            = ""
	lstr_movimiento_almacen.documento          = ""
	lstr_movimiento_almacen.fecha              = datetime(date(ahora))
	lstr_movimiento_almacen.id_ubicacion       = 0
	lstr_movimiento_almacen.id_alm_bultos		 = dw_pedido.object.id_alm_bultos[elemento]
	...
	if not(f_movimiento_almacen(lstr_movimiento_almacen)) then
		...
		if var_cliente = '1663' then
			...
			lstr_movimiento_almacen.tipo_pallet        = var_tipo_pallet
			lstr_movimiento_almacen.caja               = var_caja
			lstr_movimiento_almacen.cantidad           = total_cantidad
	...
	movimiento_nuevo.tipomov = "206"
	movimiento_nuevo.usuario = nombre_usuario
	movimiento_nuevo.almacen = var_almacen
	movimiento_nuevo.zona = var_zona
	movimiento_nuevo.fila = var_fila
	movimiento_nuevo.altura = var_altura
	movimiento_nuevo.articulo = var_articulo	
	movimiento_nuevo.calidad = var_calidad
	movimiento_nuevo.tono = var_tono
	movimiento_nuevo.calibre = var_calibre
	movimiento_nuevo.fecha_mov = date(ahora)
	movimiento_nuevo.orden = 0
	movimiento_nuevo.fecha_intr = ahora
	movimiento_nuevo.tipo_pallet = var_tipo_pallet
	movimiento_nuevo.caja = var_caja
	
	
	// David: El formato ha de ser el del pedido, no el del artículo. Lo quito de esta consulta.
	SELECT articulos.familia, articulos.subformato, articulos.subfamilia, articulos.modelo 
	INTO   :var_familia,  :var_subformato, :var_subfamilia, :var_modelo
	FROM   articulos  
	WHERE  articulos.empresa = :codigo_empresa
	AND    articulos.codigo = :var_articulo;
	...
	movimiento_nuevo.tipo_unidad = var_tipounidad
	
	movimiento_nuevo.origen = ''
	//movimiento_nuevo.nummov = ''
	movimiento_nuevo.observaciones = "EXPEDICIÓN"+var_orden_carga+"- Albarán "+string(var_albaran[var_contador],"########")
	movimiento_nuevo.trazabilidad = ""	
	
	movimiento_nuevo.cantidade = 0
	movimiento_nuevo.cantidads = total_cantidad
	
	DELETE FROM almacenmovimientos_bloqueo 
	WHERE empresa = :codigo_empresa 
	AND almacen = :var_almacen 
	AND zona = :var_zona
	AND fila = :var_fila
	...
	*/
	...
	IF var_deposito = "S" and var_cliente <> '1663' Then
	///-----------------------------------------------------------------
	...
	///-----------------------------------------------------------------
	 ...
	 depubi.articulo      = var_articulo
	 depubi.familia       = var_familia
	 depubi.subfamilia    = var_subfamilia
	 depubi.formato       = var_formato
	 depubi.subformato    = var_subformato
	 depubi.modelo        = var_modelo
	 depubi.calidad       = var_calidad
	 depubi.tonochar      = var_tono
	 depubi.calibre       = var_calibre
	 depubi.f_alta        = ahora
	 depubi.tipo_pallet   = var_tipo_pallet
	 
	 
	 depubi.caja          = var_caja
	 depubi.tipo_unidad   = var_tipounidad
	 depubi.referencia    = var_referencia     
...
//IF dw_detalle.GetItemString(t,"venliped_situacion")<> "F" Then
		...
		var_peso_bruto_linea = dw_detalle.GetItemNumber(t,"venliped_peso")
		var_peso_neto_linea = dw_detalle.GetItemNumber(t,"venliped_peso_neto")
...
For reg = 1 To reg1
	...
	ub  = dw_pedido.GetItemSTring(reg,"zona")
...
orden_anyo.text = String(Year(Today()))

end event

on w_expedir_pedidos.create
int iCurrent
call super::create
...
this.orden_anyo=create orden_anyo
...
this.Control[iCurrent+24]=this.orden_anyo
this.Control[iCurrent+25]=this.orden_codigo
...
this.Control[iCurrent+30]=this.pb_3
this.Control[iCurrent+31]=this.st_cmr
this.Control[iCurrent+32]=this.em_cmr
this.Control[iCurrent+33]=this.dw_listado_cmr
this.Control[iCurrent+34]=this.dw_listado_vgm
this.Control[iCurrent+35]=this.dw_pedido
this.Control[iCurrent+36]=this.dw_detalle
end on

on w_expedir_pedidos.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_expedir_pedidos
integer x = 82
integer y = 1268
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_expedir_pedidos
end type

event sle_opcion_orden::getfocus;call super::getfocus;f_activar_campo(em_cliente)
end event

type st_empresa from w_int_con_empresa`st_empresa within w_expedir_pedidos
integer x = 55
integer y = 16
integer width = 4539
integer height = 80
...
//				var_cantidad_facturar = dw_temporal.GetItemNumber(n,"cantidad_facturar")
				...
				IF f_control_almacen_ventipolin(codigo_empresa,var_tipolinea)="S" and (not isnull(articulo) and articulo <> "") then
					...
					IF dw_pedido.GetItemString(t,"venliped_situacion") = "P" Then
						var_ubicacion = f_ubicaciones_referencia_preparado(codigo_empresa,var_referencia,var_tipo_pallet,var_anyo,var_pedido,var_linea)
					Else
						IF IsNull(var_tipo_pallet) Then  var_tipo_pallet = ""
						var_almacen_de_carga = f_almacen_de_carga_venped(codigo_empresa,var_anyo,var_pedido)
...
//						if IsNull(var_ubicacion) or var_ubicacion = "" then
//							var_ubicacion = "00001A    000100"	
//						end if
					END IF
					*/
					...
					//ALMACEN NUEVO 01/2014 
					//var_almacen_de_carga = f_almacen_de_carga_venped(codigo_empresa,var_anyo,var_pedido)
					//ANGEL: EL ALMACEN Y LA ZONA SIEMPRE VIENEN FIJADAS A 1 P
//					zona = "P"
//					fila = 0
//					altura = 0
						
					//NOVEDAD 04/2015 - Las ordenes de carga obtienen la ubicación de destino guardada
					var_oc_almacen_destino = dw_pedido.object.orden_carga_almacen_destino[t]
					var_oc_zona_destino = dw_pedido.object.orden_carga_zona_destino[t]
					var_oc_fila_destino = dw_pedido.object.orden_carga_fila_destino[t]
					var_oc_altura_destino = dw_pedido.object.orden_carga_altura_destino[t]
					if not isnull(var_oc_almacen_destino) and var_oc_almacen_destino <> "" and not isnull(var_oc_zona_destino) and var_oc_zona_destino <> "" and not isnull(var_oc_fila_destino) and var_oc_fila_destino > 0 and not isnull(var_oc_altura_destino) and var_oc_altura_destino > 0 then
						hay_stock = 1
						var_almacen_de_carga = var_oc_almacen_destino
						zona = var_oc_zona_destino
						fila = var_oc_fila_destino
						altura = var_oc_altura_destino
					else
						var_almacen_de_carga = "1"
						zona = "P"
						if var_tipo_unidad = "1"  and var_metros_caja <> 0  then
							var_cantidad_unidad = dec(string(round((var_cantidad / var_metros_caja) * var_piezas_caja,0),'###,###,###.##'))
						else
							var_cantidad_unidad = var_cantidad
						end if
						hay_stock = f_ubicacion_ref_pallet_caja_cantidad_alm(codigo_empresa,var_tipo_pallet,var_caja,var_cantidad_unidad,articulo,var_almacen_de_carga,zona,fila,altura,calibre,tono,calidad, formato)
					end if
						
					if hay_stock = 1 then
						dw_pedido.SetItem(t,"almacen",var_almacen_de_carga)
						dw_pedido.SetItem(t,"zona",zona)
						dw_pedido.SetItem(t,"fila",fila)
						dw_pedido.SetItem(t,"altura",altura)
					end if
				END IF
...
IF This.GetItemString(GetRow(),"venliped_situacion") = "P" Then Return
...
integer height = 2484
integer taborder = 30
string dataobject = "dw_expedir_pedidos2"
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
