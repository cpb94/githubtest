$PBExportHeader$wtc_consulta_ultima_factura.srw
forward
global type wtc_consulta_ultima_factura from wt_consultas_padre
end type
type st_reservado from statictext within wtc_consulta_ultima_factura
end type
type sle_observaciones from singlelineedit within wtc_consulta_ultima_factura
end type
type pb_grabar_obs from picturebutton within wtc_consulta_ultima_factura
end type
type st_1 from statictext within wtc_consulta_ultima_factura
end type
type dw_etiquetas_bultos_100 from datawindow within wtc_consulta_ultima_factura
end type
end forward
...
global wtc_consulta_ultima_factura wtc_consulta_ultima_factura
...
public function string f_montar_filtro (string consulta_inicial);string montaje, principio
int j
...
if montaje <> '' then
	...
	principio = " ) as TOTAL "+&
					" from ( select max (venlifac.ffactura) as fecha, venlifac.articulo, venlifac.anyo, venlifac.factura, venlifac.linea "+&
					" from venlifac "+&
					" inner join genter on venlifac.cliente = genter.codigo"+&
					" where venlifac.articulo <> '' and venlifac.articulo is not null "+&
					" and venlifac.empresa = '"+codigo_empresa+"' " 
					
				
	montaje = consulta_inicial + " AND " + montaje + principio +  " AND " + montaje + " group by venlifac.articulo, "+&
	" venlifac.anyo, venlifac.factura, venlifac.linea ) as consulta"+&
	" inner join venlifac "+&
	" on venlifac.factura = consulta.factura and venlifac.anyo = consulta.anyo and venlifac.linea = consulta.linea "+&
	" inner join genter "+&
	" on venlifac.cliente = genter.codigo"+&
	" inner join articulos "+&
	" on venlifac.articulo = articulos.codigo"
...
public function string f_construir_consulta ();string cadena, cadena_1
datastore ds_datos



cadena = " select consulta.articulo as codigo_articulo, articulos.descripcion as descripcion, consulta.fecha as fecha_ultima,  venlifac.cantidad as cantidad, "+&
" ( SELECT SUM(CANTIDAD) FROM VENLIFAC V2 WHERE V2.empresa = '"+codigo_empresa+"' AND V2.ARTICULO = VENLIFAC.ARTICULO "
...
on wtc_consulta_ultima_factura.create
int iCurrent
call super::create
...
on wtc_consulta_ultima_factura.destroy
call super::destroy
...
type st_registros_padre from wt_consultas_padre`st_registros_padre within wtc_consulta_ultima_factura
integer x = 5477
integer y = 144
end type

type st_total_padre from wt_consultas_padre`st_total_padre within wtc_consulta_ultima_factura
integer x = 5719
integer y = 144
end type

type pb_imprimir from wt_consultas_padre`pb_imprimir within wtc_consulta_ultima_factura
integer x = 3570
integer y = 104
integer height = 124
integer taborder = 0
...
type pb_borrar from wt_consultas_padre`pb_borrar within wtc_consulta_ultima_factura
integer x = 3726
integer y = 104
integer height = 124
integer taborder = 0
...
type dw_montaje_filtro from wt_consultas_padre`dw_montaje_filtro within wtc_consulta_ultima_factura
integer width = 6245
integer height = 224
integer taborder = 30
end type

type pb_1 from wt_consultas_padre`pb_1 within wtc_consulta_ultima_factura
integer x = 3305
integer y = 108
integer height = 124
integer taborder = 0
...
type pb_reset from wt_consultas_padre`pb_reset within wtc_consulta_ultima_factura
integer x = 3886
integer y = 104
integer height = 124
integer taborder = 0
...
type pb_anyadir1 from wt_consultas_padre`pb_anyadir1 within wtc_consulta_ultima_factura
integer x = 3150
integer y = 108
integer height = 124
integer taborder = 0
...
if ddlb_campo.text <> "" or dw_montaje_filtro.rowcount() = 0 then
	...
	choose case ddlb_campo.text
		...
		case "Periodo factura"
			if cadena_or = '' then
				cadena_or = " venlifac.ffactura "+ddlb_operador.text+" '"+em_fecha1.text+"' "
...
type ddlb_operador from wt_consultas_padre`ddlb_operador within wtc_consulta_ultima_factura
integer x = 1280
integer y = 128
integer width = 293
integer taborder = 0
...
type ddlb_campo from wt_consultas_padre`ddlb_campo within wtc_consulta_ultima_factura
integer x = 27
integer y = 128
integer width = 1211
integer height = 2220
...
type dw_datos from wt_consultas_padre`dw_datos within wtc_consulta_ultima_factura
integer y = 512
integer width = 6222
integer height = 3008
integer taborder = 40
string dataobject = "dwtc_consulta_ultima_factura"
end type

type pb_salir from wt_consultas_padre`pb_salir within wtc_consulta_ultima_factura
integer x = 4046
integer y = 104
integer height = 124
integer taborder = 0
...
type st_titulo from wt_consultas_padre`st_titulo within wtc_consulta_ultima_factura
end type

type p_logo from wt_consultas_padre`p_logo within wtc_consulta_ultima_factura
integer x = 5806
integer y = 4
end type

type dw_filtro from wt_consultas_padre`dw_filtro within wtc_consulta_ultima_factura
integer x = 1605
integer y = 240
integer width = 4247
integer height = 260
...
type pb_buscar from wt_consultas_padre`pb_buscar within wtc_consulta_ultima_factura
integer x = 2939
integer y = 112
integer width = 128
integer height = 112
...
type sle_valor1 from wt_consultas_padre`sle_valor1 within wtc_consulta_ultima_factura
integer x = 1591
integer y = 132
integer width = 421
integer height = 108
...
type em_fecha1 from wt_consultas_padre`em_fecha1 within wtc_consulta_ultima_factura
integer x = 1605
integer height = 96
integer taborder = 90
string text = ""
...
type sle_texto from wt_consultas_padre`sle_texto within wtc_consulta_ultima_factura
integer x = 1609
integer y = 132
integer width = 1321
integer height = 96
...
type lb_lista from wt_consultas_padre`lb_lista within wtc_consulta_ultima_factura
integer x = 1609
integer y = 132
integer width = 462
integer height = 356
