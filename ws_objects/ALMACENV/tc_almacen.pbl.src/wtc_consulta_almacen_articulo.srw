$PBExportHeader$wtc_consulta_almacen_articulo.srw
forward
global type wtc_consulta_almacen_articulo from wt_consultas_padre
end type
type st_info from statictext within wtc_consulta_almacen_articulo
end type
type progreso from hprogressbar within wtc_consulta_almacen_articulo
end type
type mermas from checkbox within wtc_consulta_almacen_articulo
end type
type cb_calcular from commandbutton within wtc_consulta_almacen_articulo
end type
type st_1 from statictext within wtc_consulta_almacen_articulo
end type
type cb_calcular_hist from commandbutton within wtc_consulta_almacen_articulo
end type
type ddlb_fechas from dropdownlistbox within wtc_consulta_almacen_articulo
end type
type cb_detalle_stock from commandbutton within wtc_consulta_almacen_articulo
end type
type dw_detalle_stock from datawindow within wtc_consulta_almacen_articulo
end type
type gb_2 from groupbox within wtc_consulta_almacen_articulo
end type
end forward
...
st_1 st_1
cb_calcular_hist cb_calcular_hist
ddlb_fechas ddlb_fechas
cb_detalle_stock cb_detalle_stock
dw_detalle_stock dw_detalle_stock
gb_2 gb_2
end type
global wtc_consulta_almacen_articulo wtc_consulta_almacen_articulo
...
on wtc_consulta_almacen_articulo.create
int iCurrent
call super::create
...
this.cb_calcular_hist=create cb_calcular_hist
...
this.Control[iCurrent+6]=this.cb_calcular_hist
this.Control[iCurrent+7]=this.ddlb_fechas
this.Control[iCurrent+8]=this.cb_detalle_stock
this.Control[iCurrent+9]=this.dw_detalle_stock
this.Control[iCurrent+10]=this.gb_2
end on

on wtc_consulta_almacen_articulo.destroy
call super::destroy
...
type st_registros_padre from wt_consultas_padre`st_registros_padre within wtc_consulta_almacen_articulo
integer x = 2807
integer y = 20
end type

type st_total_padre from wt_consultas_padre`st_total_padre within wtc_consulta_almacen_articulo
integer x = 3049
integer y = 20
end type

type pb_imprimir from wt_consultas_padre`pb_imprimir within wtc_consulta_almacen_articulo
integer x = 6926
integer y = 124
integer taborder = 0
end type
...
type pb_borrar from wt_consultas_padre`pb_borrar within wtc_consulta_almacen_articulo
integer x = 7081
integer y = 124
integer taborder = 0
end type

type dw_montaje_filtro from wt_consultas_padre`dw_montaje_filtro within wtc_consulta_almacen_articulo
integer width = 3273
integer height = 224
integer taborder = 40
end type

type pb_1 from wt_consultas_padre`pb_1 within wtc_consulta_almacen_articulo
integer x = 3186
integer y = 108
integer taborder = 0
end type
...
type pb_reset from wt_consultas_padre`pb_reset within wtc_consulta_almacen_articulo
integer x = 7237
integer y = 124
integer taborder = 0
end type

type pb_anyadir1 from wt_consultas_padre`pb_anyadir1 within wtc_consulta_almacen_articulo
integer x = 3035
integer y = 108
integer taborder = 0
end type
	...
	choose case ddlb_campo.text
		...
		case "Colección"
			for i = 1 to dw_filtro.rowcount()
				if dw_filtro.IsSelected (i)  then
					if cadena_or = '' then
						cadena_or = " almacenmovimientos.articulo IN ( SELECT codigo FROM articulos WHERE articulos.submarcas_codigo = '"+dw_filtro.object.#1[i]+"' ) "
						cadena_or_visible = " Colección = "+dw_filtro.object.#2[i]
					else
						cadena_or = cadena_or+" OR almacenmovimientos.articulo IN ( SELECT codigo FROM articulos WHERE articulos.submarcas_codigo = '"+dw_filtro.object.#1[i]+"' ) "
						cadena_or_visible = cadena_or_visible+", "+dw_filtro.object.#2[i]
					end if
				end if
			next 					
...
type ddlb_operador from wt_consultas_padre`ddlb_operador within wtc_consulta_almacen_articulo
integer x = 951
integer taborder = 0
integer weight = 700
...
type ddlb_campo from wt_consultas_padre`ddlb_campo within wtc_consulta_almacen_articulo
integer width = 878
integer height = 1308
integer taborder = 0
integer weight = 700
...
string text = "0"
boolean sorted = false
string item[] = {"Fecha","Articulo (Descripción)","Artículo (Código)","Formato","Uso","Cliente","Stock Obsoleto","Base Cliente","Ubicado P-1-1","Ubicado Todos los P","Familia","Calidad","Promoción","Submarca","Colección"}
end type
...
CHOOSE CASE ddlb_campo.text
	...
	CASE "Articulo (Descripción)"
		...
		ddlb_operador.AddItem ("=")
		ddlb_operador.text = "="
		consulta = "Select distinct almacenmovimientos.articulo as Código, articulos.descripcion as Descripción, formatos.abreviado as Formato, art_ver_tipooperacion.descripcion_abr as Operación, almusos.descripcion AS Uso "+&
						...
						"INNER JOIN art_ver_tipooperacion ON articulos.empresa = art_ver_tipooperacion.empresa AND articulos.tipo_operacion = art_ver_tipooperacion.codigo "+&
						"where almacenmovimientos.empresa = '"+codigo_empresa+"' order by 2"
	CASE "Artículo (Código)"
		dw_filtro.visible = false
		pb_buscar.visible = false
		sle_texto.visible = false
...
type dw_datos from wt_consultas_padre`dw_datos within wtc_consulta_almacen_articulo
integer y = 512
integer width = 7543
integer height = 2216
integer taborder = 50
...
type pb_salir from wt_consultas_padre`pb_salir within wtc_consulta_almacen_articulo
integer x = 7392
integer y = 124
integer taborder = 0
end type

type st_titulo from wt_consultas_padre`st_titulo within wtc_consulta_almacen_articulo
integer width = 2670
end type

type p_logo from wt_consultas_padre`p_logo within wtc_consulta_almacen_articulo
integer x = 7026
integer y = 16
end type

type dw_filtro from wt_consultas_padre`dw_filtro within wtc_consulta_almacen_articulo
integer x = 1303
integer y = 240
integer width = 3849
integer height = 260
...
type pb_buscar from wt_consultas_padre`pb_buscar within wtc_consulta_almacen_articulo
integer x = 2871
integer y = 132
integer taborder = 80
end type

type sle_valor1 from wt_consultas_padre`sle_valor1 within wtc_consulta_almacen_articulo
integer x = 1312
integer width = 398
integer height = 96
integer taborder = 20
...
type em_fecha1 from wt_consultas_padre`em_fecha1 within wtc_consulta_almacen_articulo
integer x = 1307
integer height = 96
integer taborder = 90
string text = ""
...
type sle_texto from wt_consultas_padre`sle_texto within wtc_consulta_almacen_articulo
integer x = 1312
integer y = 132
integer height = 96
end type

type lb_lista from wt_consultas_padre`lb_lista within wtc_consulta_almacen_articulo
integer x = 1312
integer y = 132
integer width = 462
integer height = 356
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Calcular v. stock con Coste Actual"
end type
...
For i = 1 To dw_datos.rowcount() 
	...
	f_calculo_coste_articulo_mp_fab_packing_compras (codigo_empresa, articulo , coste_mp, coste_fab, coste_mermas, coste_packing, coste_compras)
	coste =  coste_mp + coste_fab + coste_packing + coste_compras
//	coste =  ftc_calculo_coste_antiguo_articulo(codigo_empresa, articulo,  con_mermas, coste_packing)
	if coste < 0 then
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Calcular v stock con coste de Tabla Hist."
end type
...
progreso.SetStep = 1
progreso.Position = 0
cb_calcular_hist.visible = false
st_info.visible = true
progreso.visible = true
...
dec{6} v_coste_fab, v_coste_mp, v_coste_packing, v_coste_compras
For i = 1 To dw_datos.rowcount() 
	piezas =  Dec(dw_datos.object.stock_piezas[i])
	
	coste =  ftc_calculo_coste_articulo_hist(codigo_empresa, String(dw_datos.object.articulo[i]),  con_mermas, fecha, v_coste_fab, v_coste_mp, v_coste_packing, v_coste_compras)
	
//	coste =  ftc_calculo_coste_antiguo_articulo(codigo_empresa, String(dw_datos.object.articulo[i]),  con_mermas)
	if coste < 0 then
		MessageBox(String(dw_datos.object.articulo[i]), coste)
	end if
	coste = piezas * coste
	dw_datos.object.coste_hist[i] = coste
	
	progreso.StepIt()
	
Next

dw_datos.object.coste_hist.visible = true
dw_datos.object.t_coste_hist.visible = true
dw_datos.object.coste_hist_total.visible = true

st_info.visible = false
progreso.visible = false
cb_calcular_hist.visible = true

orden_seleccionado = "coste_hist D"
dw_datos.setsort(orden_seleccionado)
dw_datos.sort()

/*
Int i, con_mermas
Decimal piezas, coste, v_coste_fab, v_coste_mp
datetime fecha
...
For i = 1 To dw_datos.rowcount() 
	...
	into :v_coste_fab, :v_coste_mp
	from articulos_coste
	where empresa = :codigo_empresa
	and fecha in ( select max (fecha) from articulos_coste where fecha <= :fecha );

	if isnull(v_coste_fab) then v_coste_fab = 0
	if isnull(v_coste_mp) then v_coste_mp = 0
	coste =  v_coste_Fab + v_coste_mp
