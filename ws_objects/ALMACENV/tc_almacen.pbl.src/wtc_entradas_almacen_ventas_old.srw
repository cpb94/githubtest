$PBExportHeader$wtc_entradas_almacen_ventas_old.srw
forward
global type wtc_entradas_almacen_ventas_old from window
end type
type dw_stock from datawindow within wtc_entradas_almacen_ventas_old
end type
type dw_pedidos from datawindow within wtc_entradas_almacen_ventas_old
end type
type dw_observaciones from datawindow within wtc_entradas_almacen_ventas_old
end type
type pb_nuevo from picturebutton within wtc_entradas_almacen_ventas_old
end type
type dw_comlinubica from datawindow within wtc_entradas_almacen_ventas_old
end type
type dw_detalle from datawindow within wtc_entradas_almacen_ventas_old
end type
type dw_movimientos from u_dw within wtc_entradas_almacen_ventas_old
end type
type pb_grabar from picturebutton within wtc_entradas_almacen_ventas_old
end type
type pb_salir from picturebutton within wtc_entradas_almacen_ventas_old
end type
type dp_fecha from datepicker within wtc_entradas_almacen_ventas_old
end type
type st_1 from statictext within wtc_entradas_almacen_ventas_old
end type
type st_2 from statictext within wtc_entradas_almacen_ventas_old
end type
end forward
...
global wtc_entradas_almacen_ventas_old wtc_entradas_almacen_ventas_old
...
public function integer f_insertar_movimiento_cajas (ref transaction mi_transaccion)
...
public subroutine f_calcular_pallets_caja (integer pallets_intro, integer cajas_intro, integer piezas_intro)
end prototypes
...
dw_comlinubica.SetTransObject(trans_ts)					
...
IF rtn = 1 then
	...
	if dw_movimientos.rowcount() > 0 and rtn = 1 then
		...
		param_colisiones.filtro = " fecha_mov = '"+fecha+"' "
		param_colisiones.buscar_huecos = false
		
		nuevo_codigo = String(f_colisiones ( trans_ts, param_colisiones))
		if nuevo_codigo = '0'  then
...
IF rtn = 1 THEN
	...
	if ld_disponible > 0 then
		//Asignamos stock a pedidos
		string ls_sel,ls_referencia
...
if rtn = 1 then
	...
	dw_detalle.retrieve (string (datetime (dp_fecha.value) ) )
...
public function integer f_insertar_movimiento_cajas (ref transaction mi_transaccion);// Devuelve 1 si es correcto
...
string mp, caja_ventas, v_almacen
long fila_actual, nummov
string familia, tipo_unidad
dec cajas_necesarias, cajas_disponibles
dec linea
long orden
boolean faltan_cajas

param_colisiones.empresa = codigo_empresa
param_colisiones.tabla = "comlinubicahis"
param_colisiones.campo = "nummov"
param_colisiones.filtro = " anyo = "+string (year(today()))
param_colisiones.buscar_huecos = false

nummov= long(f_colisiones ( mi_transaccion, param_colisiones))

fila_actual = dw_comlinubica.getrow()
...
into :familia, :tipo_unidad
from venmprima
where empresa = :codigo_empresa
and codigo = :mp 
using mi_transaccion;
	
orden = 	f_calcula_orden(mi_transaccion )
if orden = 0 then
	rtn = 0
else
	com_ubihis.empresa       		= codigo_empresa
	com_ubihis.anyo          		= year(date(String(dp_fecha.value)))
	com_ubihis.almacen			= dw_comlinubica.object.comlinubica_almacen[fila_actual]
	com_ubihis.orden				= orden
	com_ubihis.nummov   			= nummov	
	...
	com_ubihis.mp		       		= mp
	com_ubihis.familia				= dw_comlinubica.object.comlinubica_familia[fila_actual]
	com_ubihis.tipomov			= '6'
	com_ubihis.tipo_unidad		= dw_comlinubica.object.comlinubica_tipo_unidad[fila_actual]
	com_ubihis.f_alta				= datetime(today(),now())
	com_ubihis.usuario				= nombre_usuario
	com_ubihis.externo			= dw_comlinubica.object.comlinubica_externo[fila_actual]
	com_ubihis.envase				= dw_comlinubica.object.comlinubica_envase[fila_actual]
	com_ubihis.calidad				= dw_comlinubica.object.comlinubica_calidad[fila_actual]
	com_ubihis.situacion			= 'N'
	com_ubihis.fecha				= datetime(today())
	com_ubihis.precio				= dw_comlinubica.object.comlinubica_precio[fila_actual]
	com_ubihis.comp_almacen   = ''
	com_ubihis.comp_anyo       = 0
	com_ubihis.comp_nummov   = 0
	...
	com_mov.precio				= com_ubihis.precio
	com_mov.codtercero			= ''
	com_mov.numdoc				= ''
	com_mov.situacion			= com_ubihis.situacion
	com_mov.familia				= com_ubihis.familia
	com_mov.calidad				= com_ubihis.calidad
	com_mov.tipo_unidad			= com_ubihis.tipo_unidad
	com_mov.f_alta				= com_ubihis.f_alta
	com_mov.usuario				= com_ubihis.usuario
	com_mov.externo				= com_ubihis.externo
	com_mov.envase				= com_ubihis.envase
	com_mov.comp_almacen   = ''
	com_mov.comp_anyo       = 0
	com_mov.comp_nummov   = 0
	
	cajas_necesarias = dw_movimientos.object.cajas[dw_movimientos.getrow( )] 
	cajas_disponibles = dw_comlinubica.object.comlinubica_existencias[dw_comlinubica.getrow()] 
	linea = dw_comlinubica.object.comlinubica_linea[dw_comlinubica.getrow()]

	if cajas_disponibles  > cajas_necesarias then
		update comlinubica
		set existencias = :cajas_disponibles - :cajas_necesarias
		where empresa = :codigo_empresa
		and ubicacion = :com_ubihis.ubicacion
		and linea = :linea 
		USING mi_transaccion;
			
		if mi_transaccion.Sqlcode <> 0 then
			rtn = 0
			messagebox("Error", "No se ha poedido actualizar la tabla de existencias (comlinubica)")
		end if
	elseif cajas_necesarias = cajas_disponibles then
...
IF dw_movimientos.rowcount() > 0 THEN
	...
	if not falta_campo then
		...
		select altura_desde, altura_hasta
...
public subroutine f_calcular_pallets_caja (integer pallets_intro, integer cajas_intro, integer piezas_intro);Integer pallets, cajas
...
integer taborder = 40
string title = "none"
string dataobject = "dwtc_consulta_existencias_caja"
boolean border = false
boolean livescroll = true
end type

event itemchanged;long fila, i
...
if dw_comlinubica.rowcount() > 0 then
	if dwo.name = 'marcado' then
		fila = dw_comlinubica.getrow()
		for i = 1 to dw_comlinubica.rowcount()
			if i = fila then
				dw_comlinubica.object.marcado[i] = 1
			else
				dw_comlinubica.object.marcado[i] = 0
			end if
		next
	end if		
end if
	...
	CHOOSE CASE dwo.name
		...
		CASE "articulo"
			...
			SELECT ARTICULOS.CODIGO, ARTICULOS.DESCRIPCION, ARTICULOS.FORMATO, FORMATOS.ABREVIADO, ALMARTCAJAS.CAJA ,
						ALMCAJAS.DESCRIPCION_ABR, PALARTICULO.CODIGO, PALLETS.DESCRIPCION, TONO, CALIBRE, ARTICULOS.USO, ALMARTCAJAS.PIEZASCAJA, ARTICULOS.FAMILIA
			INTO :articulos_codigo, :articulos_descripcion, :articulos_formato, :formatos_abreviado, :almartcajas_caja, 
					:almcajas_descripcion_abr, :palarticulo_codigo, :pallets_descripcion, :tono, :calibre, :uso, :piezascaja, :familia
...
//										" ORDER BY ARTICULOS.DESCRIPCION "
			...
			if resultado.resultado > 0 then
				...
				//LAS CAJAS SE PIDEN AL FINAL
				dw_comlinubica.reset()
				dw_comlinubica.visible = false
				/*
				if this.object.articulos_uso[this.GetRow()] = '3' and this.object.cajas[this.GetRow()] > 0 then
					dw_comlinubica.settransobject( SQLCA)
					caja_ventas = dw_movimientos.object.caja[dw_movimientos.getrow( )]
...
datetime value = DateTime(Date("2018-03-20"), Time("10:52:10.000000"))
integer textsize = -10
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
dw_detalle.retrieve(string(dtm))
dw_detalle.setsort( "orden D")
dw_detalle.sort( )
