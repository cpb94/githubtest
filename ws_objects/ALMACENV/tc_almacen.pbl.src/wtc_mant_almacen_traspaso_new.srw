$PBExportHeader$wtc_mant_almacen_traspaso_new.srw
forward
global type wtc_mant_almacen_traspaso_new from wt_ventana_padre
end type
type dw_salidas from datawindow within wtc_mant_almacen_traspaso_new
end type
type dw_mover from u_dw within wtc_mant_almacen_traspaso_new
end type
type pb_busqueda_codigo from picturebutton within wtc_mant_almacen_traspaso_new
end type
type dw_pedidos from datawindow within wtc_mant_almacen_traspaso_new
end type
type st_1 from statictext within wtc_mant_almacen_traspaso_new
end type
end forward
...
global wtc_mant_almacen_traspaso_new wtc_mant_almacen_traspaso_new
...
if dw_mover.object.piezas[1] > dw_1.object.stock_total[dw_1.getrow()] then
...
if ll_almubimapa_codbar_id_alm_bultos > 0 then
	...
	if dw_mover.object.mostrar_todos_los_bultos[1] = 'S' then
		...
		if dw_mover.object.permitir_mas_de_pallet[1] <> 'S' then
		ls_sel+= "GROUP BY alm_bultos.id, "+&
					"			 palarticulo.cajaspallet, "+&
					"			 almartcajas.piezascaja "+&
	...
	if dw_mover.object.traspasar_reservado[1] = 'S' then		
		...
		if dw_mover.object.piezas_reservado[1] = 0 then		
			dw_mover.Object.id_bulto_destino[1] = dw_1.object.alm_bultos_id[fila]
		else			
			if ll_total > 0 then
...
on wtc_mant_almacen_traspaso_new.create
int iCurrent
call super::create
...
on wtc_mant_almacen_traspaso_new.destroy
call super::destroy
...
type pb_imprimir from wt_ventana_padre`pb_imprimir within wtc_mant_almacen_traspaso_new
...
type p_logo from wt_ventana_padre`p_logo within wtc_mant_almacen_traspaso_new
integer x = 4974
integer y = 8
end type

type dw_1 from wt_ventana_padre`dw_1 within wtc_mant_almacen_traspaso_new
integer width = 5467
integer height = 820
string title = "Mantenimeinto de Colecciones"
string dataobject = "dwtc_almacen_traspaso_new"
...
//		end if
		...
		dw_mover.object.piezas_no_reservado[1] = Dec(dw_1.object.stock_totaL[row]) - ld_cantidad_reservada
		if Dec(dw_1.object.stock_totaL[row]) - ld_cantidad_reservada > 0 then
...
type pb_borrar from wt_ventana_padre`pb_borrar within wtc_mant_almacen_traspaso_new
boolean visible = false
integer x = 3063
integer y = 52
boolean enabled = false
...
type pb_salir from wt_ventana_padre`pb_salir within wtc_mant_almacen_traspaso_new
integer x = 5330
integer y = 124
end type

type pb_grabar from wt_ventana_padre`pb_grabar within wtc_mant_almacen_traspaso_new
boolean visible = false
integer x = 2734
integer y = 52
boolean enabled = false
...
type st_titulo from wt_ventana_padre`st_titulo within wtc_mant_almacen_traspaso_new
end type

type pb_nuevo from wt_ventana_padre`pb_nuevo within wtc_mant_almacen_traspaso_new
boolean visible = false
integer x = 2894
integer y = 52
boolean enabled = false
...
type pb_cancelar from wt_ventana_padre`pb_cancelar within wtc_mant_almacen_traspaso_new
boolean visible = false
integer x = 3218
integer y = 52
boolean enabled = false
...
type pb_buscar from wt_ventana_padre`pb_buscar within wtc_mant_almacen_traspaso_new
integer x = 37
...
string title = "none"
string dataobject = "dwtc_almacen_traspaso_dia_new"
boolean vscrollbar = true
boolean border = false
...
string title = ""
string dataobject = "dwtc_almacen_traspaso_aux_new"
boolean border = true
...
CHOOSE CASE dwo.name
	...
	CASE "pallets"
		...
		if this.object.piezas_reservado[row] > 0 then
			...
			this.event itemchanged(row,this.object.traspasar_no_reservado,this.object.traspasar_no_reservado[row])
		end if
	CASE "cajas"
...
boolean bringtotop = true
string title = "none"
string dataobject = "dw_ver_pedidos_new"
boolean vscrollbar = true
boolean border = false
boolean livescroll = true
end type
