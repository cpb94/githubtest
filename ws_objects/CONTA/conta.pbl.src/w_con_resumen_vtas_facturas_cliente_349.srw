$PBExportHeader$w_con_resumen_vtas_facturas_cliente_349.srw
$PBExportComments$€
forward
global type w_con_resumen_vtas_facturas_cliente_349 from w_int_con_empresa
end type
type pb_1 from upb_salir within w_con_resumen_vtas_facturas_cliente_349
end type
type pb_2 from upb_imprimir within w_con_resumen_vtas_facturas_cliente_349
end type
type em_fechadesde from u_em_campo within w_con_resumen_vtas_facturas_cliente_349
end type
type em_fechahasta from u_em_campo within w_con_resumen_vtas_facturas_cliente_349
end type
type dw_listado from datawindow within w_con_resumen_vtas_facturas_cliente_349
end type
type cb_1 from commandbutton within w_con_resumen_vtas_facturas_cliente_349
end type
type gb_3 from groupbox within w_con_resumen_vtas_facturas_cliente_349
end type
type uo_cliente from u_em_campo_2 within w_con_resumen_vtas_facturas_cliente_349
end type
type gb_2 from groupbox within w_con_resumen_vtas_facturas_cliente_349
end type
type cb_2 from commandbutton within w_con_resumen_vtas_facturas_cliente_349
end type
type gb_1 from groupbox within w_con_resumen_vtas_facturas_cliente_349
end type
type cb_3 from u_boton within w_con_resumen_vtas_facturas_cliente_349
end type
type dw_lineas from datawindow within w_con_resumen_vtas_facturas_cliente_349
end type
type dw_proceso from datawindow within w_con_resumen_vtas_facturas_cliente_349
end type
type em_telefono_contacto from u_em_campo within w_con_resumen_vtas_facturas_cliente_349
end type
type em_persona_contacto from u_em_campo within w_con_resumen_vtas_facturas_cliente_349
end type
type st_1 from statictext within w_con_resumen_vtas_facturas_cliente_349
end type
type st_2 from statictext within w_con_resumen_vtas_facturas_cliente_349
end type
type pb_4 from picturebutton within w_con_resumen_vtas_facturas_cliente_349
end type
type dw_detalle from datawindow within w_con_resumen_vtas_facturas_cliente_349
end type
type gb_10 from groupbox within w_con_resumen_vtas_facturas_cliente_349
end type
end forward
...
global w_con_resumen_vtas_facturas_cliente_349 w_con_resumen_vtas_facturas_cliente_349
...
string   v_periodo_numero_identificativo
...
if li_mes_desde = li_mes_hasta or li_mes_desde+1 = li_mes_hasta then
	...
	v_periodo_numero_identificativo = v_ejercicio+v_periodo
else
	if li_mes_desde = 1 and li_mes_hasta = 12 then
		//Anual
		v_periodo = "0A"
		ls_indicador_cambio_periodicidad = " "
		v_periodo_numero_identificativo = v_ejercicio+"01"
	else
		
		//Trimestral
		ls_indicador_cambio_periodicidad = " "
		if month(date(em_fechadesde.text)) >=1 Then 
			v_periodo = "1T"
			v_periodo_numero_identificativo = v_ejercicio+"01"
		else
			if month(date(em_fechadesde.text)) >3 Then 
				v_periodo = "2T"
				v_periodo_numero_identificativo = v_ejercicio+"02"
			else
				if month(date(em_fechadesde.text)) >6 Then 
					v_periodo = "3T"
					v_periodo_numero_identificativo = v_ejercicio+"03"
				else
					if month(date(em_fechadesde.text)) >9 Then 
						v_periodo = "4T"
						v_periodo_numero_identificativo = v_ejercicio+"04"
					end if
				end if
			end if
		end if
...
for v_indice = 1 to v_total
 ...
 if ds_datos.object.contaapun_tipoter[v_indice] = 'P' then
	v_clipro = ds_datos.object.contaapun_clipro[v_indice]
	v_cuenta_operador_intracomunitario = ""
	
	select max(isnull(comproveedores_cuenta.cuenta,''))
	into   :v_cuenta_operador_intracomunitario
	from   comproveedores_cuenta
	where  comproveedores_cuenta.empresa = :codigo_empresa
	and    comproveedores_cuenta.codigo  = :v_clipro ;
 else
  ...
  if v_base_imponible_decimal <> 0 then
   if v_base_imponible_decimal > 0 then
    ...
    choose case v_tipoter
     ...
     case "P"
      //Proveedores        
		if left(v_cuenta_operador_intracomunitario,3) = "410" then
...
v_numero_de_justificante_declaracion          = '3490000'+v_periodo_numero_identificativo
...
for v_indice = 1 to v_total
	f_mensaje_proceso('Calculando',v_indice,v_total)
	v_cuenta_operador_intracomunitario = ds_datos.object.genter_cuenta[v_indice]
	if left(v_cuenta_operador_intracomunitario,3) <> "410" or 1 = 1 then
		//No es un agente
...
on w_con_resumen_vtas_facturas_cliente_349.create
int iCurrent
call super::create
...
on w_con_resumen_vtas_facturas_cliente_349.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_resumen_vtas_facturas_cliente_349
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_resumen_vtas_facturas_cliente_349
integer x = 110
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_resumen_vtas_facturas_cliente_349
integer width = 2359
integer height = 108
end type
