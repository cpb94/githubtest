$PBExportHeader$wi_mant_contaiva.srw
forward
global type wi_mant_contaiva from wi_mvent_con_empresa
end type
type pb_calculadora from u_calculadora within wi_mant_contaiva
end type
type uo_ejercicio from u_ejercicio within wi_mant_contaiva
end type
type gb_1 from groupbox within wi_mant_contaiva
end type
end forward
...
global wi_mant_contaiva wi_mant_contaiva

type variables
datawindowchild  idwc_tipoalm,idwc_codcli
end variables

event open;call super::open;   istr_parametros.s_titulo_ventana  = "Mantenimiento de tipos de iva"
...
type dw_1 from wi_mvent_con_empresa`dw_1 within wi_mant_contaiva
integer x = 23
integer y = 280
integer width = 3602
integer height = 1644
string dataobject = "dw_contaiva"
end type
...
CASE 'pb_libro'
   ...
   OpenWithParm(wi_mant_contalibros,lstr_param)
...
event dw_1::rbuttondown; bus_filtro=""
 ...
 CHOOSE CASE bus_campo
	CASE "ctaiva"
      ...
      bus_filtro      = "ejercicio = " + uo_ejercicio.em_ejercicio.text 
	CASE "recargo"
      bus_titulo     = "VENTANA SELECCION DE TIPO DE IVA "
 		bus_datawindow = "dw_ayuda_contaiva"
      bus_filtro      = "ejercicio = " + uo_ejercicio.em_ejercicio.text 
CASE "libro"
      bus_titulo     = "VENTANA SELECCION DE LIBROS DE IVA "
 		bus_datawindow = "dw_ayuda_contalibros"
      bus_filtro      = "ejercicio = " + uo_ejercicio.em_ejercicio.text
CASE ELSE
		SetNull(bus_campo)
...
if rowcount > 0 then
	...
	if ls_tipo_libro = 'V' then
		this.modify("t_sii_sujeta.visible = true")			
		this.modify("sii_sujeta.visible = true")					
		this.modify("t_sii_isp.visible = false")			
		this.modify("sii_isp.visible = false")			
		
		if this.Object.sii_sujeta[rowcount] = 'S' then
			...
			this.object.sii_exenta[rowcount] = 'S'
			...
			ls_valores_sii_causa_exencion =  "/No sujeta art 7,14,otros~tN1"+&
														"/No sujeta TAI~tN2"
			
			this.Object.sii_causa_exencion.Values = ls_valores_sii_causa_exencion
			
			this.modify("sii_causa_exencion.visible = true")
			this.modify("t_sii_causa_exencion.text = 'Tipo no Sujeta:'")	
			this.modify("t_sii_causa_exencion.visible = true")									
		end if							
		/*
		this.modify("sii_exenta.visible = true")
		...
		*/
		...
		this.modify("t_sii_sujeta.visible = false")			
		this.modify("sii_sujeta.visible = false")							
		this.object.sii_sujeta[rowcount] = 'S'
		this.modify("sii_exenta.visible = false")
		this.modify("t_sii_exenta.visible = false")			
		this.modify("sii_tipo_no_exenta.visible = false")
		this.modify("t_sii_tipo_no_exenta.visible = false")	
		this.modify("sii_causa_exencion.visible = false")
		this.modify("t_sii_causa_exencion.visible = false")					
		this.modify("t_sii_isp.visible = true")			
		this.modify("sii_isp.visible = true")					
	end if
	...
	if ls_valor_inicial <> "" then
		...
		if ls_tipo_libro = 'V' then
			...
			ls_valor_inicial = this.Object.sii_sujeta.Initial
...
event dw_1::itemchanged;call super::itemchanged;int     ejercicio
...
choose case dwo.name	
	...
	case "sii_exenta"
		if data = 'S' then
			...
			this.modify("sii_causa_exencion.visible = true")
			this.modify("t_sii_causa_exencion.text = 'Causa Exención:'")	
			this.modify("t_sii_causa_exencion.visible = true")						
	...
	case "sii_sujeta"
		if data = 'S' then
			...
			this.event itemchanged(row,this.object.sii_exenta,this.object.sii_exenta[row])
		else
			this.object.sii_exenta[row] = 'S'
			...
			this.object.sii_causa_exencion[row] = "N1"
...
type st_1 from wi_mvent_con_empresa`st_1 within wi_mant_contaiva
integer x = 201
integer y = 136
integer width = 270
...
type cb_insertar from wi_mvent_con_empresa`cb_insertar within wi_mant_contaiva
integer x = 1687
integer y = 152
integer height = 84
end type

type cb_borrar from wi_mvent_con_empresa`cb_borrar within wi_mant_contaiva
integer x = 2094
integer y = 152
integer height = 84
end type

type sle_valor from wi_mvent_con_empresa`sle_valor within wi_mant_contaiva
integer x = 507
integer y = 136
integer width = 370
end type
...
type cb_salir from wi_mvent_con_empresa`cb_salir within wi_mant_contaiva
integer x = 2501
integer y = 152
integer height = 84
end type

type control_tabulador from wi_mvent_con_empresa`control_tabulador within wi_mant_contaiva
end type

type p_logotencer from wi_mvent_con_empresa`p_logotencer within wi_mant_contaiva
end type

type st_empresa from wi_mvent_con_empresa`st_empresa within wi_mant_contaiva
integer width = 2834
end type
