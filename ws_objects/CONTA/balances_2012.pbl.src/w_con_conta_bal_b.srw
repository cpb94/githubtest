$PBExportHeader$w_con_conta_bal_b.srw
forward
global type w_con_conta_bal_b from w_int_con_empresa
end type
type pb_1 from upb_salir within w_con_conta_bal_b
end type
type pb_2 from upb_imprimir within w_con_conta_bal_b
end type
type dw_detalle from datawindow within w_con_conta_bal_b
end type
type em_fechadesde from u_em_campo within w_con_conta_bal_b
end type
type em_fechahasta from u_em_campo within w_con_conta_bal_b
end type
type gb_1 from groupbox within w_con_conta_bal_b
end type
type cb_calcular from commandbutton within w_con_conta_bal_b
end type
type uo_balance from u_em_campo_2 within w_con_conta_bal_b
end type
type gb_2 from groupbox within w_con_conta_bal_b
end type
type gb_3 from groupbox within w_con_conta_bal_b
end type
type uo_manejo from u_manejo_datawindow within w_con_conta_bal_b
end type
type pb_excel from picturebutton within w_con_conta_bal_b
end type
type cbx_con_cuentas from checkbox within w_con_conta_bal_b
end type
type cbx_version_para_chequeo from checkbox within w_con_conta_bal_b
end type
type cbx_cuentas_importe_cero from checkbox within w_con_conta_bal_b
end type
type cbx_un_ejercicio from checkbox within w_con_conta_bal_b
end type
type gb_4 from groupbox within w_con_conta_bal_b
end type
type istr_saldos_grupo from structure within w_con_conta_bal_b
end type
end forward
...
global w_con_conta_bal_b w_con_conta_bal_b
...
dec    ld_porcentaje_ant,ld_saldo6,ld_saldo7,ld_saldo6_ant,ld_saldo7_ant,ld_saldo129,ld_saldo129_ant
...
datetime ldt_f_desde,ldt_f_hasta,ldt_f_desde_ant,ldt_f_hasta_ant,ldt_f_hasta_aux
...
//ldt_f_hasta_aux = datetime(relativedate(date(ldt_f_hasta),1))
//
//ldt_f_desde_ant = datetime(mdy(month(date(ldt_f_desde)),day(date(ldt_f_desde)),year(date(ldt_f_desde)) -1))
//ldt_f_hasta_ant = datetime(mdy(month(date(ldt_f_hasta_aux)),day(date(ldt_f_hasta_aux)),year(date(ldt_f_hasta_aux)) -1))
//ldt_f_hasta_ant = datetime(relativedate(date(ldt_f_hasta_ant),-1))

ldt_f_desde_ant = datetime(f_misma_fecha_ejercicio_anterior(date(ldt_f_desde)))
ldt_f_hasta_ant = datetime(f_misma_fecha_ejercicio_anterior(date(ldt_f_hasta)))
	...
	if ld_porcentaje > 0 or ld_porcentaje_ant > 0 then
		choose case ls_cuenta
			case "120"
				...
				ld_saldo129  = 0//f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,"129",ldt_f_desde,ldt_f_hasta)	
				...
				ld_saldo129_ant = 0//f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,"129",ldt_f_desde_ant,ldt_f_hasta_ant)	
		...
		ll_donde_presupuesto = 0 //Esto no vale en cristal ids_presupuestos.find("contaplan_cuenta = '"+ls_cuenta+"'",1,ids_presupuestos.rowcount())
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_conta_bal_b
integer x = 2240
integer y = 28
integer taborder = 110
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_conta_bal_b
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_conta_bal_b
integer width = 2427
integer height = 112
end type
...
dw_detalle.Object.DataWindow.Zoom = 75
...
integer width = 4613
integer height = 2176
string dataobject = "dw_con_conta_bal_b"
...
if cbx_version_para_chequeo.checked then
	dw_detalle.dataobject = "dw_con_conta_bal_b_chequeo"
else	
	dw_detalle.dataobject = "dw_con_conta_bal_b"
end if
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string picturename = "C:\Bmp\excel.bmp"
...
if ll_registros > 0 then
	...
	if GetFileSaveName("Seleccione Archivo", ls_ruta_fichero, ls_nombre_fichero, "xls", "Archivos Excel (*.xls), *.xls") = 1 then
		...
		//dw_listado.SaveAs(ls_ruta_fichero, Excel!, true)
