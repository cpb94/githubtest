$PBExportHeader$w_con_conta_bal.srw
forward
global type w_con_conta_bal from w_int_con_empresa
end type
type pb_1 from upb_salir within w_con_conta_bal
end type
type pb_2 from upb_imprimir within w_con_conta_bal
end type
type dw_detalle from datawindow within w_con_conta_bal
end type
type em_fechadesde from u_em_campo within w_con_conta_bal
end type
type em_fechahasta from u_em_campo within w_con_conta_bal
end type
type cb_calcular from commandbutton within w_con_conta_bal
end type
type uo_balance from u_em_campo_2 within w_con_conta_bal
end type
type gb_2 from groupbox within w_con_conta_bal
end type
type gb_3 from groupbox within w_con_conta_bal
end type
type uo_manejo from u_manejo_datawindow within w_con_conta_bal
end type
type pb_excel from picturebutton within w_con_conta_bal
end type
type cbx_version_para_chequeo from checkbox within w_con_conta_bal
end type
type cbx_ocultar_grupos_importe0 from checkbox within w_con_conta_bal
end type
type gb_4 from groupbox within w_con_conta_bal
end type
type cbx_cuentas_importe_cero from checkbox within w_con_conta_bal
end type
type cbx_un_ejercicio from checkbox within w_con_conta_bal
end type
type cbx_con_cuentas from checkbox within w_con_conta_bal
end type
type em_resultado_ejercicio_anterior from editmask within w_con_conta_bal
end type
type em_fechadesde_ant from u_em_campo within w_con_conta_bal
end type
type em_fechahasta_ant from u_em_campo within w_con_conta_bal
end type
type gb_5 from groupbox within w_con_conta_bal
end type
type gb_1 from groupbox within w_con_conta_bal
end type
type istr_saldos_grupo from structure within w_con_conta_bal
end type
end forward
...
cbx_ocultar_grupos_importe0 cbx_ocultar_grupos_importe0
gb_4 gb_4
cbx_cuentas_importe_cero cbx_cuentas_importe_cero
cbx_un_ejercicio cbx_un_ejercicio
cbx_con_cuentas cbx_con_cuentas
em_resultado_ejercicio_anterior em_resultado_ejercicio_anterior
em_fechadesde_ant em_fechadesde_ant
em_fechahasta_ant em_fechahasta_ant
gb_5 gb_5
gb_1 gb_1
end type
global w_con_conta_bal w_con_conta_bal
...
public function istr_saldos_grupo f_saldos_grupo_old (string arg_grupo, integer arg_nivel, long arg_orden, string arg_saldos_en_valor_absoluto, string arg_visible, integer arg_nivel_indentacion)
end prototypes

public function long f_cargar_arbol ();int    li_nivel_indentacion
long   ll_indice,ll_total_grupos,ll_nivel = 1,ll_orden = 0,ll_donde,ll_total_gpos_totaliza,ll_ejercicio,ll_ejercicio_ant,ll_color_texto,ll_color_fondo
string ls_codigo_grupo,ls_codigo_grupo_nivel_superior,ls_nombre_grupo,ls_gpo_totalizador
string ls_sel,ls_gpo_de_ultimo_nivel,ls_linea,ls_saldos_en_valor_absoluto,ls_mostrar_subgrupos,ls_negrita,ls_cambio_de_pagina
...
for ll_indice = 1 to ll_total_grupos
	...
	ls_cambio_de_pagina                = ids_grupos.object.cambio_de_pagina[ll_indice]	
	if ls_cambio_de_pagina = "S" then
		il_pagina ++
	end if
	...
	dw_detalle.object.pagina[ll_donde]                          = il_pagina
	...
	if cbx_ocultar_grupos_importe0.checked and ls_gpo_de_ultimo_nivel = "S" then
		if listr_saldos_grupo.valor_periodo = 0 and listr_saldos_grupo.presupuesto_periodo = 0 and listr_saldos_grupo.valor_periodo_ant = 0 and not(cbx_con_cuentas.checked) then
			dw_detalle.object.visible[ll_donde] = "N"
			dw_detalle.deleterow(ll_donde)
		end if
	end if						
...
//if uo_balance.em_codigo.text = '2' and dec(em_resultado_ejercicio_anterior.text) <> 0 and arg_grupo = '83' then
//	ld_valor_periodo = ld_valor_periodo + dec(em_resultado_ejercicio_anterior.text)
//end if
...
dec    ld_porcentaje_ant,ld_saldo129,ld_saldo129_ant,ld_saldo6,ld_saldo7,ld_saldo6_ant,ld_saldo7_ant
...
ldt_f_desde_ant = datetime(date(em_fechadesde_ant.text))//datetime(mdy(month(date(ldt_f_desde)),day(date(ldt_f_desde)),year(date(ldt_f_desde)) -1))
ldt_f_hasta_ant = datetime(date(em_fechahasta_ant.text))//datetime(mdy(month(date(ldt_f_hasta)),day(date(ldt_f_hasta)),year(date(ldt_f_hasta)) -1))
...
for ll_indice = 1 to ll_total_cuentas
	...
	if ld_porcentaje > 0 or ld_porcentaje_ant > 0 then
		//Nuevo
		choose case left(ls_cuenta,3)
			case "120"
				//Si la cuenta es la 120 (Remanente) se le añade el saldo de la cuenta 129 
				ld_saldo     = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,ls_cuenta,ldt_f_desde,ldt_f_hasta)	
				if dec(em_resultado_ejercicio_anterior.text) <> 0 then
					if dec(em_resultado_ejercicio_anterior.text) > 0 then
						ld_saldo129 -= dec(em_resultado_ejercicio_anterior.text)
					end if
				else
					ld_saldo129  = f_saldo_cuenta(ll_ejercicio_desde,codigo_empresa,"129",ldt_f_desde,ldt_f_hasta)//f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,"129",ldt_f_desde,ldt_f_hasta)
					if ld_saldo129 < 0 then ld_saldo129 = 0
				end if									
				ld_saldo    += ld_saldo129
				...
				ld_saldo129_ant = f_saldo_cuenta(ll_ejercicio_desde,codigo_empresa,"129",ldt_f_desde_ant,ldt_f_hasta_ant)//f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,"129",ldt_f_desde_ant,ldt_f_hasta_ant)	
				if ld_saldo129_ant < 0 then ld_saldo129_ant = 0
				
				ld_saldo_ant   += ld_saldo129_ant
			...
			case "129"
				...
				ld_saldo6    = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,"6",ldt_f_desde,ldt_f_hasta)	
				ld_saldo7    = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,"7",ldt_f_desde,ldt_f_hasta)	
				
				ld_saldo     = ld_saldo6 + ld_saldo7 //f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,ls_cuenta,ldt_f_desde,ldt_f_hasta)	
				ld_saldo     = round((ld_saldo / 100) * ld_porcentaje,2)
		
				ld_saldo6_ant = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,"6",ldt_f_desde_ant,ldt_f_hasta_ant)	
				ld_saldo7_ant = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,"7",ldt_f_desde_ant,ldt_f_hasta_ant)	
				
				ld_saldo_ant = ld_saldo6_ant + ld_saldo7_ant //f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,ls_cuenta,ldt_f_desde_ant,ldt_f_hasta_ant)	
				ld_saldo_ant = round((ld_saldo_ant / 100) * ld_porcentaje_ant,2)
			case else
				ld_saldo     = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,ls_cuenta,ldt_f_desde,ldt_f_hasta)	
				ld_saldo     = round((ld_saldo / 100) * ld_porcentaje,2)
...
//		ld_saldo     = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde,codigo_empresa,ls_cuenta,ldt_f_desde,ldt_f_hasta)	
//		
//		if left(ls_cuenta,3) = '120' and uo_balance.em_codigo.text = '2'  then
//			if dec(em_resultado_ejercicio_anterior.text) <> 0 then
//				ld_saldo -= dec(em_resultado_ejercicio_anterior.text)
//			else
//				ld_saldo  = f_saldo_cuenta(ll_ejercicio_desde,codigo_empresa,'129',ldt_f_desde,ldt_f_hasta)	
//			end if
//		end if		
//		
//		ld_saldo     = round((ld_saldo / 100) * ld_porcentaje,2)
...
for ll_indice = 1 to ll_total_cuentas
	...
	if ld_porcentaje_ant > 0 then
		...
		choose case left(ls_cuenta,3)
			case "120"
				//Si la cuenta es la 120 (Remanente) se le pone el saldo de la cuenta 129
				ld_saldo     = 0
				ld_saldo     = round((ld_saldo / 100) * ld_porcentaje,2)
		
		
				ld_saldo_ant    = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,ls_cuenta,ldt_f_desde_ant,ldt_f_hasta_ant)	
				ld_saldo129_ant = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,"129",ldt_f_desde_ant,ldt_f_hasta_ant)	
				if ld_saldo129_ant < 0 then ld_saldo129_ant = 0
				ld_saldo_ant   += ld_saldo129_ant		
				ld_saldo_ant = round((ld_saldo_ant / 100) * ld_porcentaje_ant,2)		
...
string ls_codigo_grupo,ls_codigo_grupo_nivel_superior,ls_nombre_grupo,ls_gpo_totalizador
string ls_gpo_de_ultimo_nivel,ls_linea,ls_saldos_en_valor_absoluto,ls_mostrar_subgrupos,ls_negrita,ls_cambio_de_pagina
dec    ld_valor_periodo,ld_presupuesto_periodo
...
for ll_indice = 1 to ll_total_grupos
	...
	ls_cambio_de_pagina                = ids_grupos2.object.cambio_de_pagina[ll_indice]	
	if ls_cambio_de_pagina = "S" then
		il_pagina ++
	end if		
...
public function istr_saldos_grupo f_saldos_grupo_old (string arg_grupo, integer arg_nivel, long arg_orden, string arg_saldos_en_valor_absoluto, string arg_visible, integer arg_nivel_indentacion);dec    ld_saldo,ld_saldo_total,ld_porcentaje,ld_saldo_ant,ld_saldo_ant_total,ld_presupuesto,ld_presupuesto_total
dec    ld_porcentaje_ant,ld_saldo129,ld_saldo129_ant,ld_saldo6,ld_saldo7,ld_saldo6_ant,ld_saldo7_ant
int    li_mes,li_mes_desde,li_mes_hasta,li_nivel_indentacion
long   ll_indice,ll_total_cuentas,ll_ejercicio_desde,ll_ejercicio_hasta,ll_nivel,ll_donde,ll_ejercicio_desde_ant,ll_ejercicio_hasta_ant
long   ll_donde_presupuesto,ll_donde_cuenta_ant,ll_color_texto,ll_color_fondo
	...
	if ld_porcentaje > 0 or ld_porcentaje_ant > 0 then
		...
		choose case left(ls_cuenta,3)
			case "120"
				...
				ld_saldo_ant    = f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,ls_cuenta,ldt_f_desde_ant,ldt_f_hasta_ant)	
				ld_saldo129_ant = f_saldo_cuenta(ll_ejercicio_desde,codigo_empresa,"129",ldt_f_desde,ldt_f_hasta)//f_saldo_cuenta_balance_npgc(ll_ejercicio_desde_ant,codigo_empresa,"129",ldt_f_desde_ant,ldt_f_hasta_ant)	
				ld_saldo_ant   += ld_saldo129_ant
				ld_saldo_ant    = round((ld_saldo_ant / 100) * ld_porcentaje_ant,2)			
...
event open;call super::open;istr_parametros.s_titulo_ventana = "Balances"
...
em_fechadesde_ant.text = String(MDY(01,01,Year(Today())-1))
em_fechahasta_ant.text = String(MDY(12,31,Year(Today())-1))

dw_detalle.SetTransObject(SQLCA)
dw_detalle.Object.DataWindow.Zoom = 133
...
this.cbx_ocultar_grupos_importe0=create cbx_ocultar_grupos_importe0
...
this.em_resultado_ejercicio_anterior=create em_resultado_ejercicio_anterior
this.em_fechadesde_ant=create em_fechadesde_ant
this.em_fechahasta_ant=create em_fechahasta_ant
...
this.Control[iCurrent+6]=this.cb_calcular
...
this.Control[iCurrent+12]=this.cbx_version_para_chequeo
this.Control[iCurrent+13]=this.cbx_ocultar_grupos_importe0
...
this.Control[iCurrent+17]=this.cbx_con_cuentas
this.Control[iCurrent+18]=this.em_resultado_ejercicio_anterior
this.Control[iCurrent+19]=this.em_fechadesde_ant
this.Control[iCurrent+20]=this.em_fechahasta_ant
this.Control[iCurrent+21]=this.gb_5
this.Control[iCurrent+22]=this.gb_1
end on

on w_con_conta_bal.destroy
call super::destroy
...
destroy(this.cbx_ocultar_grupos_importe0)
destroy(this.gb_4)
destroy(this.cbx_cuentas_importe_cero)
destroy(this.cbx_un_ejercicio)
destroy(this.cbx_con_cuentas)
destroy(this.em_resultado_ejercicio_anterior)
destroy(this.em_fechadesde_ant)
destroy(this.em_fechahasta_ant)
destroy(this.gb_5)
destroy(this.gb_1)
end on
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_conta_bal
integer x = 2240
integer y = 28
integer taborder = 110
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_conta_bal
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_conta_bal
integer width = 2427
integer height = 112
end type
...
u_imp.isSubTitulo = uo_balance.em_campo.text+" ["+em_fechadesde.text+" a "+em_fechahasta.text+"] ["+em_fechadesde_ant.text+" a "+em_fechahasta_ant.text+"]"
...
//		//dw_detalle.SaveAs(ls_ruta_fichero, Excel!, true)
//		
//		guardaraexcel2(dw_detalle,ls_ruta_fichero)
//	end if
...
long textcolor = 33554432
long backcolor = 12632256
string text = "Ocultar grupos con importe 0"
end type

event clicked;if this.checked then
	cbx_cuentas_importe_cero.checked = false
	cbx_con_cuentas.checked = false
end if
end event
...
event clicked;if this.checked then
	cbx_ocultar_grupos_importe0.checked = false
end if
end event
