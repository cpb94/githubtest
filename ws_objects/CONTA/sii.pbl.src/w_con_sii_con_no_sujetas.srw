$PBExportHeader$w_con_sii_con_no_sujetas.srw
forward
global type w_con_sii_con_no_sujetas from w_int_con_empresa
end type
type pb_1 from upb_salir within w_con_sii_con_no_sujetas
end type
type cb_consultar from commandbutton within w_con_sii_con_no_sujetas
end type
type gb_1 from groupbox within w_con_sii_con_no_sujetas
end type
type cb_enviar from u_boton within w_con_sii_con_no_sujetas
end type
type gb_3 from groupbox within w_con_sii_con_no_sujetas
end type
type pb_2 from upb_imprimir within w_con_sii_con_no_sujetas
end type
type em_fecha_desde from u_em_campo within w_con_sii_con_no_sujetas
end type
type em_fecha_hasta from u_em_campo within w_con_sii_con_no_sujetas
end type
type cb_1 from u_boton within w_con_sii_con_no_sujetas
end type
type p_calendario from picture within w_con_sii_con_no_sujetas
end type
type cbx_pruebas from checkbox within w_con_sii_con_no_sujetas
end type
type cbx_reenviar from checkbox within w_con_sii_con_no_sujetas
end type
type p_refrescar_dia from picture within w_con_sii_con_no_sujetas
end type
type dw_calendario from u_datawindow within w_con_sii_con_no_sujetas
end type
type dw_detalle from datawindow within w_con_sii_con_no_sujetas
end type
type str_fichero_respuesta from structure within w_con_sii_con_no_sujetas
end type
end forward
...
global w_con_sii_con_no_sujetas w_con_sii_con_no_sujetas
...
long     ll_indice,ll_total,ll_ejercicio,ll_donde,ll_existe_registro
long     ll_indice_tipos_impositivos,ll_total_tipos_impositivos
string   ls_moneda_empresa,sel,ls_formula_ventas,ls_formula_compras,ls_venfac_serie,ls_sel,ls_filtro
string   ls_tipoiva_recargo
integer  li_decimales_moneda_empresa
...
IF ll_total > 0 then	
	for ll_indice = 1 to ll_total
		...
		//ID
		string ls_pais_tercero,ls_tipo_pais_tercero,ls_ID
		...
		if ls_tipo_pais_tercero = 'N' then
			//Nacional
			ls_ID = f_cif_genter(lstr_contasii.empresa,lstr_contasii.tipoter,lstr_contasii.tercero)
			ls_ID = f_tratar_nif(ls_ID,"",ls_pais_tercero)
		else			
			if ls_tipo_pais_tercero = 'C' then
				...
				ls_ID         = f_cif_genter(lstr_contasii.empresa,lstr_contasii.tipoter,lstr_contasii.tercero)
				ls_ID         = f_tratar_nif(ls_ID,"",ls_pais_tercero)
				if isnull(ls_ID) or trim(ls_ID) = '' then ls_ID = lstr_contasii.tercero
			end if
		end if					
		//ID
		
		if lstr_contasii.factura = '003278' then
			lstr_contasii.factura = '003278'
		end if
...
ls_nombre_fichero = istr_contasii_parametros.directorio_ficheros+"\EMITIDAS"+string(date(em_fecha_hasta.text),'dd-mm-yyyy')+'.XML'

if lds_fichero.rowcount() > 0 then
	if lds_fichero.saveas(ls_nombre_fichero,text!,false) = -1 then 
		messagebox("ERROR","AL GRABAR EL FICHERO")
		ls_nombre_fichero = ''
	else
		ls_nombre_fichero_utf8 = istr_contasii_parametros.directorio_ficheros+"\EMITIDAS"+string(date(em_fecha_hasta.text),'dd-mm-yyyy')+'UTF8.XML'
		...
		lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml				
			...
			for ll_indice = 1 to ds_datos.rowcount()
				...
				*/
				...
				ls_NumSerieFacturaEmisor       = lstr_contasii.numseriefactura				
				f_tratar_campo(ls_NumSerieFacturaEmisor,'A',1,60)
				...
				lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml					
					...
					lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml						
						...
						lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml									
							...
							if ls_ClaveRegimenEspecialOTrascendencia = '09' then
								//Adquisiciones intracomunitarias de bienes y prestaciones de servicios.~t09"+&
								//Aqui puede haber inversion del sujeto pasivo en las prestaciones de servicio
								ls_sel = "  SELECT contasii_tiposimpositivos.tipoiva, "+&
											"			 contasii_tiposimpositivos.tipoimpositivo, "+&
											"         contasii_tiposimpositivos.exenta, "+&
											"			 contasii_tiposimpositivos.tiponoexenta, "+&
								...
								if ll_total_tipos_impositivos > 0 then
									...
									for ll_indice_tipos_impositivos = 1 to ll_total_tipos_impositivos												
											...
											lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml																										
												...
												//Aqui no deberia haber ningun tipo de iva con recargo
												if ds_tipos_impositivos.object.tiporecargoequivalencia[ll_indice_tipos_impositivos] > 0 then
													ls_DesgloseIVA_TipoRecargoEquivalencia = f_sust(String(ds_tipos_impositivos.object.tiporecargoequivalencia[ll_indice_tipos_impositivos],'#########0.00'),',','.')
													ls_linea_xml = '<sii:TipoRecargoEquivalencia>'+ls_DesgloseIVA_TipoRecargoEquivalencia+'</sii:TipoRecargoEquivalencia>' 
													ll_donde     = lds_fichero.insertrow(0)
									...
									ls_filtro_tipos = "  AND    contasii_tiposimpositivos.entrega_o_servicio <> 'S' "
...
ls_nombre_fichero = istr_contasii_parametros.directorio_ficheros+"\RECIBIDAS"+string(date(em_fecha_hasta.text),'dd-mm-yyyy')+'.XML'
...
if lds_fichero.rowcount() > 0 then
	if lds_fichero.saveas(ls_nombre_fichero,text!,false) = -1 then 
		...
		ls_nombre_fichero_utf8 = istr_contasii_parametros.directorio_ficheros+"\RECIBIDAS"+string(date(em_fecha_hasta.text),'dd-mm-yyyy')+'UTF8.XML'
		ls_run = 'wscript.exe "'+istr_contasii_parametros.directorio_ficheros+'\COM\ScriptConversion.vbs" "'+ls_nombre_fichero+'" "'+ls_nombre_fichero_utf8+'"'
...
on w_con_sii_con_no_sujetas.create
int iCurrent
call super::create
...
on w_con_sii_con_no_sujetas.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_sii_con_no_sujetas
integer x = 992
integer y = 0
integer height = 84
integer taborder = 0
...
type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_sii_con_no_sujetas
integer x = 110
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_sii_con_no_sujetas
integer width = 3451
integer height = 108
end type
...
integer height = 2396
integer taborder = 10
string dataobject = "dw_con_sii_con_no_sujetas_calendario"
...
integer width = 4599
integer height = 2396
string dataobject = "dw_con_sii_con_no_sujetas"
boolean vscrollbar = true
...
for ll_row = 1 to rowcount				
	...
	SELECT isnull(contasii_envios.fecha_envio,getdate()),
	...
	GROUP BY contasii_envios.fecha_envio,
			   contasii_envios.fichero_envio,
			   contasii_envios.resultado_envio 	
	HAVING contasii_envios.fecha_envio = max(contasii_envios.fecha_envio);
