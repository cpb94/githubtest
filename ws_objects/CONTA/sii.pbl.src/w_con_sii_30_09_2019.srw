$PBExportHeader$w_con_sii_30_09_2019.srw
forward
global type w_con_sii_30_09_2019 from w_int_con_empresa
end type
type pb_1 from upb_salir within w_con_sii_30_09_2019
end type
type cb_consultar from commandbutton within w_con_sii_30_09_2019
end type
type gb_1 from groupbox within w_con_sii_30_09_2019
end type
type cb_enviar from u_boton within w_con_sii_30_09_2019
end type
type gb_3 from groupbox within w_con_sii_30_09_2019
end type
type pb_2 from upb_imprimir within w_con_sii_30_09_2019
end type
type em_fecha_desde from u_em_campo within w_con_sii_30_09_2019
end type
type em_fecha_hasta from u_em_campo within w_con_sii_30_09_2019
end type
type cb_1 from u_boton within w_con_sii_30_09_2019
end type
type p_calendario from picture within w_con_sii_30_09_2019
end type
type cbx_pruebas from checkbox within w_con_sii_30_09_2019
end type
type cbx_reenviar from checkbox within w_con_sii_30_09_2019
end type
type p_refrescar_dia from picture within w_con_sii_30_09_2019
end type
type cb_2 from commandbutton within w_con_sii_30_09_2019
end type
type dw_calendario from u_datawindow within w_con_sii_30_09_2019
end type
type tab_1 from tab within w_con_sii_30_09_2019
end type
...
type cbx_dar_por_buenas_las_duplicadas from checkbox within w_con_sii_30_09_2019
end type
type str_fichero_respuesta from structure within w_con_sii_30_09_2019
end type
end forward
...
tab_1 tab_1
cbx_dar_por_buenas_las_duplicadas cbx_dar_por_buenas_las_duplicadas
end type
global w_con_sii_30_09_2019 w_con_sii_30_09_2019
...
public function string sii_alta_facturas_emitidas_sin_desglose (datastore ds_datos, string arg_es_un_reenvio)
...
for ll_indice_eliminadas = 1 to ll_total_eliminadas	
	...
	//Me salto el control en el 2017 por los problemas que hemos tenido
	if lstr_contasii.ejercicio = 2017 then li_enviada_correcta = 0
...
IF ll_total > 0 then	
	...
	//
	if lstr_contasii_calendario.fecha = datetime(date('31-12-2017')) then
		lstr_contasii_calendario.fecha = lstr_contasii_calendario.fecha
	end if
...
public function string sii_alta_facturas_emitidas_sin_desglose (datastore ds_datos, string arg_es_un_reenvio);//8.1. Operativa: Alta de una factura 
//8.1.1. Alta de facturas en el libro de registro de Facturas Emitidas 
//8.1.1.1.Ejemplo mensaje XML de alta  
			...
			lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml					
				...
				lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml		
					...
					lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml		
						...
						ls_linea_xml = '<sii:DesgloseFactura>' //'<sii:DesgloseFactura>' Siempre desglosamos el tipo de opercion
						ll_donde     = lds_fichero.insertrow(0)
						lds_fichero.object.linea_xml[ll_donde] = ls_linea_xml							
						...
						ls_sel = "  SELECT contasii_tiposimpositivos.tipoiva, "+&
									...
									"			 contasii_tiposimpositivos.tiponoexenta, "+&
									"			 contasii_tiposimpositivos.causaexencion, "+&
									"			 sum(contasii_tiposimpositivos.baseimponible) as baseimponible, "+&
									"			 sum(contasii_tiposimpositivos.cuota) as cuota, "+&
									"			 contasii_tiposimpositivos.tiporecargoequivalencia, "+&
									"			 sum(contasii_tiposimpositivos.cuotarecargo) as cuotarecargo "+&
									"	FROM   contasii_tiposimpositivos "+&
									"	WHERE  contasii_tiposimpositivos.empresa = '"+lstr_contasii.empresa+"' "+&
									"	AND	 contasii_tiposimpositivos.ejercicio = "+string(lstr_contasii.ejercicio,"###0")+" "+&
									"	AND	 contasii_tiposimpositivos.factura = '"+lstr_contasii.factura+"' "+&
									"	AND	 contasii_tiposimpositivos.tipoter = '"+lstr_contasii.tipoter+"' "+&
									"	AND	 contasii_tiposimpositivos.tercero = '"+lstr_contasii.tercero+"' "+&
									"  GROUP BY contasii_tiposimpositivos.tipoiva, "+&
									"			 contasii_tiposimpositivos.tipoimpositivo, "+&
									"         contasii_tiposimpositivos.exenta, "+&
									"			 contasii_tiposimpositivos.tiponoexenta, "+&
									"			 contasii_tiposimpositivos.causaexencion, "+&
...
do
	...
	if ll_segundos_hemos_esperado = 10 then
...
on w_con_sii_30_09_2019.create
int iCurrent
call super::create
...
this.cbx_dar_por_buenas_las_duplicadas=create cbx_dar_por_buenas_las_duplicadas
...
this.Control[iCurrent+15]=this.dw_calendario
this.Control[iCurrent+16]=this.tab_1
this.Control[iCurrent+17]=this.cbx_dar_por_buenas_las_duplicadas
end on

on w_con_sii_30_09_2019.destroy
call super::destroy
...
destroy(this.tab_1)
destroy(this.cbx_dar_por_buenas_las_duplicadas)
end on
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_sii_30_09_2019
integer x = 992
integer y = 0
integer height = 84
integer taborder = 0
...
type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_sii_30_09_2019
integer x = 110
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_sii_30_09_2019
integer width = 3451
integer height = 108
end type
...
if messagebox("ATENCIÓN","Se va a proceder a enviar su registro de IVA a la AEAT~n¿Desea continuar?",question!,yesno!,1) = 1 then
	...
	if not(f_activar_directorio(ls_directorio_de_envio)) then
							...
							if f_esperar_respuesta(ls_fichero_respuesta) then
								...
								if ll_resultado_importacion < 1 then
									...
									for ll_indice_respuesta = 1 to ll_total_respuesta 													
										...
										if upper(trim(lstr_contasii_envios.resultado_envio)) = upper('Incorrecto:Factura duplicada') and cbx_dar_por_buenas_las_duplicadas.checked then
		...
		if lb_correcto then
			commit;
			dw_calendario.retrieve(codigo_empresa,f_ejercicio_activo(codigo_empresa))
		else
			rollback;
...
string facename = "Arial"
boolean enabled = false
string text = "Chequear Facturas"
end type

event clicked;//SELECT convert(date,contaapun.fapunte) as fapunte,isnull(contaapun.tipoter,'') as tipoter,isnull(contaapun.clipro,'') as clipro,isnull(contaapun.factura,'') as factura,isnull(contaapun.nif,'') as cif
//FROM  contaapun 
//WHERE contaapun.empresa = '1' 
//AND   datepart(yyyy,contaapun.fapunte) = 2017 
//AND   contaapun.tipoiva in (SELECT  contaiva.tipoiva from contaiva 
//									 WHERE   contaiva.ejercicio = 2017 
//									 AND     contaiva.empresa   = '1' 
//									 AND     contaiva.incluir_en_sii = 'S' ) 
//AND EXISTS (SELECT contaapun_fras_duplicadas.tipoter,contaapun_fras_duplicadas.clipro,contaapun_fras_duplicadas.factura
//				FROM   contaapun as contaapun_fras_duplicadas
//				WHERE  contaapun_fras_duplicadas.empresa = contaapun.empresa
//				AND    contaapun_fras_duplicadas.ejercicio = contaapun.ejercicio
//				AND    contaapun_fras_duplicadas.tipoter = contaapun.tipoter
//				AND    contaapun_fras_duplicadas.clipro = contaapun.clipro
//				AND    contaapun_fras_duplicadas.factura = contaapun.factura
//				AND    contaapun_fras_duplicadas.tipoiva in (SELECT  contaiva.tipoiva from contaiva 
//													 WHERE   contaiva.ejercicio = 2017 
//													 AND     contaiva.empresa   = '1' 
//													 AND     contaiva.incluir_en_sii = 'S' )
//				GROUP BY contaapun_fras_duplicadas.tipoter,contaapun_fras_duplicadas.clipro,contaapun_fras_duplicadas.factura
//				HAVING count(*) > 1);
   ...
   AND    contasii_envios.fecha_envio = (SELECT MAX(contasii_envios_ultimo.fecha_envio) 
	...
	if ls_resultado_envio = "" then
		...
		if pos(upper(ls_resultado_envio),'INCORRECTO') > 0 then
			...
			if pos(upper(ls_resultado_envio),'CORRECTO') > 0 then
				this.object.correcto[ll_row] = 'S'
				this.object.incorrecto[ll_row] = 'N'			
			else
				this.object.correcto[ll_row] = 'N'
