$PBExportHeader$rf_salidas_produccion.srw
forward
global type rf_salidas_produccion from window
end type
type st_piezas_disponibles from statictext within rf_salidas_produccion
end type
type em_piezas_disponibles from editmask within rf_salidas_produccion
end type
type st_piezas_reservadas from statictext within rf_salidas_produccion
end type
type em_piezas_reservadas from editmask within rf_salidas_produccion
end type
type st_piezas_disponibles_totales from statictext within rf_salidas_produccion
end type
type em_piezas_disponibles_totales from editmask within rf_salidas_produccion
end type
type st_pallets from statictext within rf_salidas_produccion
end type
type st_cajas from statictext within rf_salidas_produccion
end type
type st_piezas from statictext within rf_salidas_produccion
end type
type em_pallets from editmask within rf_salidas_produccion
end type
type em_cajas from editmask within rf_salidas_produccion
end type
type em_piezas from editmask within rf_salidas_produccion
end type
type pb_mas_pallet from picturebutton within rf_salidas_produccion
end type
type pb_mas_cajas from picturebutton within rf_salidas_produccion
end type
type pb_mas_piezas from picturebutton within rf_salidas_produccion
end type
type pb_menos_pallet from picturebutton within rf_salidas_produccion
end type
type pb_menos_cajas from picturebutton within rf_salidas_produccion
end type
type pb_menos_piezas from picturebutton within rf_salidas_produccion
end type
type pb_atras from picturebutton within rf_salidas_produccion
end type
type st_paso3 from statictext within rf_salidas_produccion
end type
type st_paso1 from statictext within rf_salidas_produccion
end type
type pb_1 from picturebutton within rf_salidas_produccion
end type
type pb_aceptar from picturebutton within rf_salidas_produccion
end type
type uo_lectura_origen from u_sle_codbar within rf_salidas_produccion
end type
type st_paso2 from statictext within rf_salidas_produccion
end type
end forward
...
string title = "Traspaso a Producción"
...
global rf_salidas_produccion rf_salidas_produccion

type variables
string is_tipo_movimiento_salida  = '102'//'209'
string is_tipo_movimiento_entrada = '103'
long   il_color_fondo_activo,il_color_fondo_inactivo
int    ii_paso_actual = 1
string is_almacen_destino,is_zona_destino
long   il_fila_destino,il_altura_destino
end variables

on rf_salidas_produccion.create
...
on rf_salidas_produccion.destroy
...
st_paso2.text = this.title//f_nombre_almtipomov(codigo_empresa,is_tipo_movimiento_salida)

is_almacen_destino = "7"
is_zona_destino    = "A"
il_fila_destino    = 1
il_altura_destino  = 1
...
long   posi_aux,posi,li_piezas,li_piezascaja
...
if uo_lectura_origen.ii_bultos = 1 and uo_lectura_origen.ii_indice_lineas_bulto > 0 then
	if uo_lectura_origen.ii_anyo_orden_carga = 0 and uo_lectura_origen.is_orden_carga = "" then
		...
		if f_movimiento_almacen(lstr_movimiento_almacen) then
			...
			lstr_movimiento_almacen.tipo_pallet        = uo_lectura_origen.is_codigo_pallet
			lstr_movimiento_almacen.caja               = uo_lectura_origen.is_codigo_caja
			lstr_movimiento_almacen.cantidad           = long(em_piezas.text)
			lstr_movimiento_almacen.orden_carga_anyo   = uo_lectura_origen.ii_anyo_orden_carga
			lstr_movimiento_almacen.orden_carga_codigo = uo_lectura_origen.is_orden_carga
			lstr_movimiento_almacen.orden_carga_linea  = uo_lectura_origen.il_linea_orden_carga
			lstr_movimiento_almacen.tipo_movimiento    = is_tipo_movimiento_entrada
			lstr_movimiento_almacen.observaciones      = ""
			lstr_movimiento_almacen.almacen            = is_almacen_destino
			lstr_movimiento_almacen.zona               = is_zona_destino
			lstr_movimiento_almacen.fila               = il_fila_destino
			lstr_movimiento_almacen.altura             = il_altura_destino
			lstr_movimiento_almacen.tercero            = ""
			lstr_movimiento_almacen.documento          = ""
			lstr_movimiento_almacen.fecha              = datetime(today())
...
event modified;call super::modified;if ib_ubicacion_con_bulto_asociado then
	Messagebox("Error","Ha seleccionado un bulto de destino asociado a una ubicación.~nNo se permite hacer salida de producción de esta ubicación.")
	this.SetFocus()
	this.event getfocus()					
else
