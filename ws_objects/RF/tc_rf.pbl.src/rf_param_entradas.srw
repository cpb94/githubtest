$PBExportHeader$rf_param_entradas.srw
forward
global type rf_param_entradas from window
end type
type p_ayuda_ventana from picture within rf_param_entradas
end type
type pb_1 from picturebutton within rf_param_entradas
end type
type pb_atras from picturebutton within rf_param_entradas
end type
type st_paso1 from statictext within rf_param_entradas
end type
type st_paso2 from statictext within rf_param_entradas
end type
type pb_aceptar from picturebutton within rf_param_entradas
end type
type pb_menos_piezas from picturebutton within rf_param_entradas
end type
type pb_menos_cajas from picturebutton within rf_param_entradas
end type
type pb_menos_pallet from picturebutton within rf_param_entradas
end type
type pb_mas_piezas from picturebutton within rf_param_entradas
end type
type pb_mas_cajas from picturebutton within rf_param_entradas
end type
type pb_mas_pallet from picturebutton within rf_param_entradas
end type
type em_piezas from editmask within rf_param_entradas
end type
type em_cajas from editmask within rf_param_entradas
end type
type em_pallets from editmask within rf_param_entradas
end type
type st_piezas from statictext within rf_param_entradas
end type
type st_cajas from statictext within rf_param_entradas
end type
type st_pallets from statictext within rf_param_entradas
end type
type st_paso3 from statictext within rf_param_entradas
end type
type uo_lectura_pallet from u_sle_codbar within rf_param_entradas
end type
type uo_lectura_caja from u_sle_codbar within rf_param_entradas
end type
end forward
...
global rf_param_entradas rf_param_entradas
...
IF istr_parametros.i_nargumentos = 2 THEN
	...
	if f_cargar_str_rf_parametros_ventanas(long(istr_parametros.s_argumentos[2]),istr_rf_parametros_ventanas) then
		...
		//istr_rf_parametros_ventanas.tipologia_articulo
...
//	end if
	...
	if f_movimiento_almacen(lstr_movimiento_almacen) then
		//if f_promocion_articulo(lstr_movimiento_almacen.empresa,lstr_movimiento_almacen.articulo) = "S" or lstr_movimiento_almacen.tipo_movimiento <> "1" then
		if istr_rf_parametros_ventanas.entrada_consume_cajas <> 'S' then
			commit;
