$PBExportHeader$rf_param_traspasos_lineas.srw
forward
global type rf_param_traspasos_lineas from window
end type
type pb_reset_piezas from picturebutton within rf_param_traspasos_lineas
end type
type p_ayuda_ventana from picture within rf_param_traspasos_lineas
end type
type pb_menos_piezas from picturebutton within rf_param_traspasos_lineas
end type
type pb_menos_cajas from picturebutton within rf_param_traspasos_lineas
end type
type pb_menos_pallet from picturebutton within rf_param_traspasos_lineas
end type
type pb_mas_piezas from picturebutton within rf_param_traspasos_lineas
end type
type pb_mas_cajas from picturebutton within rf_param_traspasos_lineas
end type
type pb_mas_pallet from picturebutton within rf_param_traspasos_lineas
end type
type em_piezas from editmask within rf_param_traspasos_lineas
end type
type em_cajas from editmask within rf_param_traspasos_lineas
end type
type em_pallets from editmask within rf_param_traspasos_lineas
end type
type st_piezas from statictext within rf_param_traspasos_lineas
end type
type st_cajas from statictext within rf_param_traspasos_lineas
end type
type st_pallets from statictext within rf_param_traspasos_lineas
end type
type pb_salir from picturebutton within rf_param_traspasos_lineas
end type
type st_paso3 from statictext within rf_param_traspasos_lineas
end type
type st_paso2 from statictext within rf_param_traspasos_lineas
end type
type st_paso1 from statictext within rf_param_traspasos_lineas
end type
type pb_atras from picturebutton within rf_param_traspasos_lineas
end type
type pb_aceptar from picturebutton within rf_param_traspasos_lineas
end type
type uo_lectura_destino from u_sle_codbar within rf_param_traspasos_lineas
end type
type uo_lectura_origen from u_sle_codbar within rf_param_traspasos_lineas
end type
end forward
...
global rf_param_traspasos_lineas rf_param_traspasos_lineas
...
on rf_param_traspasos_lineas.create
...
on rf_param_traspasos_lineas.destroy
...
if integer(this.text) > 0 then
	...
	posi = pos(cadena,"|",posi_aux)
	if posi <>0 then
		if uo_lectura_origen.ib_ubicacion_gestionada_en_piezas then //and uo_lectura_destino.ib_ubicacion_gestionada_en_piezas then
...
if uo_lectura_destino.il_id_ubicacion = 0 and not(ib_ubicacion_destino_fija) then
				...
				if f_movimiento_almacen(lstr_movimiento_almacen) then					
					...
					if uo_lectura_destino.il_id_ubicacion > 0 then
						...
						if ib_ubicacion_destino_fija then
							lstr_movimiento_almacen.almacen = istr_rf_parametros_ventanas.almacen_entrada
							lstr_movimiento_almacen.zona    = istr_rf_parametros_ventanas.zona_entrada
							lstr_movimiento_almacen.fila    = istr_rf_parametros_ventanas.fila_entrada
							lstr_movimiento_almacen.altura  = istr_rf_parametros_ventanas.altura_entrada
						else
							//Si el bulto de destino no tiene una ubicacion definida(Es un bulto nuevo)
							//Le asignamos la ubicacion del bulto de origen o la de istr_rf_parametros_ventanas si existe
							if istr_rf_parametros_ventanas.almacen_entrada <> '' and istr_rf_parametros_ventanas.zona_entrada <> '' and lstr_movimiento_almacen.fila <> 0 and lstr_movimiento_almacen.altura <> 0 then
								lstr_movimiento_almacen.almacen = istr_rf_parametros_ventanas.almacen_entrada
								lstr_movimiento_almacen.zona    = istr_rf_parametros_ventanas.zona_entrada
								lstr_movimiento_almacen.fila    = istr_rf_parametros_ventanas.fila_entrada
								lstr_movimiento_almacen.altura  = istr_rf_parametros_ventanas.altura_entrada								
...
if ib_ubicacion_destino_fija then
	...
	SELECT isnull(alm_bultos.ean13,'')
	INTO   :ls_ean_13
	FROM   almubimapa_codbar,   
			 alm_bultos  
	WHERE ( almubimapa_codbar.id_alm_bultos = alm_bultos.id ) 
	AND ( ( almubimapa_codbar.empresa       = :codigo_empresa ) 
	AND   ( almubimapa_codbar.almacen       = :istr_rf_parametros_ventanas.almacen_entrada ) 
	AND   ( almubimapa_codbar.zona          = :istr_rf_parametros_ventanas.zona_entrada ) 
	AND   ( almubimapa_codbar.fila          = :istr_rf_parametros_ventanas.fila_entrada ) 
	AND   ( almubimapa_codbar.altura        = :istr_rf_parametros_ventanas.altura_entrada ) );		
...
if istr_rf_parametros_ventanas.entrada_ubicacion_fija = 'S' then
	if ( istr_rf_parametros_ventanas.almacen_entrada        = is_almacen and &
		  ...
		  istr_rf_parametros_ventanas.altura_entrada         = ii_altura ) or il_id_ubicacion = 0 then
...
if lb_control_ubicacion_pasado then
	if il_id_bulto = 0 then
		...
		if il_id_bulto = uo_lectura_origen.il_id_bulto then
			...
			if il_id_ubicacion = 0 and not(ib_ubicacion_destino_fija) then
				//Asignaremos la ubicacion de origen al bulto de destino
//				f_reset()
//				messagebox("¡Atención!","Debe seleccionar un bulto con ubicación o asignarse una ubicación de preparación.",StopSign!)
//				uo_lectura_destino.SetFocus()
//				uo_lectura_destino.event getfocus()			
			else
				if il_id_ubicacion = 0 then
					if not f_ubi_exist(codigo_empresa,istr_rf_parametros_ventanas.almacen_entrada,istr_rf_parametros_ventanas.zona_entrada,istr_rf_parametros_ventanas.fila_entrada,istr_rf_parametros_ventanas.altura_entrada) then
						messagebox("¡Atención!","Ubicación de preparación incorrecta.",StopSign!)
						uo_lectura_destino.SetFocus()
						uo_lectura_destino.event getfocus()								
					end if
				end if
