$PBExportHeader$w_con_pagos_vencimiento_banco.srw
forward
global type w_con_pagos_vencimiento_banco from w_int_con_empresa
end type
type pb_2 from upb_salir within w_con_pagos_vencimiento_banco
end type
type cb_consulta from u_boton within w_con_pagos_vencimiento_banco
end type
type pb_imprimir_preli from picturebutton within w_con_pagos_vencimiento_banco
end type
type dw_listado from datawindow within w_con_pagos_vencimiento_banco
end type
type cb_2 from u_boton within w_con_pagos_vencimiento_banco
end type
type gb_2 from groupbox within w_con_pagos_vencimiento_banco
end type
type gb_1 from groupbox within w_con_pagos_vencimiento_banco
end type
type gb_cuenta from groupbox within w_con_pagos_vencimiento_banco
end type
type em_hasta from u_em_campo within w_con_pagos_vencimiento_banco
end type
type ddlb_1 from dropdownlistbox within w_con_pagos_vencimiento_banco
end type
type st_1 from statictext within w_con_pagos_vencimiento_banco
end type
type st_2 from statictext within w_con_pagos_vencimiento_banco
end type
type em_desde from u_em_campo within w_con_pagos_vencimiento_banco
end type
type cb_3 from u_boton within w_con_pagos_vencimiento_banco
end type
type uo_banco from u_marca_lista within w_con_pagos_vencimiento_banco
end type
type uo_tipo2 from u_marca_lista within w_con_pagos_vencimiento_banco
end type
type dw_1 from datawindow within w_con_pagos_vencimiento_banco
end type
end forward
...
global w_con_pagos_vencimiento_banco w_con_pagos_vencimiento_banco
...
public function string f_filtro_bancos ()
end prototypes
...
public subroutine f_cargar (datawindow dw_data);string   sel,filtro_documentos,banco,busqueda,moneda
string   filtro_bancos
...
filtro_bancos     = f_filtro_bancos()

sel = "select sum(importe) importe_dia,fvto,banco "+&
		"from carpagos "+&
		"where empresa = '"+codigo_empresa+"' "+&
		"and fvto between '"+string(fdesde,formato_fecha_cursores)+"' and '"+string(fhasta,formato_fecha_cursores)+"' "+&
		"and ("+filtro_documentos+") "+&
		"and ("+filtro_bancos+") "+&
		"and situacion between '"+situaciond+"' and '"+situacionh+"' "+&
		"group by fvto,banco"		
...
for indice = 1 to total
	fvto           = pagos.object.fvto[indice]
	banco          = pagos.object.banco[indice]
	...
	busqueda = "dia = "+string(dia,"##")+" and banco = '"+banco+"'"
	...
	if donde = 0 then
		...
		dw_data.object.banco[donde]          = banco
		for indice2 = 1 to 12
...
public function string f_filtro_bancos ();Integer numero,x2
String filtro,filtro1

filtro = " banco = '' "
numero = uo_banco.dw_marca.RowCount()
IF  numero =0 Then return ""
FOR x2 = 1 To numero
   	IF uo_banco.dw_marca.GetItemString(x2,"marca") = "S" THEN
      	IF Trim(filtro) = "" Then
         	filtro1 = " banco = '" + uo_banco.dw_marca.GetItemString(x2,"codigo") + "'"
      	Else
         	filtro1 = " Or banco = '" + uo_banco.dw_marca.GetItemString(x2,"codigo") + "'"
      	End If
      	filtro = filtro  + filtro1
  		END IF
	NEXT
...
event open;call super::open;This.title = " CONSULTA EFECTOS POR VENCIMIENTOS AGRUPADOS POR BANCO"
...
dw_comodin = f_cargar_cursor("Select banco,nombre from carbancos where empresa = '"+codigo_empresa+"'")
registros2 = dw_comodin.RowCOunt()
If registros2 > 0 THEN
	FOR x2= 1 To registros2
		var_codigo2  = dw_comodin.GetItemString(x2,"banco")
		var_texto2   = dw_comodin.GetItemString(x2,"nombre")
		marca2       = "S"
		uo_banco.dw_marca.InsertRow(x2)
		uo_banco.dw_marca.setitem(x2,"marca",marca2)
		uo_banco.dw_marca.setitem(x2,"codigo",var_codigo2)
		uo_banco.dw_marca.setitem(x2,"texto",var_texto2)
	NEXT
END IF
uo_banco.st_titulo1.text="Codigo"
uo_banco.st_titulo2.text="Banco"
...
on w_con_pagos_vencimiento_banco.create
int iCurrent
call super::create
...
on w_con_pagos_vencimiento_banco.destroy
call super::destroy
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_pagos_vencimiento_banco
integer taborder = 0
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_pagos_vencimiento_banco
integer x = 55
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_pagos_vencimiento_banco
integer x = 59
integer width = 2642
end type
...
integer width = 379
integer height = 112
string dataobject = "report_con_pagos_vto_banco2"
...
event clicked;uo_banco.visible = not(uo_banco.visible)
uo_tipo2.visible = false
...
integer width = 3630
integer height = 1672
string dataobject = "dw_con_pagos_vencimiento_banco2"
