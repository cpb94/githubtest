$PBExportHeader$w_con_menu_utigrupos_perfil.srw
forward
global type w_con_menu_utigrupos_perfil from w_int_con_empresa
end type
type gb_1 from groupbox within w_con_menu_utigrupos_perfil
end type
type pb_1 from upb_salir within w_con_menu_utigrupos_perfil
end type
type uo_manejo from u_manejo_datawindow within w_con_menu_utigrupos_perfil
end type
type pb_2 from upb_imprimir within w_con_menu_utigrupos_perfil
end type
type dw_listado from datawindow within w_con_menu_utigrupos_perfil
end type
type cb_aceptar from commandbutton within w_con_menu_utigrupos_perfil
end type
type dw_3 from datawindow within w_con_menu_utigrupos_perfil
end type
type dw_4 from datawindow within w_con_menu_utigrupos_perfil
end type
type dw_2 from datawindow within w_con_menu_utigrupos_perfil
end type
type dw_1 from datawindow within w_con_menu_utigrupos_perfil
end type
type gb_2 from groupbox within w_con_menu_utigrupos_perfil
end type
end forward
...
global w_con_menu_utigrupos_perfil w_con_menu_utigrupos_perfil
...
forward prototypes
public subroutine wf_copiar_perfil (string as_utigrupos_org, string as_emp_org, string as_utigrupos_dst, string as_emp_dst)
public subroutine f_contador_procesos (decimal cont_pro_contador, decimal cont_pro_total)
end prototypes

public subroutine wf_copiar_perfil (string as_utigrupos_org, string as_emp_org, string as_utigrupos_dst, string as_emp_dst);DataStore		lds_origen, lds_destino
Long				ll_cont, ll_total


//DELETE FROM utigrupos_menu WHERE utigrupos = :as_utigrupos_dst AND empresa = :as_emp_dst USING SQLCA;
//
//IF SQLCA.sqlcode = -1 THEN
//	ROLLBACK;
//	MessageBox("Copiar Perfil", "ERROR al borrar utigrupos: " +  SQLCA.SQLErrText)
//	Return
//ELSE
//	COMMIT;
//	lds_destino = f_cargar_cursor ("SELECT * FROM utigrupos_menu" )
//	lds_origen = f_cargar_cursor ("SELECT * FROM utigrupos_menu Where utigrupos = '"  + as_utigrupos_org + "' And  empresa = '" + as_emp_org + "'")
//	
//	ll_total = lds_origen.RowCount()
//	
//	FOR ll_cont = 1 TO ll_total
//		lds_origen.Object.utigrupos[ll_cont] = as_utigrupos_dst
...
lds_destino.SetFilter ( " utigrupos = '"  + as_utigrupos_dst + "' And  empresa = '" + as_emp_dst + "'" )
lds_destino.filter()
...
event open;call super::open;istr_parametros.s_titulo_ventana="Copiar Perfil en permisos de utigrupos por menus"
...
on w_con_menu_utigrupos_perfil.create
int iCurrent
call super::create
...
on w_con_menu_utigrupos_perfil.destroy
call super::destroy
destroy(this.gb_1)
destroy(this.pb_1)
...
type ole_cont_pro from w_int_con_empresa`ole_cont_pro within w_con_menu_utigrupos_perfil
integer x = 1189
integer y = 176
integer taborder = 20
end type

type sle_opcion_orden from w_int_con_empresa`sle_opcion_orden within w_con_menu_utigrupos_perfil
end type

type st_empresa from w_int_con_empresa`st_empresa within w_con_menu_utigrupos_perfil
integer x = 9
integer y = 12
integer width = 3296
integer height = 104
...
IF ll_row = 0 THEN 
	MessageBox("Copiar Perfil", "Debe seleccionar un utigrupos de origen¡", Exclamation!)
	Return
ELSE
	ls_usu_origen = dw_1.Object.utigrupos[ll_row]
END IF
...
IF ll_row = 0 THEN 
	MessageBox("Copiar Perfil", "Debe seleccionar un utigrupos de destino¡", Exclamation!)
	Return
ELSE
	ls_usu_destino = dw_3.Object.utigrupos[ll_row]
END IF
...
boolean bringtotop = true
string title = "none"
string dataobject = "dw_menu_perfil_utigrupos"
